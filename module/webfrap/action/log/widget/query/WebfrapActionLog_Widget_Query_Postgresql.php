 ...
 * @licence WebFrap.net
 */
class WebfrapActionLog_Widget_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
 ...
 /**
  ...
  public function setCalcQuery( $criteria, $params )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_action_log.'.Db::PK.') as '.Db::Q_SIZE );
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_action_log_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_action_log.rowid as "wbfsys_action_log_rowid"', // variant: def-rowid 
      'wbfsys_action_log.content as "wbfsys_action_log_content"', // variant: field_mgmt-attr 
      'wbfsys_action_log.m_time_created as "wbfsys_action_log_m_time_created"', // variant: field_mgmt-attr 
    );
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_action_log.rowid = \''.$part.'\'  or 
                wbfsys_action_log.content = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_action_log.content) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_action_log
      if( isset( $condition['wbfsys_action_log'] ) )
      {
        ...
        if( isset( $whereCond['content']) && trim( $whereCond['content'] ) != ''  )
          $criteria->where( ' wbfsys_action_log.content = \''.$whereCond['content'].'\' ');
        ...
        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_action_log.m_role_change = '.$whereCond['m_role_change'].' ');
        ...
        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_action_log.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_action_log.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_action_log.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_action_log.m_uuid >= \''.$whereCond['m_uuid'].'\' ');
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_action_log.content ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_action_log.content,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
    ...
    {
      $criteria->orderBy( 'wbfsys_action_log.rowid' );
  ...
  public function injectOrder( $criteria, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $criteria->orderBy( 'wbfsys_action_log.rowid' );
      $criteria->selectAlso( 'wbfsys_action_log.rowid as "wbfsys_action_log-rowid-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_action_log-rowid-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_action_log-rowid-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_action_log-rowid-order"' );
...
}// end class WebfrapActionLog_Widget_Query_Postgresql
