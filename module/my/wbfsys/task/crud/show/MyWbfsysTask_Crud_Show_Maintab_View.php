 ...
 * @licence WebFrap.net
 */
class MyWbfsysTask_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der MyWbfsysTask Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityMyWbfsysTask = $this->model->getEntityMyWbfsysTask();
    ...
    (
      ...
      array( 'text' => $entityMyWbfsysTask->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=My.WbfsysTask.show  &amp;objid='.$entityMyWbfsysTask
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=My.WbfsysTask.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-my_wbfsys_task-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'my/wbfsys_task/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'MyWbfsysTask_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-my_wbfsys_task-ref-comments' ) || $params->access->getPathLevel( 'mgmt-wbfsys_task-ref-comments' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mgmt-my_wbfsys_task-ref-attachments' ) || $params->access->getPathLevel( 'mgmt-wbfsys_task-ref-attachments' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=My.WbfsysTask.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_my_wbfsys_task' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_my_wbfsys_task a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=My.WbfsysTask.listing' );
      }
    });
    ...
    self.getObject().find(".wgtac_mask_wbfsys_task").click(function(){
      \$S('#{$this->id}_dropmenu-control').dropdown('remove');
      self.close();
      \$R.get('maintab.php?c=Wbfsys.Task.show&objid={$objid}');
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=My.WbfsysTask.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class MyWbfsysTask_Crud_Show_Maintab_View
