...
{$entries->support}
  ...
  <div 
  	class="wgt-panel-control {$this->view->id}-my_wbfsys_task-acl box-qfd_users"
  	style="display:none;"
  	 >
    <div 
...
HTML;
  ...
  public function injectMenuLogic( $view, $areaId, $params  )
  {
    ...
    self.getObject().find(".wgtac_sync_push_to_entity").click(function(){
      \$R.post( 'ajax.php?c=Acl.Mgmt.pushToEntity&dkey=my_wbfsys_task',{} );
      \$S('#{$this->id}-control').dropdown('close');
    });
    
    self.getObject().find(".wgtac_sync_pull_from_entity").click(function(){
      \$R.post( 'ajax.php?c=ajax.php?c=Acl.Mgmt.pullFromEntity&dkey=my_wbfsys_task',{} );
      \$S('#{$this->id}-control').dropdown('close');
    });
    
    self.getObject().find(".wgtac_empty_qfdu_assignment").click(function(){
      \$R.del( 'ajax.php?c=ajax.php?c=Acl.Mgmt_Qfdu.emptyUsers&dkey=my_wbfsys_task' );
      \$S('#{$this->id}-control').dropdown('close');
    }); 
    ...
    self.getObject().find(".wgtac_mask_wbfsys_task").click(function(){
      \$S('#{$this->id}-control').dropdown('remove');
      self.close( );
      \$R.get( 'maintab.php?c=Acl.Mgmt.listing&dkey=wbfsys_task' );
    });
