...
////////////////////////////////////////////////////////////////////////////////
  ...
  *
  *
  * @param MyMessage_Selection_Query $data
  ...
  *  }
  *
  * @return MyMessage_Selection_Element
  */
  public function createListItem( $data, $access, $params  )
  {
    ...
    $table = new MyMessage_Selection_Element
    (
      'selectionMyMessage',
      $view
    );
    ...
    if( 'ajax' != $view->type )
    {
      ...
      $tabPanel->title = $this->view->i18n->l( 'Selection My Messages', 'my.message.label' ) ;
      $tabPanel->searchKey = 'my_message';
    ...
    \$S('input#wgt-input-{$params->input}').data('assign',function( objid ){
      ...
      \$R.get( 'ajax.php?c=My.Message.data{$loadParam}&objid='+objid );
      \$S.modal.close();
    });
...
WGTJS;
  ...
  */
  public function listEntry( $access, $params, $insert = false  )
  {
    ...
    if( !$params->noParse )
      $view->setAreaContent( 'tabRowMyMessage' , $table->buildAjax() );
...
WGTJS;
   ...
   * die MyMessage listing maske
   *
   * @param MyMessage_Selection_Model $model
   * @param TFlag $params
   * @return void
   */
...
}//end class MyMessage_Selection_Ui
