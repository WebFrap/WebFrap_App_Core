 ...
 * @licence WebFrap.net
 */
class MyMessage_Acl_Maintab_Menu
  extends WgtDropmenu
{
...
{$entries->support}
  ...
  <div 
  	class="wgt-panel-control {$this->view->id}-my_message-acl box-qfd_users"
  	style="display:none;"
  	 >
    <div 
      ...
      <ul>
        <li><a 
          class="wcm wcm_req_ajax" 
            href="modal.php?c=Webfrap.Docu.show&amp;key=my_message-acl" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a>
        </li>
        <!--
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Issue.create&amp;refer=my_message-acl" >{$iconBug} {$this->view->i18n->l('Bug','wbf.label')}</a>
        </li>
        -->
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Faq.create&amp;refer=my_message-acl" >{$iconFaq} {$this->view->i18n->l('Faq','wbf.label')}</a>
        </li>
      </ul>
    </span>
  </li>
...
HTML;
   ...
   *
   * @param MyMessage_Acl_Maintab $view
   * @param int $areaId
   * @param TArray $params
   */
  public function injectMenuLogic( $view, $areaId, $params  )
  {
    ...
    self.getObject().find(".wgtac_mask_entity_rights").click(function(){
      ...
      self.close( );
      \$R.get( 'maintab.php?c=Mgmt.Acl.listing&dkey=wbfsys_message' );
    });
    
    self.getObject().find(".wgtac_masks_overview").click(function(){
      \$R.get( 'modal.php?c=Mgmt.Acl.listAllMasks&dkey=wbfsys_message' );
      \$S('#{$this->id}-control').dropdown('close');
    });
    

    self.getObject().find('#wgt-button-my_message-acl-form-append').click(function(){
    
      if(\$S('#wgt-input-my_message-acl-id_group').val()==''){
        \$D.errorWindow('Error','Please select a group first');
        return false;
      }

      \$R.form('wgt-form-my_message-acl-append');
      \$S('#wgt-form-my_message-acl-append').get(0).reset();
      return false;

    });

    self.getObject().find(".wgtac_sync_push_to_entity").click(function(){
      \$R.post( 'ajax.php?c=Acl.Mgmt.pushToEntity&dkey=my_message',{} );
      \$S('#{$this->id}-control').dropdown('close');
    });
    
    self.getObject().find(".wgtac_sync_pull_from_entity").click(function(){
      \$R.post( 'ajax.php?c=ajax.php?c=Acl.Mgmt.pullFromEntity&dkey=my_message',{} );
      \$S('#{$this->id}-control').dropdown('close');
    });
    
    self.getObject().find(".wgtac_empty_qfdu_assignment").click(function(){
      \$R.del( 'ajax.php?c=ajax.php?c=Acl.Mgmt_Qfdu.emptyUsers&dkey=my_message' );
      \$S('#{$this->id}-control').dropdown('close');
    }); 
    ...
    self.getObject().find(".wgtac_mask_wbfsys_message").click(function(){
      \$S('#{$this->id}-control').dropdown('remove');
      self.close( );
      \$R.get( 'maintab.php?c=Acl.Mgmt.listing&dkey=wbfsys_message' );
    });
...
HTML;
      ...
      if( $acl->access( 'mod-wbfsys>mgmt-wbfsys_message:admin' ) )
      {
        $html .= '<li><a class="wgtac_mask_wbfsys_message" >'.$iconMask.' '.$this->view->i18n->l( 'Message', 'wbfsys.message.label' ).'</a></li>'.NL;
        ++ $numEntries;
      }
...
} // end class MyMessage_Acl_Maintab_Menu */
