...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der MyMessage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityMyMessage = $this->model->getEntityMyMessage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show My Messages: {@text@}',
      'my.message.label',
    ...
    (
      $i18nTitle,
      'maintab.php?c=My.Message.show  &amp;objid='.$entityMyMessage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=My.Message.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-my_message-show-'.$objid;
    $params->refId  = $objid;
    ...
    $form = $this->newForm( 'MyMessage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_mask_wbfsys_message").click(function(){
      \$S('#{$this->id}_dropmenu-control').dropdown('remove');
      self.close();
      \$R.get('maintab.php?c=Wbfsys.Message.show&objid={$objid}');
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=My.Message.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class MyMessage_Crud_Show_Maintab_View
