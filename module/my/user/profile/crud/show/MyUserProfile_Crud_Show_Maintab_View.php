 ...
 * @licence WebFrap.net
 */
class MyUserProfile_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der MyUserProfile Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityMyUserProfile = $this->model->getEntityMyUserProfile();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show My Profile: {@text@}',
      'my.user_profile.label',
      array( 'text' => $entityMyUserProfile->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=My.UserProfile.show  &amp;objid='.$entityMyUserProfile
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=My.UserProfile.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-my_user_profile-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'my/user_profile/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'MyUserProfile_Crud_Show' );
  ...
  $form->setEntityEmbedPerson( $this->model->getEntityEmbedPerson( $entity ) );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-user_roles' ) || $params->access->getPathLevel( 'mgmt-my_user_profile-ref-user_roles' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-user_profiles' ) || $params->access->getPathLevel( 'mgmt-my_user_profile-ref-user_profiles' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=My.UserProfile.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_my_user_profile' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_my_user_profile a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=My.UserProfile.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=My.UserProfile.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class MyUserProfile_Crud_Show_Maintab_View
