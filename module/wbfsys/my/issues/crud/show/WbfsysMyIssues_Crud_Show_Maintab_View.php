 ...
 * @licence WebFrap.net
 */
class WbfsysMyIssues_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMyIssues Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMyIssues = $this->model->getEntityWbfsysMyIssues();
    ...
    (
      ...
      array( 'text' => $entityWbfsysMyIssues->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MyIssues.show  &amp;objid='.$entityWbfsysMyIssues
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MyIssues.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_my_issues-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/my_issues/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMyIssues_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_my_issues-ref-comments' ) || $params->access->getPathLevel( 'mgmt-wbfsys_issue-ref-comments' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_my_issues-ref-tags' ) || $params->access->getPathLevel( 'mgmt-wbfsys_issue-ref-tags' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MyIssues.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_my_issues' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_my_issues a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MyIssues.listing' );
      }
    });
    ...
    self.getObject().find(".wgtac_mask_wbfsys_issue").click(function(){
      \$S('#{$this->id}_dropmenu-control').dropdown('remove');
      self.close();
      \$R.get('maintab.php?c=Wbfsys.Issue.show&objid={$objid}');
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MyIssues.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMyIssues_Crud_Show_Maintab_View
