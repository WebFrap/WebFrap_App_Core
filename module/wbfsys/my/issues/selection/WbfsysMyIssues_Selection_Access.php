 ...
 * @licence WebFrap.net
 */
class WbfsysMyIssues_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysMyIssues_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_issue.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_issue'), UPPER('mgmt-wbfsys_my_issues') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_issue.rowid ",
      'acls'
    );
...
}//end class WbfsysMyIssues_Selection_Access
