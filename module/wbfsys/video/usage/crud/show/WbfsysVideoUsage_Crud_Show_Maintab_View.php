 ...
 * @licence WebFrap.net
 */
class WbfsysVideoUsage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysVideoUsage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysVideoUsage = $this->model->getEntityWbfsysVideoUsage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Video Usage: {@text@}',
      'wbfsys.video_usage.label',
      array( 'text' => $entityWbfsysVideoUsage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.VideoUsage.show  &amp;objid='.$entityWbfsysVideoUsage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.VideoUsage.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_video_usage-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/video_usage/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysVideoUsage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.VideoUsage.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_video_usage' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_video_usage a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.VideoUsage.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.VideoUsage.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysVideoUsage_Crud_Show_Maintab_View
