 ...
 * @licence WebFrap.net
 */
class WbfsysVideoVariant_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_video_variant_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_video_variant.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_video_variant.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_video_variant_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_video_variant.rowid as "wbfsys_video_variant_rowid"', // variant: def-rowid 
      'wbfsys_video_codec.name as "wbfsys_video_codec_name"', // variant: def-by-context  used source field wbfsys_video_codec
      'wbfsys_video_variant.id_video_codec as "wbfsys_video_variant_id_video_codec"', // ref wbfsys_video_variant def-by-context 
      'wbfsys_audio_codec.name as "wbfsys_audio_codec_name"', // variant: def-by-context  used source field wbfsys_audio_codec
      'wbfsys_video_variant.id_audio_codec as "wbfsys_video_variant_id_audio_codec"', // ref wbfsys_video_variant def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_video_variant' );
    ...
    (
      ...
      null,
      'wbfsys_video_codec'
    );// wbfsys_video_codec  by alias wbfsys_video_codec
    ...
    (
      ...
      null,
      'wbfsys_audio_codec'
    );// wbfsys_audio_codec  by alias wbfsys_audio_codec
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_video_variant.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_video_variant.rowid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_video_variant.rowid = \''.$part.'\' 
              )'
            );
         }
      ...
      // search conditions for  wbfsys_video_variant
      if( isset( $condition['wbfsys_video_variant'] ) )
      {
        $whereCond = $condition['wbfsys_video_variant'];

        if( isset($whereCond['id_video_codec']) && count( $whereCond['id_video_codec'] ) )
          $criteria->where( " wbfsys_video_variant.id_video_codec IN( '".implode("','",$whereCond['id_video_codec'])."' ) " );

        if( isset($whereCond['id_audio_codec']) && count( $whereCond['id_audio_codec'] ) )
          $criteria->where( " wbfsys_video_variant.id_audio_codec IN( '".implode("','",$whereCond['id_audio_codec'])."' ) " );

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_video_variant.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_video_variant.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_video_variant']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_video_variant.id_video ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_video_variant.id_video,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_video_variant-rowid-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_video_variant-rowid-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_video_variant-rowid-order"' );
...
}//end class WbfsysVideoVariant_Table_Query_Postgresql
