 ...
 * @licence WebFrap.net
 */
class WbfsysVideo_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysVideo_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_video.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_video') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_video.rowid ",
      'acls'
    );
...
}//end class WbfsysVideo_Selection_Access
