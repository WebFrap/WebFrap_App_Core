 ...
 * @licence WebFrap.net
 */
class WbfsysVideo_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysVideo Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysVideo = $this->model->getEntityWbfsysVideo();
    ...
    (
      'Show Video: {@text@}',
      'wbfsys.video.label',
      array( 'text' => $entityWbfsysVideo->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Video.show  &amp;objid='.$entityWbfsysVideo
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Video.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_video-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/video/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysVideo_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_video-ref-comments' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_video-ref-tags' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Video.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_video' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_video a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Video.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Video.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysVideo_Crud_Show_Maintab_View
