 ...
 * @licence WebFrap.net
 */
class WbfsysKnowHowRepository_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysKnowHowRepository Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysKnowHowRepository = $this->model->getEntityWbfsysKnowHowRepository();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Know How Repository: {@text@}',
      'wbfsys.know_how_repository.label',
      array( 'text' => $entityWbfsysKnowHowRepository->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Know How Repository: {@text@}',
      'wbfsys.know_how_repository.label',
      array( 'text' => $entityWbfsysKnowHowRepository->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.KnowHowRepository.show  &amp;objid='.$entityWbfsysKnowHowRepository
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.KnowHowRepository.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_know_how_repository-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/know_how_repository/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysKnowHowRepository_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.KnowHowRepository.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_know_how_repository' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_know_how_repository a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.KnowHowRepository.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.KnowHowRepository.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysKnowHowRepository_Crud_Show_Maintab_View
