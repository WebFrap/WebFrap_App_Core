 ...
 * @licence WebFrap.net
 */
class WbfsysDashboard_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
    ...
    /**
    * @var WbfsysDashboard_Crud_Model
    */
    public $model = null;
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysDashboard Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysDashboard = $this->model->getEntityWbfsysDashboard();
    ...
    (
      ...
      array( 'text' => $entityWbfsysDashboard->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Dashboard.show  &amp;objid='.$entityWbfsysDashboard
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Dashboard.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_dashboard-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/dashboard/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysDashboard_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-widgets' ) || $params->access->getPathLevel( 'mgmt-wbfsys_dashboard-ref-widgets' ) )
    {
      // many to many reference
      // if the model is loadable append the tab box for ref: Widgets
      if( Webfrap::loadable( 'WbfsysDashboardWidget_Entity' ) )
      {
        $this->addVar( 'showRefWidgets', true );
       $this->addVar( 'showTabWidgets', true );

      }
      else
      {
        Log::warn( 'The Model for: WbfsysDashboardWidget is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Dashboard.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_dashboard' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_dashboard a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Dashboard.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Dashboard.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysDashboard_Crud_Show_Maintab_View
