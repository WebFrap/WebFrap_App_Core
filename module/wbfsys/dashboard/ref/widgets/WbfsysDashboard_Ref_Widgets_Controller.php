 ...
 * If you want to extend this class write your coden in ControllerWbfsysDashboard
 *
 ...
 * @licence WebFrap.net
 */
class WbfsysDashboard_Ref_Widgets_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des widgets Referenztabs
  * für den wbfsys_dashboard management knoten
  *
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $containerClass = "WbfsysDashboard_Ref_Widgets_".$listType."_Access";
    ...
    $accessWidgets = new $containerClass( );
    $accessWidgets->load( $user->getProfileName(), $params );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-widgets';

    $params->accessWidgets = $accessWidgets;
    $params->paramsWidgets = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessWidgets->listing  )
    {
    ...
    if( !$params->saveFormId )
      $params->saveFormId = 'wgt-form-wbfsys_dashboard-edit-'.$objid;
    ...
    (
      $params->tabId,
      'WbfsysDashboard_Ref_Widgets_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelWidgets = $this->loadModel
    (
      'WbfsysDashboard_Ref_Widgets_'.$listType
    );
    $modelWidgets->setAccess( $accessWidgets );
    
    $view->setModel( $modelWidgets );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysDashboard_Ref_Widgets_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_dashboard-ref-widgets-'.$listType.'-ajax',
    	'WbfsysDashboard_Ref_Widgets_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysDashboardWidget by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {
    ...
    $this->request->removePost( 'wbfsys_dashboard_widget' ); //def wbfsys_dashboard_widget


  } // end public function cleanPost */

 /**
  * connect existining WbfsysMenuEntry to a WbfsysDashboard
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mod-wbfsys-cat-core_data-ref-widgets'
    );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-widgets';

    $params->accessWidgets = $access;
    $params->paramsWidgets = $params;

    $model  = $this->loadModel( 'WbfsysDashboard_Ref_Widgets_Crud' );
    ...
    $entityWbfsysMenuEntry = $orm->get( 'WbfsysMenuEntry', $refId );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysDashboard_Ref_Widgets_'.$listType );
      $listModel = $this->loadModel( 'WbfsysDashboard_Ref_Widgets_'.$listType );
      $listModel->setAccess( $access );
      $ui->setModel( $listModel );

      if( !$ui->listEntry
      ( 
        $conEntiy->getData( 'id_widget' ), 
        $access, 
        $params, 
        true 
      ))
...
} // end WbfsysDashboard_Ref_Widgets_Controller
