 ...
 * @licence WebFrap.net
 */
class WbfsysFileStorage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysFileStorage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysFileStorage = $this->model->getEntityWbfsysFileStorage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show File Storage: {@text@}',
      'wbfsys.file_storage.label',
      array( 'text' => $entityWbfsysFileStorage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.FileStorage.show  &amp;objid='.$entityWbfsysFileStorage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.FileStorage.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_file_storage-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/file_storage/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysFileStorage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.FileStorage.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_file_storage' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_file_storage a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.FileStorage.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.FileStorage.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysFileStorage_Crud_Show_Maintab_View
