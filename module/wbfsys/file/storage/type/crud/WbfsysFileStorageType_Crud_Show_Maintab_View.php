 ...
 * @licence WebFrap.net
 */
class WbfsysFileStorageType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysFileStorageType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysFileStorageType = $this->model->getEntityWbfsysFileStorageType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show file storage Type: {@text@}',
      'wbfsys.file_storage_type.label',
      array( 'text' => $entityWbfsysFileStorageType->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'file storage Type: {@text@}',
      'wbfsys.file_storage_type.label',
      array( 'text' => $entityWbfsysFileStorageType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.FileStorageType.show  &amp;objid='.$entityWbfsysFileStorageType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.FileStorageType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_file_storage_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/file_storage_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysFileStorageType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.FileStorageType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_file_storage_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_file_storage_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.FileStorageType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.FileStorageType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysFileStorageType_Crud_Show_Maintab_View
