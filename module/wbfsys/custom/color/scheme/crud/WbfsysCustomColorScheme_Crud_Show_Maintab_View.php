 ...
 * @licence WebFrap.net
 */
class WbfsysCustomColorScheme_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysCustomColorScheme Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysCustomColorScheme = $this->model->getEntityWbfsysCustomColorScheme();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Custom Color Scheme: {@text@}',
      'wbfsys.custom_color_scheme.label',
      array( 'text' => $entityWbfsysCustomColorScheme->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Custom Color Scheme: {@text@}',
      'wbfsys.custom_color_scheme.label',
      array( 'text' => $entityWbfsysCustomColorScheme->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.CustomColorScheme.show  &amp;objid='.$entityWbfsysCustomColorScheme
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.CustomColorScheme.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_custom_color_scheme-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/custom_color_scheme/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysCustomColorScheme_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.CustomColorScheme.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_custom_color_scheme' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_custom_color_scheme a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.CustomColorScheme.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.CustomColorScheme.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysCustomColorScheme_Crud_Show_Maintab_View
