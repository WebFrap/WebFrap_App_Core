 ...
 * @licence WebFrap.net
 */
class WbfsysCustomAttachment_Crud_Model
  extends Model
{
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function getRequestedEntity( $request )
  {
    ...
    if( $val = $request->data( 'wbfsys_custom_attachment', Validator::EID, 'objid' ) )
    {
      $objid = $val;
    }
    ...
    if( $objid )
    {
      $searchId = $objid;
      $keyType  = 'rowid';
      $entity   = $orm->get( 'WbfsysCustomAttachment', $objid );
    }
    else if( $uuid )
    {
      $searchId = $uuid;
      $keyType  = 'uuid';
      $entity   = $orm->getByUuid( 'WbfsysCustomAttachment', $uuid );
    }
    else if( $accessKey )
    {
      $searchId = $accessKey;
      $keyType  = 'access_key';
      $entity   = $orm->getByKey( 'WbfsysCustomAttachment', $accessKey );
    }
    else
    {
...
////////////////////////////////////////////////////////////////////////////////
    ...
    try
    {
      if( !$entityWbfsysCustomAttachment = $this->getRegisterd( 'entityWbfsysCustomAttachment' ) )
      {
        ...
        (
          ...
          (
            ...
            'wbf.message',
            array( 'key' => 'entityWbfsysCustomAttachment' )
          )
        );
      }
      ...
      if( !$orm->insert( $entityWbfsysCustomAttachment ) )
      {
        ...
        (
          $response->i18n->l
          (
            'Failed to create Custom Attachment {@label@}',
            'wbfsys.custom_attachment.message',
            array
            (
              'label' => $entityText
      ...
      else
      {
        ...
        (
          $response->i18n->l
          (
            'Successfully created Custom Attachment {@label@}',
            'wbfsys.custom_attachment.message',
        ...
        $this->protocol
        (
          'Created New Custom Attachment: '.$entityText,
          'create',
        ...
        if( $saveSrc )
          $orm->update( $entityWbfsysCustomAttachment );
      }
    ...
    try
    {
      ...
      if( !$orm->update( $entityWbfsysCustomAttachment ) )
      {
        ...
        (
          $response->i18n->l
          (
            'Failed to update Custom Attachment {@label@}',
            'wbfsys.custom_attachment.message',
            array
            (
              'label' =>  $entityText
      ...
      else
      {
        ...
        (
          $response->i18n->l
          (
            'Successfully updated Custom Attachment {@label@}',
            'wbfsys.custom_attachment.message',
            array
            (
              'label' =>  $entityText
        ...
        $this->protocol
        (
          'edited Custom Attachment: '.$entityText,
          'edit',
          $entityWbfsysCustomAttachment
        );
   ...
   */
  public function delete( $entityWbfsysCustomAttachment, $params  )
  {
    ...
    $delId     = $entityWbfsysCustomAttachment->getId();
    ...
    try
    {
      ...
      $orm->delete( $entityWbfsysCustomAttachment );

      $response->addMessage
      (
        $response->i18n->l
        (
          'Successfully deleted Custom Attachment {@label@}',
          'wbfsys.custom_attachment.message',
          array( 'label' => $entityText )
        )
      );
        ...
        $this->protocol
        (
          'Deleted Custom Attachment: '.$entityText,
          'delete',
          array( 'WbfsysCustomAttachment', $delId )
        );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function fetchInsertData(  $params )
  {
    ...
    try
    {
      ...
      //management  wbfsys_custom_attachment source wbfsys_custom_attachment
      $entityWbfsysCustomAttachment = $orm->newEntity( 'WbfsysCustomAttachment' );

      if( !$params->fieldsWbfsysCustomAttachment )
      {
        if( isset( $fields['wbfsys_custom_attachment'] )  )
          $params->fieldsWbfsysCustomAttachment  = $fields['wbfsys_custom_attachment'];
        else
          $params->fieldsWbfsysCustomAttachment  = array();
      }
     ...
     $entityWbfsysCustomAttachment->id_file = $entityEntityFile; //src is saved after target
   ...
   * @return boolean
   */
  public function fetchUpdateData( $entityWbfsysCustomAttachment, $params )
  {
    ...
    if( !$idEntityFile = $httpRequest->data( 'entity_file', Validator::EID, 'rowid' ) )
      $idEntityFile = $entityWbfsysCustomAttachment->id_file;
  ...
  public function fetchPostData( $params, $id = null )
  {
    ...
    else
    {
      ...
      if(!$entityWbfsysCustomAttachment = $orm->get( 'WbfsysCustomAttachment',  $id ))
      {
        $response->addError
        (
          $response->i18n->l
          (
            'There is no Custom Attachment with the id: {@id@}',
            'wbfsys.custom_attachment.message',
            array
            (
              'id' => $id
    ...
    if( !$params->fieldsWbfsysCustomAttachment )
      $params->fieldsWbfsysCustomAttachment  = $entityWbfsysCustomAttachment->getCols
      (
        $params->categories
      );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function getInsertFields()
  {
    ...
    $saveFields = array();
    $saveFields['wbfsys_custom_attachment'] = array();
    $saveFields['wbfsys_custom_attachment'][] = 'vid';
        $saveFields['wbfsys_custom_attachment'][] = 'id_entity';
        $saveFields['wbfsys_custom_attachment'][] = 'id_user';
        $saveFields['wbfsys_custom_attachment'][] = 'id_file';
        $saveFields['wbfsys_custom_attachment'][] = 'm_version';
        $saveFields['entity_file'] = array();
    $saveFields['entity_file'][] = 'access_key';
        $saveFields['entity_file'][] = 'name';
        $saveFields['entity_file'][] = 'link';
        $saveFields['entity_file'][] = 'activ';
        $saveFields['entity_file'][] = 'id_type';
        $saveFields['entity_file'][] = 'id_confidentiality';
        $saveFields['entity_file'][] = 'flag_local';
        $saveFields['entity_file'][] = 'id_licence';
        $saveFields['entity_file'][] = 'flag_versioning';
        $saveFields['entity_file'][] = 'id_mediathek';
        $saveFields['entity_file'][] = 'id_storage';
        $saveFields['entity_file'][] = 'max_version';
        $saveFields['entity_file'][] = 'id_visibility';
        $saveFields['entity_file'][] = 'description';
        $saveFields['entity_file'][] = 'mimetype';
        $saveFields['entity_file'][] = 'file_size';
        $saveFields['entity_file'][] = 'file_hash';
...
}//end WbfsysCustomAttachment_Crud_Model
