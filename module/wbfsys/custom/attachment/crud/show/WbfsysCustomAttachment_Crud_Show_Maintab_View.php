 ...
 * @licence WebFrap.net
 */
class WbfsysCustomAttachment_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysCustomAttachment Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysCustomAttachment = $this->model->getEntityWbfsysCustomAttachment();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Custom Attachment: {@text@}',
      'wbfsys.custom_attachment.label',
      array( 'text' => $entityWbfsysCustomAttachment->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.CustomAttachment.show  &amp;objid='.$entityWbfsysCustomAttachment
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.CustomAttachment.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_custom_attachment-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/custom_attachment/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysCustomAttachment_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.CustomAttachment.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_custom_attachment' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_custom_attachment a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.CustomAttachment.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.CustomAttachment.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysCustomAttachment_Crud_Show_Maintab_View
