 ...
 * @licence WebFrap.net
 */
class WbfsysCustomAttachment_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_custom_attachment_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_custom_attachment.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_custom_attachment.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_custom_attachment_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_custom_attachment.rowid as "wbfsys_custom_attachment_rowid"', // variant: def-rowid 
      'wbfsys_file.name as "entity_file_name"', // variant: def-by-context  used refname entity_file
      'wbfsys_file.link as "entity_file_link"', // variant: def-by-context  used refname entity_file
      'wbfsys_file_type.name as "wbfsys_file_type_name"', // variant: def-by-context  used source field wbfsys_file_type
      'wbfsys_file.id_type as "wbfsys_file_id_type"', // ref wbfsys_file def-by-context 
      'wbfsys_file.flag_versioning as "entity_file_flag_versioning"', // variant: def-by-context  used refname entity_file
      'wbfsys_file.description as "entity_file_description"', // variant: def-by-context  used refname entity_file
      'wbfsys_file.rowid as "entity_file_rowid"', // variant: def-rowid  used refname entity_file
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_custom_attachment' );
    ...
    (
      ...
      null,
      'wbfsys_file_type'
    );// wbfsys_file_type  by alias wbfsys_file_type
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_custom_attachment.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_custom_attachment.rowid = '.$condition );
        }
        else
        {
 ...
 /**
  ...
  public function checkConditions( $criteria, array $condition )
  {
    	...
    	if( isset($condition['ids']) && !empty( $condition['ids'] ) )
    	{
				$criteria->where
        (
          'wbfsys_custom_attachment.rowid = IN( '. implode( ', ', $condition['ids'] ) .' ) ';
        );
    	}
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_custom_attachment.rowid = \''.$part.'\' 
              )'
            );
         }
      ...
      // search conditions for  wbfsys_file
      if( isset( $condition['wbfsys_file'] ) )
      {
        $whereCond = $condition['wbfsys_file'];

        if( !$criteria->isJoined( 'wbfsys_file' ) )
        {
    ...
    (
      ...
      null
        ...
        if( isset( $whereCond['link']) && trim( $whereCond['link'] ) != ''  )
          $criteria->where( ' wbfsys_file.link = \''.$whereCond['link'].'\' ');

        if( isset($whereCond['id_type']) && count( $whereCond['id_type'] ) )
          $criteria->where( " wbfsys_file.id_type IN( '".implode("','",$whereCond['id_type'])."' ) " );

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_file.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_file.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_file.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_file.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_file.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_file.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_file.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_file.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_file']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_custom_attachment.vid ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_custom_attachment.vid,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
     ...
     // inject the default order
      ...
      $criteria->orderBy( 'wbfsys_custom_attachment.rowid' );
      $criteria->selectAlso( 'wbfsys_custom_attachment.rowid as "wbfsys_custom_attachment-rowid-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_custom_attachment-rowid-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_custom_attachment-rowid-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_custom_attachment-rowid-order"' );
...
}//end class WbfsysCustomAttachment_Table_Query_Postgresql
