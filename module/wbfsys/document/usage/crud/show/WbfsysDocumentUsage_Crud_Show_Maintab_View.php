 ...
 * @licence WebFrap.net
 */
class WbfsysDocumentUsage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysDocumentUsage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysDocumentUsage = $this->model->getEntityWbfsysDocumentUsage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Document Usage: {@text@}',
      'wbfsys.document_usage.label',
      array( 'text' => $entityWbfsysDocumentUsage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.DocumentUsage.show  &amp;objid='.$entityWbfsysDocumentUsage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.DocumentUsage.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_document_usage-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/document_usage/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysDocumentUsage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.DocumentUsage.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_document_usage' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_document_usage a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.DocumentUsage.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.DocumentUsage.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysDocumentUsage_Crud_Show_Maintab_View
