 ...
 * @licence WebFrap.net
 */
class WbfsysDocumentType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysDocumentType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_document_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_document_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_document_type.rowid ",
      'acls'
    );
...
}//end class WbfsysDocumentType_Selection_Access
