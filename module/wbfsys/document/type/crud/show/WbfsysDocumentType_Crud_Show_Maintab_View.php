 ...
 * @licence WebFrap.net
 */
class WbfsysDocumentType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysDocumentType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysDocumentType = $this->model->getEntityWbfsysDocumentType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show document Type: {@text@}',
      'wbfsys.document_type.label',
      array( 'text' => $entityWbfsysDocumentType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.DocumentType.show  &amp;objid='.$entityWbfsysDocumentType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.DocumentType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_document_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/document_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysDocumentType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.DocumentType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_document_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_document_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.DocumentType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.DocumentType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysDocumentType_Crud_Show_Maintab_View
