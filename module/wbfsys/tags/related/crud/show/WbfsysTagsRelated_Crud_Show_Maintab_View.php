 ...
 * @licence WebFrap.net
 */
class WbfsysTagsRelated_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysTagsRelated Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysTagsRelated = $this->model->getEntityWbfsysTagsRelated();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Tags Related: {@text@}',
      'wbfsys.tags_related.label',
      array( 'text' => $entityWbfsysTagsRelated->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.TagsRelated.show  &amp;objid='.$entityWbfsysTagsRelated
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.TagsRelated.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_tags_related-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/tags_related/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysTagsRelated_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.TagsRelated.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_tags_related' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_tags_related a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.TagsRelated.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.TagsRelated.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysTagsRelated_Crud_Show_Maintab_View
