 ...
 * @licence WebFrap.net
 */
class WbfsysProtocolUsage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProtocolUsage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProtocolUsage = $this->model->getEntityWbfsysProtocolUsage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Protocol Usage: {@text@}',
      'wbfsys.protocol_usage.label',
      array( 'text' => $entityWbfsysProtocolUsage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProtocolUsage.show  &amp;objid='.$entityWbfsysProtocolUsage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProtocolUsage.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_protocol_usage-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/protocol_usage/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProtocolUsage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProtocolUsage.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_protocol_usage' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_protocol_usage a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProtocolUsage.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProtocolUsage.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProtocolUsage_Crud_Show_Maintab_View
