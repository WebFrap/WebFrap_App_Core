 ...
 * @licence WebFrap.net
 */
class WbfsysProtocolFaillog_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProtocolFaillog Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProtocolFaillog = $this->model->getEntityWbfsysProtocolFaillog();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Protocol Faillog: {@text@}',
      'wbfsys.protocol_faillog.label',
      array( 'text' => $entityWbfsysProtocolFaillog->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Protocol Faillog: {@text@}',
      'wbfsys.protocol_faillog.label',
      array( 'text' => $entityWbfsysProtocolFaillog->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProtocolFaillog.show  &amp;objid='.$entityWbfsysProtocolFaillog
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProtocolFaillog.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_protocol_faillog-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/protocol_faillog/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProtocolFaillog_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProtocolFaillog.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_protocol_faillog' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_protocol_faillog a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProtocolFaillog.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProtocolFaillog.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProtocolFaillog_Crud_Show_Maintab_View
