 ...
 * @licence WebFrap.net
 */
class WbfsysIssueType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysIssueType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_issue_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_issue_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_issue_type.rowid ",
      'acls'
    );
...
}//end class WbfsysIssueType_Selection_Access
