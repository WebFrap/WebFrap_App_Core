 ...
 * @licence WebFrap.net
 */
class WbfsysIssueSeverity_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysIssueSeverity_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_issue_severity.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_issue_severity') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_issue_severity.rowid ",
      'acls'
    );
...
}//end class WbfsysIssueSeverity_Selection_Access
