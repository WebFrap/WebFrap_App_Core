 ...
 * @licence WebFrap.net
 */
class WbfsysIssue_Crud_Edit_Maintab_Menu
  extends WgtDropmenu
{
    ...
    <div class="wgt-dropdownbox" id="{$this->id}-cruddropbox" >
      <ul id="{$this->id}-cruddrop-menu"  >
        <li><a
          ...
          title="Save the Issue and close the Tab after saving" >{$iconSaveUCl} {$view->i18n->l('Save &amp; Close','wbf.label')}</a></li>
      </ul>
...
HTML;
  ...
  <div class="wgt-panel-control" >
    <button
      class="wcm wcm_ui_button wgtac_back_to_list wcm_ui_tip-top"
      title="Go back to the Issues list" >{$iconBack} {$view->i18n->l('Back to the list','wbf.label')}</button>
  </div>
      ...
      <li>
        ...
        <ul>
          ...
          <li><a 
            ...
            href="modal.php?c=Webfrap.Docu.open&amp;key=wbfsys_issue-edit" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a></li>
...
HTML;
      ...
      if( $this->acl->access( 'mod-wbfsys/mgmt-wbfsys_issue>mgmt-wbfsys_my_issues:access', $objid ) )
      {
        $html .= '<li><a class="wgtac_mask_wbfsys_my_issues" >'.$iconMask.' '.$this->view->i18n->l('My Issues','wbfsys.my_issues.label').'</a></li>'.NL;
        ++ $numEntries;
      }
...
}//end class WbfsysIssue_Crud_Edit_Maintab_Menu
