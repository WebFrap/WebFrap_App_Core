 ...
 * @licence WebFrap.net
 */
class WbfsysContentLicence_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysContentLicence Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysContentLicence = $this->model->getEntityWbfsysContentLicence();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Content Licence: {@text@}',
      'wbfsys.content_licence.label',
      array( 'text' => $entityWbfsysContentLicence->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Content Licence: {@text@}',
      'wbfsys.content_licence.label',
      array( 'text' => $entityWbfsysContentLicence->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ContentLicence.show  &amp;objid='.$entityWbfsysContentLicence
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ContentLicence.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_content_licence-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/content_licence/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysContentLicence_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ContentLicence.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_content_licence' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_content_licence a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ContentLicence.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ContentLicence.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysContentLicence_Crud_Show_Maintab_View
