 ...
 * @licence WebFrap.net
 */
class WbfsysModuleSetting_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysModuleSetting Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysModuleSetting = $this->model->getEntityWbfsysModuleSetting();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Module Setting: {@text@}',
      'wbfsys.module_setting.label',
      array( 'text' => $entityWbfsysModuleSetting->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ModuleSetting.show  &amp;objid='.$entityWbfsysModuleSetting
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ModuleSetting.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_module_setting-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/module_setting/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysModuleSetting_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ModuleSetting.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_module_setting' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_module_setting a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ModuleSetting.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ModuleSetting.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysModuleSetting_Crud_Show_Maintab_View
