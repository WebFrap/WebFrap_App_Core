 ...
 * @licence WebFrap.net
 */
class WbfsysModule_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysModule Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysModule = $this->model->getEntityWbfsysModule();
    ...
    (
      'Show Module: {@text@}',
      'wbfsys.module.label',
      array( 'text' => $entityWbfsysModule->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Module.show  &amp;objid='.$entityWbfsysModule
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Module.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_module-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/module/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysModule_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-entity' ) || $params->access->getPathLevel( 'mgmt-wbfsys_module-ref-entity' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Entity
      if( Webfrap::loadable( 'WbfsysEntity_Entity' ) )
      {
        $this->addVar( 'showRefEntity', true );
       $this->addVar( 'showTabEntity', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysEntity is not loadable' );
      }
    }

    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-management' ) || $params->access->getPathLevel( 'mgmt-wbfsys_module-ref-management' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Management
      if( Webfrap::loadable( 'WbfsysManagement_Entity' ) )
      {
        $this->addVar( 'showRefManagement', true );
       $this->addVar( 'showTabManagement', true );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Module.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_module' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_module a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Module.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Module.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysModule_Crud_Show_Maintab_View
