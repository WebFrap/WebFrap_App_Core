 ...
 * @licence WebFrap.net
 */
class WbfsysModuleCategory_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysModuleCategory Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysModuleCategory = $this->model->getEntityWbfsysModuleCategory();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Module Category: {@text@}',
      'wbfsys.module_category.label',
      array( 'text' => $entityWbfsysModuleCategory->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ModuleCategory.show  &amp;objid='.$entityWbfsysModuleCategory
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ModuleCategory.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_module_category-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/module_category/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysModuleCategory_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ModuleCategory.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_module_category' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_module_category a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ModuleCategory.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ModuleCategory.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysModuleCategory_Crud_Show_Maintab_View
