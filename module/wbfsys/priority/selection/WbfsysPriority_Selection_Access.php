 ...
 * @licence WebFrap.net
 */
class WbfsysPriority_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysPriority_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_priority.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_priority') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_priority.rowid ",
      'acls'
    );
...
}//end class WbfsysPriority_Selection_Access
