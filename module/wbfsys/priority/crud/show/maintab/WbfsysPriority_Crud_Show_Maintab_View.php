 ...
 * @licence WebFrap.net
 */
class WbfsysPriority_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysPriority Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysPriority = $this->model->getEntityWbfsysPriority();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show issue priority: {@text@}',
      'wbfsys.priority.label',
      array( 'text' => $entityWbfsysPriority->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Priority.show  &amp;objid='.$entityWbfsysPriority
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Priority.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_priority-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/priority/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysPriority_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Priority.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_priority' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_priority a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Priority.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Priority.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysPriority_Crud_Show_Maintab_View
