 ...
 * @licence WebFrap.net
 */
class WbfsysCalendarType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysCalendarType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_calendar_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_calendar_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_calendar_type.rowid ",
      'acls'
    );
...
}//end class WbfsysCalendarType_Selection_Access
