 ...
 * @licence WebFrap.net
 */
class WbfsysCalendarType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysCalendarType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysCalendarType = $this->model->getEntityWbfsysCalendarType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show calendar Type: {@text@}',
      'wbfsys.calendar_type.label',
      array( 'text' => $entityWbfsysCalendarType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.CalendarType.show  &amp;objid='.$entityWbfsysCalendarType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.CalendarType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_calendar_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/calendar_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysCalendarType_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_calendar_type-ref-holiday' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Holiday
      if( Webfrap::loadable( 'WbfsysCalendarHoliday_Entity' ) )
      {
        $this->addVar( 'showRefHoliday', true );
       $this->addVar( 'showTabHoliday', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysCalendarHoliday is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.CalendarType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_calendar_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_calendar_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.CalendarType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.CalendarType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysCalendarType_Crud_Show_Maintab_View
