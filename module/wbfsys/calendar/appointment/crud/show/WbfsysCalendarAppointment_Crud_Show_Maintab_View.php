 ...
 * @licence WebFrap.net
 */
class WbfsysCalendarAppointment_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysCalendarAppointment Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysCalendarAppointment = $this->model->getEntityWbfsysCalendarAppointment();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Calendar Appointment: {@text@}',
      'wbfsys.calendar_appointment.label',
      array( 'text' => $entityWbfsysCalendarAppointment->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Calendar Appointment: {@text@}',
      'wbfsys.calendar_appointment.label',
      array( 'text' => $entityWbfsysCalendarAppointment->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.CalendarAppointment.show  &amp;objid='.$entityWbfsysCalendarAppointment
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.CalendarAppointment.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_calendar_appointment-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/calendar_appointment/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysCalendarAppointment_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.CalendarAppointment.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_calendar_appointment' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_calendar_appointment a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.CalendarAppointment.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.CalendarAppointment.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysCalendarAppointment_Crud_Show_Maintab_View
