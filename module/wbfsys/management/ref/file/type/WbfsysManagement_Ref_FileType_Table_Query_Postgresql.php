 ...
 * @licence WebFrap.net
 */
class WbfsysManagement_Ref_FileType_Table_Query_Postgresql
  extends LibSqlQuery
{
...
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_vref_file_type_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_vref_file_type.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_vref_file_type.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_vref_file_type_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function fetchWithAcls( $condition = null, $params = null )
  {
    ...
    {
      $acl->injectListingAcls( $criteria, 'mod-wbfsys>mgmt-wbfsys_vref_file_type' );
    }
    else
    {
      $acl->injectListingAcls( $criteria, 'mod-wbfsys>mgmt-wbfsys_vref_file_type', true, $params->access->level  );
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_vref_file_type.rowid as "wbfsys_vref_file_type_rowid"', // variant:  
      'wbfsys_file_type.name as "wbfsys_file_type_name"', // variant:  
      'wbfsys_file_type.description as "wbfsys_file_type_description"', // variant:  
      'wbfsys_file_type.mimetype as "wbfsys_file_type_mimetype"', // variant:  
      'wbfsys_file_type.ending as "wbfsys_file_type_ending"', // variant:  
      'wbfsys_file_type.rowid as "wbfsys_file_type_rowid"', // variant:  
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_vref_file_type' );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_vref_file_type.vid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_vref_file_type.vid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_vref_file_type.rowid = \''.$part.'\'  or 
                wbfsys_file_type.name = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_file_type.name) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_file_type
      if( isset( $condition['wbfsys_file_type'] ) )
      {
        $whereCond = $condition['wbfsys_file_type'];

        if( !$criteria->isJoined( 'wbfsys_file_type' ) )
        {
    ...
    (
      ...
      'wbfsys_file_type'
        ...
        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_file_type.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        if( isset( $whereCond['description'] ) && trim( $whereCond['description'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_file_type.description) like UPPER(\'%'.$whereCond['description'].'%\') ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_file_type.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_file_type.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_file_type']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_vref_file_type.vid ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_vref_file_type.vid,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_vref_file_type-rowid-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_vref_file_type-rowid-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_vref_file_type-rowid-order"' );
...
}//end class WbfsysManagement_Ref_FileType_Table_Query_Postgresql
