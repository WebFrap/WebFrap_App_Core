 ...
 * @licence WebFrap.net
 */
class WbfsysManagement_Ref_FileType_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des file_type Referenztabs
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $containerClass = "WbfsysManagement_Ref_FileType_".$listType."_Access";
    ...
    $accessFileType = new $containerClass( );
    $accessFileType->load( $user->getProfileName(), $params );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-file_type';

    $params->accessFileType = $accessFileType;
    $params->paramsFileType = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessFileType->listing  )
    {
    ...
    (
      $params->tabId,
      'WbfsysManagement_Ref_FileType_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelFileType = $this->loadModel
    (
      'WbfsysManagement_Ref_FileType_'.$listType
    );
    $modelFileType->setAccess( $accessFileType );
    
    $view->setModel( $modelFileType );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysManagement_Ref_FileType_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_management-ref-file_type-'.$listType.'-ajax',
    	'WbfsysManagement_Ref_FileType_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysVrefFileType by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {

    $this->request->removePost( 'wbfsys_file_type' ); //def wbfsys_file_type
    $this->request->removePost( 'wbfsys_vref_file_type' ); //def wbfsys_vref_file_type


  } // end public function cleanPost */

 /**
  * connect existining WbfsysFileType to a WbfsysManagement
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mod-wbfsys-cat-core_data-ref-file_type'
    );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-file_type';

    $params->accessFileType = $access;
    $params->paramsFileType = $params;

    $model  = $this->loadModel( 'WbfsysManagement_Ref_FileType_Crud' );
    ...
    $entityWbfsysFileType = $orm->get( 'WbfsysFileType', $refId );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysManagement_Ref_FileType_'.$listType );
      $listModel = $this->loadModel( 'WbfsysManagement_Ref_FileType_'.$listType );
      $listModel->setAccess( $access );
      $ui->setModel( $listModel );

      if( !$ui->listEntry
      ( 
        $conEntiy->getData( 'id_type' ), 
        $access, 
        $params, 
        true 
      ))
...
} // end WbfsysManagement_Ref_FileType_Controller
