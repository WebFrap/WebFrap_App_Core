 ...
 * @licence WebFrap.net
 */
class WbfsysManagement_Ref_FileType_Table_Access
  extends LibAclPermissionList
{
  /**
   * @param TFlag $params
   * @param Entity: wbfsys_vref_file_type Attributes: vid; id_type; rowid; m_time_created; m_role_create; m_time_changed; m_role_change; m_version; m_uuid_Entity $entity
   */
  public function loadDefault( $params, $entity = null )
  {
    ...
    try 
    {
      ...
      $rootPerm = $rootContainer->getRefAccess( $params->aclRootId, $params->aclLevel, 'mod-wbfsys-cat-core_data-ref-file_type' );
  ...
  /**
   * @param WbfsysManagement_Ref_FileType_Table_Query $query
  ...
  public function fetchListTableDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_vref_file_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_vref_file_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_vref_file_type.rowid ",
      'acls'
    );
...
}//end class WbfsysManagement_Ref_FileType_Table_Access
