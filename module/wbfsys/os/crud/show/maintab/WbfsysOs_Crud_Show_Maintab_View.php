 ...
 * @licence WebFrap.net
 */
class WbfsysOs_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysOs Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysOs = $this->model->getEntityWbfsysOs();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Operating System: {@text@}',
      'wbfsys.os.label',
      array( 'text' => $entityWbfsysOs->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Os.show  &amp;objid='.$entityWbfsysOs
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Os.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_os-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/os/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysOs_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-version' ) || $params->access->getPathLevel( 'mgmt-wbfsys_os-ref-version' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Version
      if( Webfrap::loadable( 'WbfsysOsVersion_Entity' ) )
      {
      ...
      {
        Log::warn( 'The Model: WbfsysOsVersion is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Os.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_os' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_os a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Os.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Os.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysOs_Crud_Show_Maintab_View
