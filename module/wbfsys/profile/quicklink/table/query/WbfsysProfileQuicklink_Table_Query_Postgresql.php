 ...
 * @licence WebFrap.net
 */
class WbfsysProfileQuicklink_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_profile_quicklink_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_profile_quicklink.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_profile_quicklink.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_profile_quicklink_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_profile_quicklink.rowid as "wbfsys_profile_quicklink_rowid"', // variant: def-rowid 
      'wbfsys_profile_quicklink.label as "wbfsys_profile_quicklink_label"', // variant: def-by-context 
      'wbfsys_profile_quicklink.http_url as "wbfsys_profile_quicklink_http_url"', // variant: def-by-context 
      'wbfsys_profile_quicklink.short_desc as "wbfsys_profile_quicklink_short_desc"', // variant: def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_profile_quicklink' );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_profile_quicklink.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_profile_quicklink.rowid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_profile_quicklink.rowid = \''.$part.'\' 
              )'
            );
         }
      ...
      // search conditions for  wbfsys_profile_quicklink
      if( isset( $condition['wbfsys_profile_quicklink'] ) )
      {
        $whereCond = $condition['wbfsys_profile_quicklink'];

        if( isset( $whereCond['label'] ) && trim( $whereCond['label'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_profile_quicklink.label) like UPPER(\'%'.$whereCond['label'].'%\') ');

        if( isset( $whereCond['http_url']) && trim( $whereCond['http_url'] ) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.http_url = \''.$whereCond['http_url'].'\' ');

        if( isset( $whereCond['short_desc']) && trim( $whereCond['short_desc'] ) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.short_desc = \''.$whereCond['short_desc'].'\' ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_profile_quicklink.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_profile_quicklink']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_profile_quicklink.id_profile ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_profile_quicklink.id_profile,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_profile_quicklink-rowid-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_profile_quicklink-rowid-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_profile_quicklink-rowid-order"' );
...
}//end class WbfsysProfileQuicklink_Table_Query_Postgresql
