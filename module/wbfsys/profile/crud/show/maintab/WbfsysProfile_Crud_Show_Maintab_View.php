 ...
 * @licence WebFrap.net
 */
class WbfsysProfile_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProfile Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProfile = $this->model->getEntityWbfsysProfile();
    ...
    (
      ...
      array( 'text' => $entityWbfsysProfile->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Profile.show  &amp;objid='.$entityWbfsysProfile
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Profile.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_profile-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/profile/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProfile_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_profile-ref-quicklinks' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Quicklinks
      if( Webfrap::loadable( 'WbfsysProfileQuicklink_Entity' ) )
      {
        $this->addVar( 'showRefQuicklinks', true );
       $this->addVar( 'showTabQuicklinks', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysProfileQuicklink is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Profile.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_profile' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_profile a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Profile.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Profile.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProfile_Crud_Show_Maintab_View
