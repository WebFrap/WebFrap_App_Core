 ...
 * @licence WebFrap.net
 */
class WbfsysPersonDuplicateSuspicion_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysPersonDuplicateSuspicion Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysPersonDuplicateSuspicion = $this->model->getEntityWbfsysPersonDuplicateSuspicion();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Person Duplicate: {@text@}',
      'wbfsys.person_duplicate_suspicion.label',
      array( 'text' => $entityWbfsysPersonDuplicateSuspicion->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Person Duplicate: {@text@}',
      'wbfsys.person_duplicate_suspicion.label',
      array( 'text' => $entityWbfsysPersonDuplicateSuspicion->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.PersonDuplicateSuspicion.show  &amp;objid='.$entityWbfsysPersonDuplicateSuspicion
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.PersonDuplicateSuspicion.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_person_duplicate_suspicion-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/person_duplicate_suspicion/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysPersonDuplicateSuspicion_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.PersonDuplicateSuspicion.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_person_duplicate_suspicion' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_person_duplicate_suspicion a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.PersonDuplicateSuspicion.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.PersonDuplicateSuspicion.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysPersonDuplicateSuspicion_Crud_Show_Maintab_View
