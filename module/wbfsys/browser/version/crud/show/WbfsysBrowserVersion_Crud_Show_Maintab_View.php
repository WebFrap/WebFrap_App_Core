 ...
 * @licence WebFrap.net
 */
class WbfsysBrowserVersion_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysBrowserVersion Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysBrowserVersion = $this->model->getEntityWbfsysBrowserVersion();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Browser Version: {@text@}',
      'wbfsys.browser_version.label',
      array( 'text' => $entityWbfsysBrowserVersion->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.BrowserVersion.show  &amp;objid='.$entityWbfsysBrowserVersion
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.BrowserVersion.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_browser_version-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/browser_version/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysBrowserVersion_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.BrowserVersion.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_browser_version' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_browser_version a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.BrowserVersion.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.BrowserVersion.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysBrowserVersion_Crud_Show_Maintab_View
