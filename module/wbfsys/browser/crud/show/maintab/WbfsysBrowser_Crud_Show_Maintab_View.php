 ...
 * @licence WebFrap.net
 */
class WbfsysBrowser_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysBrowser Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysBrowser = $this->model->getEntityWbfsysBrowser();
    ...
    (
      ...
      array( 'text' => $entityWbfsysBrowser->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Browser.show  &amp;objid='.$entityWbfsysBrowser
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Browser.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_browser-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/browser/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysBrowser_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-version' ) || $params->access->getPathLevel( 'mgmt-wbfsys_browser-ref-version' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Version
      if( Webfrap::loadable( 'WbfsysBrowserVersion_Entity' ) )
      {
        $this->addVar( 'showRefVersion', true );
       $this->addVar( 'showTabVersion', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysBrowserVersion is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Browser.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_browser' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_browser a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Browser.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Browser.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysBrowser_Crud_Show_Maintab_View
