 ...
 * @licence WebFrap.net
 */
class WbfsysMessageLog_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMessageLog Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMessageLog = $this->model->getEntityWbfsysMessageLog();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Message Log: {@text@}',
      'wbfsys.message_log.label',
      array( 'text' => $entityWbfsysMessageLog->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MessageLog.show  &amp;objid='.$entityWbfsysMessageLog
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MessageLog.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_message_log-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/message_log/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMessageLog_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MessageLog.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_log' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_log a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MessageLog.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MessageLog.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMessageLog_Crud_Show_Maintab_View
