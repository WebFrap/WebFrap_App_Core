 ...
 * @licence WebFrap.net
 */
class WbfsysMessageReferences_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMessageReferences Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMessageReferences = $this->model->getEntityWbfsysMessageReferences();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Nachrichten Quelle: {@text@}',
      'wbfsys.message_references.label',
      array( 'text' => $entityWbfsysMessageReferences->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Nachrichten Quelle: {@text@}',
      'wbfsys.message_references.label',
      array( 'text' => $entityWbfsysMessageReferences->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MessageReferences.show  &amp;objid='.$entityWbfsysMessageReferences
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MessageReferences.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_message_references-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/message_references/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMessageReferences_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MessageReferences.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_references' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_references a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MessageReferences.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MessageReferences.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMessageReferences_Crud_Show_Maintab_View
