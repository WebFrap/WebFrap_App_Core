 ...
 * @licence WebFrap.net
 */
class WbfsysMessage_Ref_Link_Table_Access
  extends LibAclPermissionList
{
  /**
   * @param TFlag $params
   * @param Entity: wbfsys_data_link Attributes: id_message; id_link; rowid; m_time_created; m_role_create; m_time_changed; m_role_change; m_version; m_uuid_Entity $entity
   */
  public function loadDefault( $params, $entity = null )
  {
    ...
    try 
    {
      ...
      $rootPerm = $rootContainer->getRefAccess( $params->aclRootId, $params->aclLevel, 'mgmt-wbfsys_message-ref-link' );
  ...
  /**
   * @param WbfsysMessage_Ref_Link_Table_Query $query
  ...
  public function fetchListTableDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_data_link.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_data_link') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_data_link.rowid ",
      'acls'
    );
...
}//end class WbfsysMessage_Ref_Link_Table_Access
