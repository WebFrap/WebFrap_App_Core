 ...
 * @licence WebFrap.net
 */
class WbfsysMessage_Ref_SendWay_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des send_way Referenztabs
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $containerClass = "WbfsysMessage_Ref_SendWay_".$listType."_Access";
    ...
    $accessSendWay = new $containerClass( );
    $accessSendWay->load( $user->getProfileName(), $params );

    $params->aclNode = 'mgmt-wbfsys_message-ref-send_way';

    $params->accessSendWay = $accessSendWay;
    $params->paramsSendWay = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessSendWay->listing  )
    {
    ...
    (
      $params->tabId,
      'WbfsysMessage_Ref_SendWay_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelSendWay = $this->loadModel
    (
      'WbfsysMessage_Ref_SendWay_'.$listType
    );
    $modelSendWay->setAccess( $accessSendWay );
    
    $view->setModel( $modelSendWay );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysMessage_Ref_SendWay_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_message-ref-send_way-'.$listType.'-ajax',
    	'WbfsysMessage_Ref_SendWay_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysMessageSendway by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {

    $this->request->removePost( 'wbfsys_message_repository' ); //def wbfsys_message_repository
    $this->request->removePost( 'wbfsys_message_sendway' ); //def wbfsys_message_sendway


  } // end public function cleanPost */

 /**
  * connect existining WbfsysMessageRepository to a WbfsysMessage
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mgmt-wbfsys_message-ref-send_way'
    );

    $params->aclNode = 'mgmt-wbfsys_message-ref-send_way';

    $params->accessSendWay = $access;
    $params->paramsSendWay = $params;

    $model  = $this->loadModel( 'WbfsysMessage_Ref_SendWay_Crud' );
    ...
    $entityWbfsysMessageRepository = $orm->get( 'WbfsysMessageRepository', $refId );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysMessage_Ref_SendWay_'.$listType );
      $listModel = $this->loadModel( 'WbfsysMessage_Ref_SendWay_'.$listType );
      $listModel->setAccess( $access );
      $ui->setModel( $listModel );

      if( !$ui->listEntry
      ( 
        $conEntiy->getData( 'id_repository' ), 
        $access, 
        $params, 
        true 
      ))
...
} // end WbfsysMessage_Ref_SendWay_Controller
