 ...
 * If you want to extend this class write your coden in ControllerWbfsysMessage
 *
 ...
 * @licence WebFrap.net
 */
class WbfsysMessage_Ref_References_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des references Referenztabs
  * für den wbfsys_message management knoten
  *
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $containerClass = "WbfsysMessage_Ref_References_".$listType."_Access";
    ...
    $accessReferences = new $containerClass( );
    $accessReferences->load( $user->getProfileName(), $params );

    $params->aclNode = 'mgmt-wbfsys_message-ref-references';

    $params->accessReferences = $accessReferences;
    $params->paramsReferences = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessReferences->listing  )
    {
    ...
    if( !$params->saveFormId )
      $params->saveFormId = 'wgt-form-wbfsys_message-edit-'.$objid;
    ...
    (
      $params->tabId,
      'WbfsysMessage_Ref_References_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelReferences = $this->loadModel
    (
      'WbfsysMessage_Ref_References_'.$listType
    );
    $modelReferences->setAccess( $accessReferences );
    
    $view->setModel( $modelReferences );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysMessage_Ref_References_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_message-ref-references-'.$listType.'-ajax',
    	'WbfsysMessage_Ref_References_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysMessageReferences by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {

    $this->request->removePost( 'wbfsys_message' ); //def wbfsys_message
    $this->request->removePost( 'wbfsys_message_references' ); //def wbfsys_message_references


  } // end public function cleanPost */

 /**
  * connect existining WbfsysMessage to a WbfsysMessage
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mgmt-wbfsys_message-ref-references'
    );

    $params->aclNode = 'mgmt-wbfsys_message-ref-references';

    $params->accessReferences = $access;
    $params->paramsReferences = $params;

    $model  = $this->loadModel( 'WbfsysMessage_Ref_References_Crud' );
    ...
    $entityWbfsysMessage = $orm->get( 'WbfsysMessage', $refId );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysMessage_Ref_References_'.$listType );
      $listModel = $this->loadModel( 'WbfsysMessage_Ref_References_'.$listType );
      $listModel->setAccess( $access );
      $ui->setModel( $listModel );

      if( !$ui->listEntry
      ( 
        $conEntiy->getData( 'id_reference' ), 
        $access, 
        $params, 
        true 
      ))
...
} // end WbfsysMessage_Ref_References_Controller
