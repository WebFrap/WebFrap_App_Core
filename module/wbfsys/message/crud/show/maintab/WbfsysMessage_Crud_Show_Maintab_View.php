 ...
 * @licence WebFrap.net
 */
class WbfsysMessage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMessage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMessage = $this->model->getEntityWbfsysMessage();
    ...
    (
      ...
      array( 'text' => $entityWbfsysMessage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Message.show  &amp;objid='.$entityWbfsysMessage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Message.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_message-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/message/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMessage_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_message-ref-tags' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_message-ref-send_way' ) )
    {
      // many to many reference
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_message-ref-link' ) )
    {
      // many to many reference
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_message-ref-references' ) )
    {
      // many to many reference
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_message-ref-attachments' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Message.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Message.listing' );
      }
    });
    

    self.getObject().find(".wgtac_mask_my_message").click(function(){
      \$S('#{$this->id}_dropmenu-control').dropdown('remove');
      self.close();
      \$R.get('maintab.php?c=My.Message.show&objid={$objid}');
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Message.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMessage_Crud_Show_Maintab_View
