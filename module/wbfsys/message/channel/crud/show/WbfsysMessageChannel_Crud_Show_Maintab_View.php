 ...
 * @licence WebFrap.net
 */
class WbfsysMessageChannel_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMessageChannel Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMessageChannel = $this->model->getEntityWbfsysMessageChannel();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Message Channel: {@text@}',
      'wbfsys.message_channel.label',
      array( 'text' => $entityWbfsysMessageChannel->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MessageChannel.show  &amp;objid='.$entityWbfsysMessageChannel
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MessageChannel.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_message_channel-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/message_channel/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMessageChannel_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MessageChannel.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_channel' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_channel a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MessageChannel.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MessageChannel.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMessageChannel_Crud_Show_Maintab_View
