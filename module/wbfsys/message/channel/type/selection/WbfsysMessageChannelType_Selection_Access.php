 ...
 * @licence WebFrap.net
 */
class WbfsysMessageChannelType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysMessageChannelType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_message_channel_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_message_channel_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_message_channel_type.rowid ",
      'acls'
    );
...
}//end class WbfsysMessageChannelType_Selection_Access
