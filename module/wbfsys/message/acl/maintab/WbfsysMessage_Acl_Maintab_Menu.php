 ...
 * @licence WebFrap.net
 */
class WbfsysMessage_Acl_Maintab_Menu
  extends WgtDropmenu
{
...
{$entries->support}
  ...
  <div 
  	class="wgt-panel-control {$this->view->id}-wbfsys_message-acl box-qfd_users"
  	style="display:none;"
  	 >
    <div 
      ...
      <ul>
        <li><a 
          class="wcm wcm_req_ajax" 
            href="modal.php?c=Webfrap.Docu.show&amp;key=wbfsys_message-acl" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a>
        </li>
        <!--
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Issue.create&amp;refer=wbfsys_message-acl" >{$iconBug} {$this->view->i18n->l('Bug','wbf.label')}</a>
        </li>
        -->
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Faq.create&amp;refer=wbfsys_message-acl" >{$iconFaq} {$this->view->i18n->l('Faq','wbf.label')}</a>
        </li>
      </ul>
    </span>
  </li>
...
HTML;
   ...
   *
   * @param WbfsysMessage_Acl_Maintab $view
   * @param int $areaId
   * @param TArray $params
   */
  public function injectMenuLogic( $view, $areaId, $params  )
  {
    ...
    self.getObject().find('#wgt-button-wbfsys_message-acl-form-append').click(function(){
    
      if(\$S('#wgt-input-wbfsys_message-acl-id_group').val()==''){
        \$D.errorWindow('Error','Please select a group first');
        return false;
      }

      \$R.form('wgt-form-wbfsys_message-acl-append');
      \$S('#wgt-form-wbfsys_message-acl-append').get(0).reset();
      return false;
    ...
    self.getObject().find(".wgtac_mask_my_message").click(function(){
      \$S('#{$this->id}-control').dropdown('remove');
      self.close( );
      \$R.get( 'maintab.php?c=Acl.Mgmt.listing&dkey=my_message' );
    });
...
HTML;
      ...
      if( $acl->access( 'mod-wbfsys/mgmt-wbfsys_message>mgmt-my_message:admin' ) )
      {
        $html .= '<li><a class="wgtac_mask_my_message" >'.$iconMask.' '.$this->view->i18n->l( 'My Messages', 'my.message.label' ).'</a></li>'.NL;
        ++ $numEntries;
      }
...
} // end class WbfsysMessage_Acl_Maintab_Menu */
