 ...
 * @licence WebFrap.net
 */
class WbfsysMessageAddresseeType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMessageAddresseeType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMessageAddresseeType = $this->model->getEntityWbfsysMessageAddresseeType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show message addressee Type: {@text@}',
      'wbfsys.message_addressee_type.label',
      array( 'text' => $entityWbfsysMessageAddresseeType->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'message addressee Type: {@text@}',
      'wbfsys.message_addressee_type.label',
      array( 'text' => $entityWbfsysMessageAddresseeType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MessageAddresseeType.show  &amp;objid='.$entityWbfsysMessageAddresseeType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MessageAddresseeType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_message_addressee_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/message_addressee_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMessageAddresseeType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MessageAddresseeType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_addressee_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_addressee_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MessageAddresseeType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MessageAddresseeType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMessageAddresseeType_Crud_Show_Maintab_View
