 ...
 * @licence WebFrap.net
 */
class WbfsysMessageAddressee_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMessageAddressee Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMessageAddressee = $this->model->getEntityWbfsysMessageAddressee();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Message Addressee: {@text@}',
      'wbfsys.message_addressee.label',
      array( 'text' => $entityWbfsysMessageAddressee->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Message Addressee: {@text@}',
      'wbfsys.message_addressee.label',
      array( 'text' => $entityWbfsysMessageAddressee->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MessageAddressee.show  &amp;objid='.$entityWbfsysMessageAddressee
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MessageAddressee.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_message_addressee-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/message_addressee/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMessageAddressee_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MessageAddressee.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_addressee' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_message_addressee a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MessageAddressee.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MessageAddressee.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMessageAddressee_Crud_Show_Maintab_View
