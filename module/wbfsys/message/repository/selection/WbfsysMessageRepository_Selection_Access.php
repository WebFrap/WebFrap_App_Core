 ...
 * @licence WebFrap.net
 */
class WbfsysMessageRepository_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysMessageRepository_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_message_repository.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_message_repository') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_message_repository.rowid ",
      'acls'
    );
...
}//end class WbfsysMessageRepository_Selection_Access
