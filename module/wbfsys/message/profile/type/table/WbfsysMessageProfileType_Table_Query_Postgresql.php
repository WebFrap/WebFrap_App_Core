 ...
 * @licence WebFrap.net
 */
class WbfsysMessageProfileType_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_message_profile_type_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_message_profile_type.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_message_profile_type.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_message_profile_type_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function fetchWithAcls( $condition = null, $params = null )
  {
    ...
    {
      $acl->injectListingAcls( $criteria, 'mod-wbfsys>mgmt-wbfsys_message_profile_type' );
    }
    else
    {
      $acl->injectListingAcls( $criteria, 'mod-wbfsys>mgmt-wbfsys_message_profile_type', true, $params->access->level  );
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_message_profile_type.rowid as "wbfsys_message_profile_type_rowid"', // variant: def-rowid 
      'wbfsys_message_profile_type.name as "wbfsys_message_profile_type_name"', // variant: def-by-context 
      'wbfsys_message_profile_type.description as "wbfsys_message_profile_type_description"', // variant: def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_message_profile_type' );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_message_profile_type.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_message_profile_type.rowid = '.$condition );
        }
        else
        {
 ...
 /**
  ...
  public function checkConditions( $criteria, array $condition )
  {
    	...
    	if( isset($condition['ids']) && !empty( $condition['ids'] ) )
    	{
				$criteria->where
        (
          'wbfsys_message_profile_type.rowid = IN( '. implode( ', ', $condition['ids'] ) .' ) ';
        );
    	}
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_message_profile_type.rowid = \''.$part.'\'  or 
                wbfsys_message_profile_type.name = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_message_profile_type.name) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_message_profile_type
      if( isset( $condition['wbfsys_message_profile_type'] ) )
      {
        $whereCond = $condition['wbfsys_message_profile_type'];

        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_message_profile_type.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        if( isset( $whereCond['description'] ) && trim( $whereCond['description'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_message_profile_type.description) like UPPER(\'%'.$whereCond['description'].'%\') ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_message_profile_type.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_message_profile_type']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_message_profile_type.m_order ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_message_profile_type.m_order,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
     ...
     // inject the default order
      $criteria->orderBy( 'wbfsys_message_profile_type.name ' );


			
      $criteria->selectAlso( 'wbfsys_message_profile_type.name as "wbfsys_message_profile_type-name-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $criteria->selectAlso( 'wbfsys_message_profile_type.name as "wbfsys_message_profile_type-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_message_profile_type-name-order"' );
      $envelop->groupBy( 'inner_acl."wbfsys_message_profile_type-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_message_profile_type-name-order" ' );
...
}//end class WbfsysMessageProfileType_Table_Query_Postgresql
