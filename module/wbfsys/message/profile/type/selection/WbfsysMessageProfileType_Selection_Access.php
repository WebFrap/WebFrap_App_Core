 ...
 * @licence WebFrap.net
 */
class WbfsysMessageProfileType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysMessageProfileType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_message_profile_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_message_profile_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_message_profile_type.rowid ",
      'acls'
    );
...
}//end class WbfsysMessageProfileType_Selection_Access
