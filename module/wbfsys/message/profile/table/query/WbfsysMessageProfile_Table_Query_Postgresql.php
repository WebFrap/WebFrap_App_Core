 ...
 * @licence WebFrap.net
 */
class WbfsysMessageProfile_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_message_profile_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_message_profile.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_message_profile.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_message_profile_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_message_profile.rowid as "wbfsys_message_profile_rowid"', // variant: def-rowid 
      'wbfsys_message_profile.name as "wbfsys_message_profile_name"', // variant: def-by-context 
      'wbfsys_message_profile_type.name as "wbfsys_message_profile_type_name"', // variant: def-by-context  used source field wbfsys_message_profile_type
      'wbfsys_message_profile.id_type as "wbfsys_message_profile_id_type"', // ref wbfsys_message_profile def-by-context 
      'wbfsys_user_contact_visibility.name as "wbfsys_user_contact_visibility_name"', // variant: def-by-context  used source field wbfsys_user_contact_visibility
      'wbfsys_message_profile.id_visibility as "wbfsys_message_profile_id_visibility"', // ref wbfsys_message_profile def-by-context 
      'wbfsys_message_profile.user_name as "wbfsys_message_profile_user_name"', // variant: def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_message_profile' );
    ...
    (
      ...
      null,
      'wbfsys_message_profile_type'
    );// wbfsys_message_profile_type  by alias wbfsys_message_profile_type
    ...
    (
      ...
      null,
      'wbfsys_user_contact_visibility'
    );// wbfsys_user_contact_visibility  by alias wbfsys_user_contact_visibility
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_message_profile.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_message_profile.rowid = '.$condition );
        }
        else
        {
 ...
 /**
  ...
  public function checkConditions( $criteria, array $condition )
  {
    	...
    	if( isset($condition['ids']) && !empty( $condition['ids'] ) )
    	{
				$criteria->where
        (
          'wbfsys_message_profile.rowid = IN( '. implode( ', ', $condition['ids'] ) .' ) ';
        );
    	}
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_message_profile.rowid = \''.$part.'\'  or 
                wbfsys_message_profile.name = \''.$part.'\'  or 
                wbfsys_message_profile.user_name = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_message_profile.name) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_message_profile.user_name) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_message_profile
      if( isset( $condition['wbfsys_message_profile'] ) )
      {
        $whereCond = $condition['wbfsys_message_profile'];

        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_message_profile.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        if( isset($whereCond['id_type']) && count( $whereCond['id_type'] ) )
          $criteria->where( " wbfsys_message_profile.id_type IN( '".implode("','",$whereCond['id_type'])."' ) " );

        if( isset($whereCond['id_visibility']) && count( $whereCond['id_visibility'] ) )
          $criteria->where( " wbfsys_message_profile.id_visibility IN( '".implode("','",$whereCond['id_visibility'])."' ) " );

        if( isset( $whereCond['user_name'] ) && trim( $whereCond['user_name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_message_profile.user_name) like UPPER(\'%'.$whereCond['user_name'].'%\') ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_message_profile.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_message_profile.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_message_profile']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_message_profile.name ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_message_profile.name,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
     ...
     // inject the default order
      ...
      $criteria->orderBy( 'wbfsys_message_profile.name' );
      $criteria->selectAlso( 'wbfsys_message_profile.name as "wbfsys_message_profile-name-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_message_profile-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_message_profile-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_message_profile-name-order"' );
...
}//end class WbfsysMessageProfile_Table_Query_Postgresql
