 ...
 * @licence WebFrap.net
 */
class WbfsysImage_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysImage_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_image.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_image') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_image.rowid ",
      'acls'
    );
...
}//end class WbfsysImage_Selection_Access
