 ...
 * @licence WebFrap.net
 */
class WbfsysImage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysImage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysImage = $this->model->getEntityWbfsysImage();
    ...
    (
      'Show Image: {@text@}',
      'wbfsys.image.label',
      array( 'text' => $entityWbfsysImage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Image.show  &amp;objid='.$entityWbfsysImage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Image.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_image-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/image/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysImage_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_image-ref-comments' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_image-ref-tags' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Image.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_image' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_image a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Image.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Image.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysImage_Crud_Show_Maintab_View
