 ...
 * @licence WebFrap.net
 */
class WbfsysImageFormat_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysImageFormat_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_image_format.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_image_format') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_image_format.rowid ",
      'acls'
    );
...
}//end class WbfsysImageFormat_Selection_Access
