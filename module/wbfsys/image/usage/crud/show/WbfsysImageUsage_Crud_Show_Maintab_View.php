 ...
 * @licence WebFrap.net
 */
class WbfsysImageUsage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysImageUsage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysImageUsage = $this->model->getEntityWbfsysImageUsage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Image Usage: {@text@}',
      'wbfsys.image_usage.label',
      array( 'text' => $entityWbfsysImageUsage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ImageUsage.show  &amp;objid='.$entityWbfsysImageUsage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ImageUsage.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_image_usage-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/image_usage/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysImageUsage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ImageUsage.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_image_usage' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_image_usage a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ImageUsage.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ImageUsage.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysImageUsage_Crud_Show_Maintab_View
