 ...
 * @licence WebFrap.net
 */
class WbfsysEventStatus_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEventStatus Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEventStatus = $this->model->getEntityWbfsysEventStatus();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show event Status: {@text@}',
      'wbfsys.event_status.label',
      array( 'text' => $entityWbfsysEventStatus->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EventStatus.show  &amp;objid='.$entityWbfsysEventStatus
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EventStatus.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_event_status-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/event_status/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEventStatus_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EventStatus.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_event_status' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_event_status a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EventStatus.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EventStatus.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEventStatus_Crud_Show_Maintab_View
