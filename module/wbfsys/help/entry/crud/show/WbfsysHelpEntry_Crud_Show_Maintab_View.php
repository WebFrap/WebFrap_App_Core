 ...
 * @licence WebFrap.net
 */
class WbfsysHelpEntry_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysHelpEntry Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysHelpEntry = $this->model->getEntityWbfsysHelpEntry();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Help Entry: {@text@}',
      'wbfsys.help_entry.label',
      array( 'text' => $entityWbfsysHelpEntry->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.HelpEntry.show  &amp;objid='.$entityWbfsysHelpEntry
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.HelpEntry.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_help_entry-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/help_entry/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysHelpEntry_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.HelpEntry.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_help_entry' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_help_entry a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.HelpEntry.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.HelpEntry.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysHelpEntry_Crud_Show_Maintab_View
