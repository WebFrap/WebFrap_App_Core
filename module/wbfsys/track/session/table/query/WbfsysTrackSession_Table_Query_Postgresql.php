 ...
 * @licence WebFrap.net
 */
class WbfsysTrackSession_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_track_session_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_track_session.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_track_session.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_track_session_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_track_session.rowid as "wbfsys_track_session_rowid"', // variant: def-rowid 
      'wbfsys_track_session.service as "wbfsys_track_session_service"', // variant: def-by-context 
      'wbfsys_track_session.refer as "wbfsys_track_session_refer"', // variant: def-by-context 
      'wbfsys_track_session.session as "wbfsys_track_session_session"', // variant: def-by-context 
      'wbfsys_track_session.browser as "wbfsys_track_session_browser"', // variant: def-by-context 
      'wbfsys_track_session.os as "wbfsys_track_session_os"', // variant: def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_track_session' );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_track_session.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_track_session.rowid = '.$condition );
        }
        else
        {
 ...
 /**
  ...
  public function checkConditions( $criteria, array $condition )
  {
    	...
    	if( isset($condition['ids']) && !empty( $condition['ids'] ) )
    	{
				$criteria->where
        (
          'wbfsys_track_session.rowid = IN( '. implode( ', ', $condition['ids'] ) .' ) ';
        );
    	}
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_track_session.rowid = \''.$part.'\'  or 
                wbfsys_track_session.session = \''.$part.'\'  or 
                wbfsys_track_session.browser = \''.$part.'\'  or 
                wbfsys_track_session.os = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_track_session.session) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_track_session.browser) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_track_session.os) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_track_session
      if( isset( $condition['wbfsys_track_session'] ) )
      {
        $whereCond = $condition['wbfsys_track_session'];

        if( isset( $whereCond['service']) && trim( $whereCond['service'] ) != ''  )
          $criteria->where( ' wbfsys_track_session.service = \''.$whereCond['service'].'\' ');

        if( isset( $whereCond['refer']) && trim( $whereCond['refer'] ) != ''  )
          $criteria->where( ' wbfsys_track_session.refer = \''.$whereCond['refer'].'\' ');

        if( isset( $whereCond['session'] ) && trim( $whereCond['session'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_track_session.session) like UPPER(\'%'.$whereCond['session'].'%\') ');

        if( isset( $whereCond['browser'] ) && trim( $whereCond['browser'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_track_session.browser) like UPPER(\'%'.$whereCond['browser'].'%\') ');

        if( isset( $whereCond['os'] ) && trim( $whereCond['os'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_track_session.os) like UPPER(\'%'.$whereCond['os'].'%\') ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_track_session.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_track_session.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_track_session']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_track_session.id_session ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_track_session.id_session,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
     ...
     // inject the default order
      ...
      $criteria->orderBy( 'wbfsys_track_session.rowid' );
      $criteria->selectAlso( 'wbfsys_track_session.rowid as "wbfsys_track_session-rowid-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_track_session-rowid-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_track_session-rowid-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_track_session-rowid-order"' );
...
}//end class WbfsysTrackSession_Table_Query_Postgresql
