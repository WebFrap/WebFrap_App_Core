 ...
 * @licence WebFrap.net
 */
class WbfsysDomain_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysDomain Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysDomain = $this->model->getEntityWbfsysDomain();
    ...
    (
      'Show Domain: {@text@}',
      'wbfsys.domain.label',
      array( 'text' => $entityWbfsysDomain->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Domain.show  &amp;objid='.$entityWbfsysDomain
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Domain.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_domain-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/domain/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysDomain_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-subdomain' ) || $params->access->getPathLevel( 'mgmt-wbfsys_domain-ref-subdomain' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Subdomain
      if( Webfrap::loadable( 'HostingSubdomain_Entity' ) )
      {
        $this->addVar( 'showRefSubdomain', true );
       $this->addVar( 'showTabSubdomain', true );

      }
      else
      {
        Log::warn( 'The Model: HostingSubdomain is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Domain.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_domain' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_domain a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Domain.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Domain.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysDomain_Crud_Show_Maintab_View
