 ...
 * @licence WebFrap.net
 */
class WbfsysDomainAlias_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysDomainAlias Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysDomainAlias = $this->model->getEntityWbfsysDomainAlias();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Domain Alias: {@text@}',
      'wbfsys.domain_alias.label',
      array( 'text' => $entityWbfsysDomainAlias->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.DomainAlias.show  &amp;objid='.$entityWbfsysDomainAlias
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.DomainAlias.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_domain_alias-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/domain_alias/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysDomainAlias_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.DomainAlias.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_domain_alias' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_domain_alias a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.DomainAlias.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.DomainAlias.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysDomainAlias_Crud_Show_Maintab_View
