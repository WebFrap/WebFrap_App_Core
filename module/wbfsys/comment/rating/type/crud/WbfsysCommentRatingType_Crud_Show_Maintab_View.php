 ...
 * @licence WebFrap.net
 */
class WbfsysCommentRatingType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysCommentRatingType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysCommentRatingType = $this->model->getEntityWbfsysCommentRatingType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show comment rating Type: {@text@}',
      'wbfsys.comment_rating_type.label',
      array( 'text' => $entityWbfsysCommentRatingType->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'comment rating Type: {@text@}',
      'wbfsys.comment_rating_type.label',
      array( 'text' => $entityWbfsysCommentRatingType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.CommentRatingType.show  &amp;objid='.$entityWbfsysCommentRatingType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.CommentRatingType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_comment_rating_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/comment_rating_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysCommentRatingType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.CommentRatingType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_comment_rating_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_comment_rating_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.CommentRatingType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.CommentRatingType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysCommentRatingType_Crud_Show_Maintab_View
