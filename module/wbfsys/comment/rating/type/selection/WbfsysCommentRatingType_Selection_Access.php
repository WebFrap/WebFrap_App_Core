 ...
 * @licence WebFrap.net
 */
class WbfsysCommentRatingType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysCommentRatingType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_comment_rating_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_comment_rating_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_comment_rating_type.rowid ",
      'acls'
    );
...
}//end class WbfsysCommentRatingType_Selection_Access
