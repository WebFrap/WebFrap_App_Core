 ...
 * @licence WebFrap.net
 */
class WbfsysComment_Treetable_Postgresql_Query
  extends LibSqlQuery
{
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if($params->loadFullSize)
      $this->calcQuery = $criteria->count( 'count(wbfsys_comment.'.Db::PK.') as '.Db::Q_SIZE );
    ...
    while( $children = $this->loadChildren( $ids, true ) )
    {
      ...
      {
        $this->childs[$child['wbfsys_comment_m_parent']][] = $child;
        $ids[] = $child['wbfsys_comment_rowid'];
      }
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $access, $condition, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_comment.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_comment_rowid']];
      $this->data[]     = $row;
      $ids[] = $row['wbfsys_comment_rowid'];
    }
    ...
    while( $childIds = $access->fetchChildrenIds( $pName, 'treetable', $this, $ids, $condition, $params ) )
    {
      ...
      foreach( $children as $child )
      {
        $child['acl-level'] = $childIds[$child['wbfsys_comment_rowid']];
 ...
 /**
  ...
  public function loadChildren( $ids, $loadChildren = false )
  {
    ...
    if( $loadChildren )
      $criteria->where('wbfsys_comment.m_parent in('.implode(',',$ids).')');
    else
      $criteria->where('wbfsys_comment.rowid in('.implode(',',$ids).')');
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_comment.rowid as "wbfsys_comment_rowid"', // variant: def-rowid 
      'wbfsys_comment.title as "wbfsys_comment_title"', // variant: def-by-context 
      'wbfsys_comment.m_parent as "wbfsys_comment_m_parent"', // variant: def-additional 
    );
 ...
 /**
  ...
  public function setCalcQuery( $criteria, $params )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_comment.'.Db::PK.') as '.Db::Q_SIZE );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    $criteria->where( ' wbfsys_comment.m_parent is null ' );
    ...
    if( $this->condition )
    {
      ...
      if( is_string($this->condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_comment.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_comment.rowid = '.$condition );
        }
        else
        {
 ...
 wbfsys_comment.rowid = \''.$part.'\' 
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_comment.m_parent ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_comment.m_parent,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_comment-title-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_comment-title-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_comment-title-order"' );
...
}//end class WbfsysComment_Treetable_Postgresql_Query
