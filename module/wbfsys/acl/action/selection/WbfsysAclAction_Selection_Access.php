 ...
 * @licence WebFrap.net
 */
class WbfsysAclAction_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysAclAction_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_acl_action.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_acl_action') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_acl_action.rowid ",
      'acls'
    );
...
}//end class WbfsysAclAction_Selection_Access
