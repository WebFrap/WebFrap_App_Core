 ...
 * @licence WebFrap.net
 */
class WbfsysAudioCodec_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysAudioCodec_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_audio_codec.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_audio_codec') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_audio_codec.rowid ",
      'acls'
    );
...
}//end class WbfsysAudioCodec_Selection_Access
