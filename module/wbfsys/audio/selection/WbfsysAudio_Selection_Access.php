 ...
 * @licence WebFrap.net
 */
class WbfsysAudio_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysAudio_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_audio.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_audio') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_audio.rowid ",
      'acls'
    );
...
}//end class WbfsysAudio_Selection_Access
