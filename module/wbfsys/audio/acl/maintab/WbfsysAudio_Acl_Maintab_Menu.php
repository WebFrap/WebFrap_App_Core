 ...
 * @licence WebFrap.net
 */
class WbfsysAudio_Acl_Maintab_Menu
  extends WgtDropmenu
{
...
{$entries->support}
  ...
  <div 
  	class="wgt-panel-control {$this->view->id}-wbfsys_audio-acl box-qfd_users"
  	style="display:none;"
  	 >
    <div 
      ...
      <ul>
        <li><a 
          class="wcm wcm_req_ajax" 
            href="modal.php?c=Webfrap.Docu.show&amp;key=wbfsys_audio-acl" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a>
        </li>
        <!--
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Issue.create&amp;refer=wbfsys_audio-acl" >{$iconBug} {$this->view->i18n->l('Bug','wbf.label')}</a>
        </li>
        -->
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Faq.create&amp;refer=wbfsys_audio-acl" >{$iconFaq} {$this->view->i18n->l('Faq','wbf.label')}</a>
        </li>
      </ul>
    </span>
  </li>
...
HTML;
   ...
   *
   * @param WbfsysAudio_Acl_Maintab $view
   * @param int $areaId
   * @param TArray $params
   */
  public function injectMenuLogic( $view, $areaId, $params  )
  {
    ...
    self.getObject().find(".wgtac_mask_entity_rights").click(function(){
      ...
      self.close( );
      \$R.get( 'maintab.php?c=Mgmt.Acl.listing&dkey=wbfsys_audio' );
    });
    
    self.getObject().find(".wgtac_masks_overview").click(function(){
      \$R.get( 'modal.php?c=Mgmt.Acl.listAllMasks&dkey=wbfsys_audio' );
      \$S('#{$this->id}-control').dropdown('close');
    });
    

    self.getObject().find('#wgt-button-wbfsys_audio-acl-form-append').click(function(){
    
      if(\$S('#wgt-input-wbfsys_audio-acl-id_group').val()==''){
        \$D.errorWindow('Error','Please select a group first');
        return false;
      }

      \$R.form('wgt-form-wbfsys_audio-acl-append');
      \$S('#wgt-form-wbfsys_audio-acl-append').get(0).reset();
      return false;
...
} // end class WbfsysAudio_Acl_Maintab_Menu */
