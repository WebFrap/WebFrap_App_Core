 ...
 * @licence WebFrap.net
 */
class WbfsysAudio_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_audio_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_audio.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_audio.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_audio_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function fetchWithAcls( $condition = null, $params = null )
  {
    ...
    {
      $acl->injectListingAcls( $criteria, 'mod-wbfsys>mgmt-wbfsys_audio' );
    }
    else
    {
      $acl->injectListingAcls( $criteria, 'mod-wbfsys>mgmt-wbfsys_audio', true, $params->access->level  );
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_audio.rowid as "wbfsys_audio_rowid"', // variant: def-rowid 
      'wbfsys_audio.title as "wbfsys_audio_title"', // variant: def-by-context 
      'wbfsys_audio_codec.name as "wbfsys_audio_codec_name"', // variant: def-by-context  used source field wbfsys_audio_codec
      'wbfsys_audio.id_codec as "wbfsys_audio_id_codec"', // ref wbfsys_audio def-by-context 
      'wbfsys_audio.description as "wbfsys_audio_description"', // variant: def-by-context 
    );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_audio.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_audio.rowid = '.$condition );
        }
        else
        {
 ...
 wbfsys_audio.rowid = \''.$part.'\' 
      ...
      // search conditions for  wbfsys_audio
      if( isset( $condition['wbfsys_audio'] ) )
      {
        $whereCond = $condition['wbfsys_audio'];

        if( isset( $whereCond['title']) && trim( $whereCond['title'] ) != ''  )
          $criteria->where( ' wbfsys_audio.title = \''.$whereCond['title'].'\' ');

        if( isset($whereCond['id_codec']) && count( $whereCond['id_codec'] ) )
          $criteria->where( " wbfsys_audio.id_codec IN( '".implode("','",$whereCond['id_codec'])."' ) " );

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_audio.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_audio.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_audio.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_audio.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_audio.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_audio.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_audio.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_audio.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_audio']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_audio.title ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_audio.title,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_audio-title-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_audio-title-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_audio-title-order"' );
...
}//end class WbfsysAudio_Table_Query_Postgresql
