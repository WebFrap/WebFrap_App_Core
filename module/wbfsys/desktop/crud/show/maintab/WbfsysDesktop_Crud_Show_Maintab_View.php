 ...
 * @licence WebFrap.net
 */
class WbfsysDesktop_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysDesktop Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysDesktop = $this->model->getEntityWbfsysDesktop();
    ...
    (
      ...
      array( 'text' => $entityWbfsysDesktop->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Desktop.show  &amp;objid='.$entityWbfsysDesktop
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Desktop.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_desktop-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/desktop/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysDesktop_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-main_menu' ) || $params->access->getPathLevel( 'mgmt-wbfsys_desktop-ref-main_menu' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: MainMenu
      if( Webfrap::loadable( 'WbfsysMenuEntry_Entity' ) )
      {
        $this->addVar( 'showRefMainMenu', true );
       $this->addVar( 'showTabMainMenu', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysMenuEntry is not loadable' );
      }
    }

    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-main_tree' ) || $params->access->getPathLevel( 'mgmt-wbfsys_desktop-ref-main_tree' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: MainTree
      if( Webfrap::loadable( 'WbfsysMenuEntry_Entity' ) )
      {
        $this->addVar( 'showRefMainTree', true );
       $this->addVar( 'showTabMainTree', true );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Desktop.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_desktop' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_desktop a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Desktop.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Desktop.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysDesktop_Crud_Show_Maintab_View
