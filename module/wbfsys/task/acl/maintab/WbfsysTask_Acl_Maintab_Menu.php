 ...
 * @licence WebFrap.net
 */
class WbfsysTask_Acl_Maintab_Menu
  extends WgtDropmenu
{
...
{$entries->support}
  ...
  <div 
  	class="wgt-panel-control {$this->view->id}-wbfsys_task-acl box-qfd_users"
  	style="display:none;"
  	 >
    <div 
      ...
      <ul>
        <li><a 
          class="wcm wcm_req_ajax" 
            href="modal.php?c=Webfrap.Docu.show&amp;key=wbfsys_task-acl" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a>
        </li>
        <!--
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Issue.create&amp;refer=wbfsys_task-acl" >{$iconBug} {$this->view->i18n->l('Bug','wbf.label')}</a>
        </li>
        -->
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Faq.create&amp;refer=wbfsys_task-acl" >{$iconFaq} {$this->view->i18n->l('Faq','wbf.label')}</a>
        </li>
      </ul>
    </span>
  </li>
...
HTML;
   ...
   *
   * @param WbfsysTask_Acl_Maintab $view
   * @param int $areaId
   * @param TArray $params
   */
  public function injectMenuLogic( $view, $areaId, $params  )
  {
    ...
    self.getObject().find(".wgtac_mask_entity_rights").click(function(){
      ...
      self.close( );
      \$R.get( 'maintab.php?c=Mgmt.Acl.listing&dkey=wbfsys_task' );
    });
    
    self.getObject().find(".wgtac_masks_overview").click(function(){
      \$R.get( 'modal.php?c=Mgmt.Acl.listAllMasks&dkey=wbfsys_task' );
      \$S('#{$this->id}-control').dropdown('close');
    });
    

    self.getObject().find('#wgt-button-wbfsys_task-acl-form-append').click(function(){
    
      if(\$S('#wgt-input-wbfsys_task-acl-id_group').val()==''){
        \$D.errorWindow('Error','Please select a group first');
        return false;
      }

      \$R.form('wgt-form-wbfsys_task-acl-append');
      \$S('#wgt-form-wbfsys_task-acl-append').get(0).reset();
      return false;
    ...
    self.getObject().find(".wgtac_mask_my_wbfsys_task").click(function(){
      \$S('#{$this->id}-control').dropdown('remove');
      self.close( );
      \$R.get( 'maintab.php?c=Acl.Mgmt.listing&dkey=my_wbfsys_task' );
    });
...
HTML;
      ...
      if( $acl->access( 'mod-wbfsys/mgmt-wbfsys_task>mgmt-my_wbfsys_task:admin' ) )
      {
...
} // end class WbfsysTask_Acl_Maintab_Menu */
