 ...
 * @licence WebFrap.net
 */
class WbfsysApp_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysApp Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysApp = $this->model->getEntityWbfsysApp();
    ...
    (
      'Show App: {@text@}',
      'wbfsys.app.label',
      array( 'text' => $entityWbfsysApp->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.App.show  &amp;objid='.$entityWbfsysApp
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.App.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_app-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/app/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysApp_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-app_modules' ) || $params->access->getPathLevel( 'mgmt-wbfsys_app-ref-app_modules' ) )
    {
      // many to many reference
      // if the model is loadable append the tab box for ref: AppModules
      if( Webfrap::loadable( 'WbfsysAppModules_Entity' ) )
      {
        $this->addVar( 'showRefAppModules', true );
       $this->addVar( 'showTabAppModules', true );

      }
      else
      {
        Log::warn( 'The Model for: WbfsysAppModules is not loadable' );
      }
    }

    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-desktops' ) || $params->access->getPathLevel( 'mgmt-wbfsys_app-ref-desktops' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Desktops
      if( Webfrap::loadable( 'WbfsysDesktop_Entity' ) )
      {
        $this->addVar( 'showRefDesktops', true );
       $this->addVar( 'showTabDesktops', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysDesktop is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.App.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_app' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_app a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.App.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.App.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysApp_Crud_Show_Maintab_View
