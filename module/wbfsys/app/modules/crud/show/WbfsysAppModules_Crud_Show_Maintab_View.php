 ...
 * @licence WebFrap.net
 */
class WbfsysAppModules_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysAppModules Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysAppModules = $this->model->getEntityWbfsysAppModules();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show App Modules: {@text@}',
      'wbfsys.app_modules.label',
      array( 'text' => $entityWbfsysAppModules->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.AppModules.show  &amp;objid='.$entityWbfsysAppModules
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.AppModules.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_app_modules-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/app_modules/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysAppModules_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.AppModules.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_app_modules' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_app_modules a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.AppModules.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.AppModules.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysAppModules_Crud_Show_Maintab_View
