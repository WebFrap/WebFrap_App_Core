 ...
 * @licence WebFrap.net
 */
class WbfsysEntityRole_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityRole Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityRole = $this->model->getEntityWbfsysEntityRole();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity Role: {@text@}',
      'wbfsys.entity_role.label',
      array( 'text' => $entityWbfsysEntityRole->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityRole.show  &amp;objid='.$entityWbfsysEntityRole
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityRole.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_role-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_role/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityRole_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-role_actions' ) || $params->access->getPathLevel( 'mgmt-wbfsys_entity_role-ref-role_actions' ) )
    {
      // many to many reference
      // if the model is loadable append the tab box for ref: RoleActions
      if( Webfrap::loadable( 'WbfsysEntityRoleActions_Entity' ) )
      {
        $this->addVar( 'showRefRoleActions', true );
       $this->addVar( 'showTabRoleActions', true );

      }
      else
      {
        Log::warn( 'The Model for: WbfsysEntityRoleActions is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityRole.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_role' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_role a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityRole.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityRole.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityRole_Crud_Show_Maintab_View
