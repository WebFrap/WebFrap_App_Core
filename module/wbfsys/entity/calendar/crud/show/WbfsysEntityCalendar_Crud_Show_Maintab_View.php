 ...
 * @licence WebFrap.net
 */
class WbfsysEntityCalendar_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityCalendar Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityCalendar = $this->model->getEntityWbfsysEntityCalendar();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity Calendar: {@text@}',
      'wbfsys.entity_calendar.label',
      array( 'text' => $entityWbfsysEntityCalendar->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Entity Calendar: {@text@}',
      'wbfsys.entity_calendar.label',
      array( 'text' => $entityWbfsysEntityCalendar->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityCalendar.show  &amp;objid='.$entityWbfsysEntityCalendar
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityCalendar.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_calendar-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_calendar/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityCalendar_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityCalendar.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_calendar' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_calendar a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityCalendar.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityCalendar.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityCalendar_Crud_Show_Maintab_View
