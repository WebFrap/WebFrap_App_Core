 ...
 * @licence WebFrap.net
 */
class WbfsysEntityAttachment_Table_Element
  extends WgtTable
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
   ...
   * @var string $id
   */
  public $id   = 'wgt_table-wbfsys_entity_attachment';
  
  /**
   * the most likley class of a given query object
   *
   * @var WbfsysEntityAttachment_Table_Query
   */
  public $data = null;
  ...
  public function loadUrl()
  {
    ...
    (
      ...
      (
        ...
        'Show',
        'maintab.php?c=Wbfsys.EntityAttachment.show&amp;target_mask=WbfsysEntityAttachment&amp;ltype=table&amp;objid=',
        'control/show.png',
        '',
      ...
      (
        Wgt::ACTION_BUTTON_GET,
        'Edit',
        'maintab.php?c=Wbfsys.EntityAttachment.edit&amp;target_mask=WbfsysEntityAttachment&amp;ltype=table&amp;objid=',
        'control/edit.png',
        '',
        'wbfsys.entity_attachment.label',
        Acl::UPDATE
      ...
      (
        Wgt::ACTION_DELETE,
        'Delete',
        'index.php?c=Wbfsys.EntityAttachment.delete&amp;target_mask=WbfsysEntityAttachment&amp;ltype=table&amp;objid=',
        'control/delete.png',
        '',
        'wbfsys.entity_attachment.label',
        Acl::DELETE
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function buildThead( )
  {
    ...
    $html .= '<th style="width:200px" >'.$this->view->i18n->l( 'Name', 'wbfsys.file.label' ).'</th>'.NL;
    $html .= '<th style="width:200px" >'.$this->view->i18n->l( 'Link', 'wbfsys.file.label' ).'</th>'.NL;
    $html .= '<th style="width:200px" >'.$this->view->i18n->l( 'Type', 'wbfsys.file.label' ).'</th>'.NL;
    $html .= '<th style="width:200px" >'.$this->view->i18n->l( 'Versioning', 'wbfsys.file.label' ).'</th>'.NL;
    $html .= '<th style="width:200px" >'.$this->view->i18n->l( 'Description', 'wbfsys.file.label' ).'</th>'.NL;
  ...
  public function buildTbody( )
  {
    ...
    foreach( $this->data as $key => $row )
    {
      ...
      $objid       = $row['wbfsys_entity_attachment_rowid'];
      ...
      if( $row['entity_file_name'] )
      {
        $body .= '<td valign="top" ><a href="file.php?f=entity_file-name-'.$row['entity_file_rowid'].'&amp;n='.base64_encode($row['entity_file_name']).'" target="new_download" >'.Validator::sanitizeHtml($row['entity_file_name']).'</a></td>'.NL;
      }
      else
      {
      ...
      $body .= '<td valign="top" style="text-align:center;" >'.( trim( $row['entity_file_link'] ) == '' ? ' ' : '<a href="'.Wgt::renderUrl($row['entity_file_link']).'">'.Validator::sanitizeHtml($row['entity_file_link']).'</a>' ).'</td>'.NL;

      $body .= '<td valign="top" ><a class="wcm wcm_req_ajax" href="maintab.php?c=Wbfsys.FileType.listing" >'.nl2br(Validator::sanitizeHtml($row['entity_file_name'])).'</a></td>'.NL;

      $body .= '<td valign="top" style="text-align:center;"  >'.WgtRndForm::checkbox( 'entity_file[flag_versioning]', $row['entity_file_flag_versioning'] , array() , true ).'</td>'.NL;

      $body .= '<td valign="top" >'.Validator::sanitizeHtml($row['entity_file_description']).'</td>'.NL;
  ...
  <div class="wgt-dropdownbox" id="{$this->id}-list-action-menu" >
    ...
    <ul>
      <li><a
      	...
      	href="ajax.php?c=Wbfsys.EntityAttachment_Multi.deleteSelection"
      	wgt_elem="table#{$this->id}-table"
      	title="Please confirm that you want to delete the selected Entity Attachments." >{$iconDelete} Delete selected Entity Attachments</a></li>
      <li><a 
      	class="wcm wcm_req_del"
      	title="You are going to delete ALL! Entity Attachments. Please confirm that you really want to do that."
      	href="ajax.php?c=Wbfsys.EntityAttachment_Multi.deleteAll" >{$iconClean} Delete all Entity Attachments</a></li>
  	</ul>
 	</div>
...
}//end class WbfsysEntityAttachment_Table_Element
