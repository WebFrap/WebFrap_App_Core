 ...
 * @licence WebFrap.net
 */
class WbfsysEntityAttachment_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityAttachment Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityAttachment = $this->model->getEntityWbfsysEntityAttachment();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity Attachment: {@text@}',
      'wbfsys.entity_attachment.label',
      array( 'text' => $entityWbfsysEntityAttachment->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Entity Attachment: {@text@}',
      'wbfsys.entity_attachment.label',
      array( 'text' => $entityWbfsysEntityAttachment->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityAttachment.show  &amp;objid='.$entityWbfsysEntityAttachment
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityAttachment.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_attachment-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_attachment/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityAttachment_Crud_Show' );
    $this->addVar( 'crudForm', $form );
    
    $entity = $this->model->getEntity();
    $form->setEntity( $entity );
  $form->setEntityEntityFile( $this->model->getEntityEntityFile( $entity ) );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityAttachment.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_attachment' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_attachment a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityAttachment.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityAttachment.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityAttachment_Crud_Show_Maintab_View
