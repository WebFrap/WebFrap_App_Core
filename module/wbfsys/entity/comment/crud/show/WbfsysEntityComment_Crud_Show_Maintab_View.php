 ...
 * @licence WebFrap.net
 */
class WbfsysEntityComment_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityComment Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityComment = $this->model->getEntityWbfsysEntityComment();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity Comment: {@text@}',
      'wbfsys.entity_comment.label',
      array( 'text' => $entityWbfsysEntityComment->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityComment.show  &amp;objid='.$entityWbfsysEntityComment
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityComment.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_comment-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_comment/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityComment_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityComment.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_comment' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_comment a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityComment.listing' );
      }
    });
    

    self.getObject().find(".wgtac_mask_wbfsys_entity_comment-simple").click(function(){
      \$S('#{$this->id}_dropmenu-control').dropdown('remove');
      self.close();
      \$R.get('maintab.php?c=Wbfsys.EntityComment_Simple.show&objid={$objid}');
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityComment.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityComment_Crud_Show_Maintab_View
