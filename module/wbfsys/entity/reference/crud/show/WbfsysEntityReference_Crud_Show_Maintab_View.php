 ...
 * @licence WebFrap.net
 */
class WbfsysEntityReference_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityReference Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityReference = $this->model->getEntityWbfsysEntityReference();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity Reference: {@text@}',
      'wbfsys.entity_reference.label',
      array( 'text' => $entityWbfsysEntityReference->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityReference.show  &amp;objid='.$entityWbfsysEntityReference
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityReference.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_reference-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_reference/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityReference_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityReference.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_reference' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_reference a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityReference.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityReference.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityReference_Crud_Show_Maintab_View
