 ...
 * @licence WebFrap.net
 */
class WbfsysEntityAttributeType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysEntityAttributeType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_entity_attribute_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_entity_attribute_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_entity_attribute_type.rowid ",
      'acls'
    );
...
}//end class WbfsysEntityAttributeType_Selection_Access
