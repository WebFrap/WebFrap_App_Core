 ...
 * @licence WebFrap.net
 */
class WbfsysEntityAttribute_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityAttribute Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityAttribute = $this->model->getEntityWbfsysEntityAttribute();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity Attribute: {@text@}',
      'wbfsys.entity_attribute.label',
      array( 'text' => $entityWbfsysEntityAttribute->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityAttribute.show  &amp;objid='.$entityWbfsysEntityAttribute
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityAttribute.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_attribute-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_attribute/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityAttribute_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-comments' ) || $params->access->getPathLevel( 'mgmt-wbfsys_entity_attribute-ref-comments' ) )
    {
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityAttribute.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_attribute' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_attribute a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityAttribute.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityAttribute.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityAttribute_Crud_Show_Maintab_View
