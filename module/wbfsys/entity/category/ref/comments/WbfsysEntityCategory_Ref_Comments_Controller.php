 ...
 * If you want to extend this class write your coden in ControllerWbfsysEntityCategory
 *
 ...
 * @licence WebFrap.net
 */
class WbfsysEntityCategory_Ref_Comments_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des comments Referenztabs
  * für den wbfsys_entity_category management knoten
  *
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $containerClass = "WbfsysEntityCategory_Ref_Comments_".$listType."_Access";
    ...
    $accessComments = new $containerClass( );
    $accessComments->load( $user->getProfileName(), $params );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-comments';

    $params->accessComments = $accessComments;
    $params->paramsComments = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessComments->listing  )
    {
    ...
    if( !$params->saveFormId )
      $params->saveFormId = 'wgt-form-wbfsys_entity_category-edit-'.$objid;
    ...
    (
      $params->tabId,
      'WbfsysEntityCategory_Ref_Comments_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelComments = $this->loadModel
    (
      'WbfsysEntityCategory_Ref_Comments_'.$listType
    );
    $modelComments->setAccess( $accessComments );
    
    $view->setModel( $modelComments );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysEntityCategory_Ref_Comments_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_entity_category-ref-comments-'.$listType.'-ajax',
    	'WbfsysEntityCategory_Ref_Comments_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysEntityComment by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {

    $this->request->removePost( 'wbfsys_comment' ); //def wbfsys_comment
    $this->request->removePost( 'wbfsys_entity_comment' ); //def wbfsys_entity_comment
    $this->request->removePost( 'wbfsys_comment' );


  } // end public function cleanPost */

 /**
  * connect existining WbfsysComment to a WbfsysEntityCategory
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mod-wbfsys-cat-core_data-ref-comments'
    );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-comments';

    $params->accessComments = $access;
    $params->paramsComments = $params;

    $model  = $this->loadModel( 'WbfsysEntityCategory_Ref_Comments_Crud' );
    ...
    $entityWbfsysComment = $orm->get( 'WbfsysComment', $refId );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysEntityCategory_Ref_Comments_'.$listType );
      $listModel = $this->loadModel( 'WbfsysEntityCategory_Ref_Comments_'.$listType );
      $listModel->setAccess( $access );
      $ui->setModel( $listModel );

      if( !$ui->listEntry
      ( 
        $conEntiy->getData( 'id_comment' ), 
        $access, 
        $params, 
        true 
      ))
...
} // end WbfsysEntityCategory_Ref_Comments_Controller
