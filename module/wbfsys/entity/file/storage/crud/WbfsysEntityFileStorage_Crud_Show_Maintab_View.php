 ...
 * @licence WebFrap.net
 */
class WbfsysEntityFileStorage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysEntityFileStorage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysEntityFileStorage = $this->model->getEntityWbfsysEntityFileStorage();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Entity File Storage: {@text@}',
      'wbfsys.entity_file_storage.label',
      array( 'text' => $entityWbfsysEntityFileStorage->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Entity File Storage: {@text@}',
      'wbfsys.entity_file_storage.label',
      array( 'text' => $entityWbfsysEntityFileStorage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.EntityFileStorage.show  &amp;objid='.$entityWbfsysEntityFileStorage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.EntityFileStorage.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_entity_file_storage-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/entity_file_storage/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysEntityFileStorage_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.EntityFileStorage.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_file_storage' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_entity_file_storage a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.EntityFileStorage.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.EntityFileStorage.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysEntityFileStorage_Crud_Show_Maintab_View
