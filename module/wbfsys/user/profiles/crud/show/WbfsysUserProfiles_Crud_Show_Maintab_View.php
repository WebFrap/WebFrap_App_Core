 ...
 * @licence WebFrap.net
 */
class WbfsysUserProfiles_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysUserProfiles Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysUserProfiles = $this->model->getEntityWbfsysUserProfiles();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show User Profiles: {@text@}',
      'wbfsys.user_profiles.label',
      array( 'text' => $entityWbfsysUserProfiles->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.UserProfiles.show  &amp;objid='.$entityWbfsysUserProfiles
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.UserProfiles.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_user_profiles-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/user_profiles/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysUserProfiles_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.UserProfiles.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_profiles' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_profiles a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.UserProfiles.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.UserProfiles.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysUserProfiles_Crud_Show_Maintab_View
