 ...
 * @licence WebFrap.net
 */
class WbfsysUserContactVisibility_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysUserContactVisibility Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysUserContactVisibility = $this->model->getEntityWbfsysUserContactVisibility();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show message profile visibility: {@text@}',
      'wbfsys.user_contact_visibility.label',
      array( 'text' => $entityWbfsysUserContactVisibility->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'message profile visibility: {@text@}',
      'wbfsys.user_contact_visibility.label',
      array( 'text' => $entityWbfsysUserContactVisibility->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.UserContactVisibility.show  &amp;objid='.$entityWbfsysUserContactVisibility
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.UserContactVisibility.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_user_contact_visibility-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/user_contact_visibility/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysUserContactVisibility_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.UserContactVisibility.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_contact_visibility' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_contact_visibility a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.UserContactVisibility.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.UserContactVisibility.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysUserContactVisibility_Crud_Show_Maintab_View
