 ...
 * @licence WebFrap.net
 */
class WbfsysUserRelationStatus_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysUserRelationStatus Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysUserRelationStatus = $this->model->getEntityWbfsysUserRelationStatus();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show user relation Status: {@text@}',
      'wbfsys.user_relation_status.label',
      array( 'text' => $entityWbfsysUserRelationStatus->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'user relation Status: {@text@}',
      'wbfsys.user_relation_status.label',
      array( 'text' => $entityWbfsysUserRelationStatus->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.UserRelationStatus.show  &amp;objid='.$entityWbfsysUserRelationStatus
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.UserRelationStatus.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_user_relation_status-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/user_relation_status/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysUserRelationStatus_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.UserRelationStatus.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_relation_status' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_relation_status a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.UserRelationStatus.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.UserRelationStatus.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysUserRelationStatus_Crud_Show_Maintab_View
