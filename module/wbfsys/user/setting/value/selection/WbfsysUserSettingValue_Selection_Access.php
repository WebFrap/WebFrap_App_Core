 ...
 * @licence WebFrap.net
 */
class WbfsysUserSettingValue_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysUserSettingValue_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_user_setting_value.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_user_setting_value') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_user_setting_value.rowid ",
      'acls'
    );
...
}//end class WbfsysUserSettingValue_Selection_Access
