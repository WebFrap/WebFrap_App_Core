 ...
 * @licence WebFrap.net
 */
class WbfsysUserSetting_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysUserSetting_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_user_setting.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_user_setting') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_user_setting.rowid ",
      'acls'
    );
...
}//end class WbfsysUserSetting_Selection_Access
