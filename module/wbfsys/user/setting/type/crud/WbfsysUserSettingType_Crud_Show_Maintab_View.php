 ...
 * @licence WebFrap.net
 */
class WbfsysUserSettingType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysUserSettingType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysUserSettingType = $this->model->getEntityWbfsysUserSettingType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show user setting value type: {@text@}',
      'wbfsys.user_setting_type.label',
      array( 'text' => $entityWbfsysUserSettingType->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'user setting value type: {@text@}',
      'wbfsys.user_setting_type.label',
      array( 'text' => $entityWbfsysUserSettingType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.UserSettingType.show  &amp;objid='.$entityWbfsysUserSettingType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.UserSettingType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_user_setting_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/user_setting_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysUserSettingType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.UserSettingType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_setting_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_setting_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.UserSettingType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.UserSettingType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysUserSettingType_Crud_Show_Maintab_View
