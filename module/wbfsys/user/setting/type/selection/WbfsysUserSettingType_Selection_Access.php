 ...
 * @licence WebFrap.net
 */
class WbfsysUserSettingType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysUserSettingType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_user_setting_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_user_setting_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_user_setting_type.rowid ",
      'acls'
    );
...
}//end class WbfsysUserSettingType_Selection_Access
