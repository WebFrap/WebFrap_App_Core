 ...
 * @licence WebFrap.net
 */
class WbfsysUserAnnouncementStatus_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysUserAnnouncementStatus Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysUserAnnouncementStatus = $this->model->getEntityWbfsysUserAnnouncementStatus();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show user announcement Status: {@text@}',
      'wbfsys.user_announcement_status.label',
      array( 'text' => $entityWbfsysUserAnnouncementStatus->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'user announcement Status: {@text@}',
      'wbfsys.user_announcement_status.label',
      array( 'text' => $entityWbfsysUserAnnouncementStatus->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.UserAnnouncementStatus.show  &amp;objid='.$entityWbfsysUserAnnouncementStatus
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.UserAnnouncementStatus.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_user_announcement_status-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/user_announcement_status/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysUserAnnouncementStatus_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.UserAnnouncementStatus.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_announcement_status' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_user_announcement_status a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.UserAnnouncementStatus.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.UserAnnouncementStatus.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysUserAnnouncementStatus_Crud_Show_Maintab_View
