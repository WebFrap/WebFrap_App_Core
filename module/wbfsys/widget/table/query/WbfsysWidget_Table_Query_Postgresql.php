 ...
 * @licence WebFrap.net
 */
class WbfsysWidget_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_widget_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_widget.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_widget.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_widget_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_widget.rowid as "wbfsys_widget_rowid"', // variant: def-rowid 
      'wbfsys_widget.name as "wbfsys_widget_name"', // variant: def-by-context 
      'wbfsys_widget.access_key as "wbfsys_widget_access_key"', // variant: def-by-context 
      'wbfsys_widget.cname as "wbfsys_widget_cname"', // variant: def-by-context 
      'wbfsys_widget.id_entity as "wbfsys_widget_id_entity"', // variant: def-by-context 
      'wbfsys_widget.id_management as "wbfsys_widget_id_management"', // variant: def-by-context 
      'wbfsys_widget.description as "wbfsys_widget_description"', // variant: def-by-context 
    );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_widget.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_widget.rowid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_widget.rowid = \''.$part.'\'  or 
                wbfsys_widget.name = \''.$part.'\'  or 
                wbfsys_widget.access_key = \''.$part.'\'  or 
                wbfsys_widget.cname = \''.$part.'\'  or 
                wbfsys_widget.description = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_widget.name) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_widget.access_key) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_widget.cname) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_widget.description) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_widget
      if( isset( $condition['wbfsys_widget'] ) )
      {
        $whereCond = $condition['wbfsys_widget'];

        if( isset( $whereCond['name']) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' wbfsys_widget.name = \''.$whereCond['name'].'\' ');

        if( isset( $whereCond['access_key']) && trim( $whereCond['access_key'] ) != ''  )
          $criteria->where( ' wbfsys_widget.access_key = \''.$whereCond['access_key'].'\' ');

        if( isset( $whereCond['cname']) && trim( $whereCond['cname'] ) != ''  )
          $criteria->where( ' wbfsys_widget.cname = \''.$whereCond['cname'].'\' ');

        if( isset( $whereCond['id_entity']) && trim( $whereCond['id_entity'] ) != ''  )
          $criteria->where( ' wbfsys_widget.id_entity = \''.$whereCond['id_entity'].'\' ');

        if( isset( $whereCond['id_management']) && trim( $whereCond['id_management'] ) != ''  )
          $criteria->where( ' wbfsys_widget.id_management = \''.$whereCond['id_management'].'\' ');

        if( isset( $whereCond['description']) && trim( $whereCond['description'] ) != ''  )
          $criteria->where( ' wbfsys_widget.description = \''.$whereCond['description'].'\' ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_widget.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_widget.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_widget.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_widget.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_widget.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_widget.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_widget.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_widget.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_widget']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_widget.name ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_widget.name,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_widget-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_widget-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_widget-name-order"' );
...
}//end class WbfsysWidget_Table_Query_Postgresql
