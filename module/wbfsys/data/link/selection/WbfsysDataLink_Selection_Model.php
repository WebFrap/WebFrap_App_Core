 ...
 * @licence WebFrap.net
 */
class WbfsysDataLink_Selection_Model
  extends Model
{
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function search( $access, $params, $condition = array() )
  {
    ...
    if( $free = $httpRequest->param( 'free_search' , Validator::TEXT ) )
      ...
      if( $mRoleCreate = $httpRequest->param( 'search_wbfsys_data_link', Validator::EID, 'm_role_create'   ) )
        $condition['wbfsys_data_link']['m_role_create'] = $mRoleCreate;

      if( $mRoleChange = $httpRequest->param( 'search_wbfsys_data_link', Validator::EID, 'm_role_change'   ) )
        $condition['wbfsys_data_link']['m_role_change'] = $mRoleChange;

      if( $mTimeCreatedBefore = $httpRequest->param( 'search_wbfsys_data_link', Validator::DATE, 'm_time_created_before'   ) )
        $condition['wbfsys_data_link']['m_time_created_before'] = $mTimeCreatedBefore;

      if( $mTimeCreatedAfter = $httpRequest->param( 'search_wbfsys_data_link', Validator::DATE, 'm_time_created_after'   ) )
        $condition['wbfsys_data_link']['m_time_created_after'] = $mTimeCreatedAfter;

      if( $mTimeChangedBefore = $httpRequest->param( 'search_wbfsys_data_link', Validator::DATE, 'm_time_changed_before'   ) )
        $condition['wbfsys_data_link']['m_time_changed_before'] = $mTimeChangedBefore;

      if( $mTimeChangedAfter = $httpRequest->param( 'search_wbfsys_data_link}', Validator::DATE, 'm_time_changed_after'   ) )
        $condition['wbfsys_data_link']['m_time_changed_after'] = $mTimeChangedAfter;

      if( $mRowid = $httpRequest->param( 'search_wbfsys_data_link', Validator::EID, 'm_rowid'   ) )
        $condition['wbfsys_data_link']['m_rowid'] = $mRowid;

      if( $mUuid = $httpRequest->param( 'search_wbfsys_data_link', Validator::TEXT, 'm_uuid'    ) )
        $condition['wbfsys_data_link']['m_uuid'] = $mUuid;
    ...
    $query = $db->newQuery( 'WbfsysDataLink_Selection' );
		...
		if( $params->dynFilters )
    {
      foreach( $params->dynFilters as $dynFilter  )
      {
        try
        {
          $filter = $db->newFilter
          (
            'WbfsysDataLink_Selection_'.SParserString::subToCamelCase( $dynFilter )
          );
    ...
    if( $params->exclude )
    {
      ...
      $excludeCond = ' wbfsys_data_link.rowid NOT IN '
  ...
  public function searchForm( $view )
  {
    ...
    //entity wbfsys_data_link
    if( !$entityWbfsysDataLink = $this->getRegisterd( 'entityWbfsysDataLink' ) )
    {
      $entityWbfsysDataLink   = new WbfsysDataLink_Entity() ;
    }
...
}//end class WbfsysDataLink_Selection_Model
