 ...
 * @licence WebFrap.net
 */
class WbfsysDocuTree_Treetable_Postgresql_Query
  extends LibSqlQuery
{
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if($params->loadFullSize)
      $this->calcQuery = $criteria->count( 'count(wbfsys_docu_tree.'.Db::PK.') as '.Db::Q_SIZE );
    ...
    {
      $this->data[] = $row;
      $ids[] = $row['wbfsys_docu_tree_rowid'];
    }
    ...
    while( $children = $this->loadChildren( $ids, true ) )
    {
      ...
      {
        $this->childs[$child['wbfsys_docu_tree_m_parent']][] = $child;
        $ids[] = $child['wbfsys_docu_tree_rowid'];
      }
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $access, $condition, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_docu_tree.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_docu_tree_rowid']];
      $this->data[]     = $row;
      $ids[] = $row['wbfsys_docu_tree_rowid'];
    }
    ...
    while( $childIds = $access->fetchChildrenIds( $pName, 'treetable', $this, $ids, $condition, $params ) )
    {
      ...
      foreach( $children as $child )
      {
        $child['acl-level'] = $childIds[$child['wbfsys_docu_tree_rowid']];
 ...
 /**
  ...
  public function loadChildren( $ids, $loadChildren = false )
  {
    ...
    if( $loadChildren )
      $criteria->where('wbfsys_docu_tree.m_parent in('.implode(',',$ids).')');
    else
      $criteria->where('wbfsys_docu_tree.rowid in('.implode(',',$ids).')');
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_docu_tree.rowid as "wbfsys_docu_tree_rowid"', // variant: def-rowid 
      'wbfsys_docu_tree.title as "wbfsys_docu_tree_title"', // variant: def-by-context 
      'wbfsys_docu_tree.short_desc as "wbfsys_docu_tree_short_desc"', // variant: def-by-context 
      'wbfsys_docu_tree.m_parent as "wbfsys_docu_tree_m_parent"', // variant: def-additional 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_docu_tree' );
 ...
 /**
  ...
  public function setCalcQuery( $criteria, $params )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_docu_tree.'.Db::PK.') as '.Db::Q_SIZE );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    $criteria->where( ' wbfsys_docu_tree.m_parent is null ' );
    ...
    if( $this->condition )
    {
      ...
      if( is_string($this->condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_docu_tree.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_docu_tree.rowid = '.$condition );
        }
        else
        {
 ...
 wbfsys_docu_tree.rowid = \''.$part.'\' 
      ...
      // search conditions for  wbfsys_docu_tree
      if( isset( $condition['wbfsys_docu_tree'] ) )
      {
        $whereCond = $condition['wbfsys_docu_tree'];

        if( isset( $whereCond['title']) && trim( $whereCond['title'] ) != ''  )
          $criteria->where( ' wbfsys_docu_tree.title = \''.$whereCond['title'].'\' ');

        if( isset( $whereCond['short_desc']) && trim( $whereCond['short_desc'] ) != ''  )
          $criteria->where( ' wbfsys_docu_tree.short_desc = \''.$whereCond['short_desc'].'\' ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_docu_tree.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_docu_tree']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_docu_tree.m_parent ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_docu_tree.m_parent,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_docu_tree-title-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_docu_tree-title-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_docu_tree-title-order"' );
...
}//end class WbfsysDocuTree_Treetable_Postgresql_Query
