 ...
 * @licence WebFrap.net
 */
class WbfsysProcessNode_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcessNode Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcessNode = $this->model->getEntityWbfsysProcessNode();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Process Node: {@text@}',
      'wbfsys.process_node.label',
      array( 'text' => $entityWbfsysProcessNode->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProcessNode.show  &amp;objid='.$entityWbfsysProcessNode
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProcessNode.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process_node-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process_node/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcessNode_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProcessNode.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_node' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_node a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProcessNode.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProcessNode.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcessNode_Crud_Show_Maintab_View
