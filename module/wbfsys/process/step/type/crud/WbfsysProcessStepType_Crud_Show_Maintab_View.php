 ...
 * @licence WebFrap.net
 */
class WbfsysProcessStepType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcessStepType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcessStepType = $this->model->getEntityWbfsysProcessStepType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show process step Type: {@text@}',
      'wbfsys.process_step_type.label',
      array( 'text' => $entityWbfsysProcessStepType->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'process step Type: {@text@}',
      'wbfsys.process_step_type.label',
      array( 'text' => $entityWbfsysProcessStepType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProcessStepType.show  &amp;objid='.$entityWbfsysProcessStepType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProcessStepType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process_step_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process_step_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcessStepType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProcessStepType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_step_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_step_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProcessStepType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProcessStepType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcessStepType_Crud_Show_Maintab_View
