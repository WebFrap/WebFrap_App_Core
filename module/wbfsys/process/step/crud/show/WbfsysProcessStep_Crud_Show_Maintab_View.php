 ...
 * @licence WebFrap.net
 */
class WbfsysProcessStep_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcessStep Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcessStep = $this->model->getEntityWbfsysProcessStep();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Process Step: {@text@}',
      'wbfsys.process_step.label',
      array( 'text' => $entityWbfsysProcessStep->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProcessStep.show  &amp;objid='.$entityWbfsysProcessStep
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProcessStep.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process_step-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process_step/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcessStep_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProcessStep.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_step' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_step a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProcessStep.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProcessStep.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcessStep_Crud_Show_Maintab_View
