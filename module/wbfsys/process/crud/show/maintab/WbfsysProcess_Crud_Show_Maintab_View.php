 ...
 * @licence WebFrap.net
 */
class WbfsysProcess_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcess Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcess = $this->model->getEntityWbfsysProcess();
    ...
    (
      ...
      array( 'text' => $entityWbfsysProcess->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Process.show  &amp;objid='.$entityWbfsysProcess
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Process.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcess_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-nodes' ) || $params->access->getPathLevel( 'mgmt-wbfsys_process-ref-nodes' ) )
    {
      // many to one reference
      // if the model is loadable append the tab box for ref: Nodes
      if( Webfrap::loadable( 'WbfsysProcessNode_Entity' ) )
      {
        $this->addVar( 'showRefNodes', true );
       $this->addVar( 'showTabNodes', true );

      }
      else
      {
        Log::warn( 'The Model: WbfsysProcessNode is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Process.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Process.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Process.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcess_Crud_Show_Maintab_View
