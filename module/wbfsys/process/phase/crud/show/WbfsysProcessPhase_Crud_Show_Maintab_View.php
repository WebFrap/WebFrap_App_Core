 ...
 * @licence WebFrap.net
 */
class WbfsysProcessPhase_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcessPhase Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcessPhase = $this->model->getEntityWbfsysProcessPhase();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Process Phase: {@text@}',
      'wbfsys.process_phase.label',
      array( 'text' => $entityWbfsysProcessPhase->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProcessPhase.show  &amp;objid='.$entityWbfsysProcessPhase
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProcessPhase.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process_phase-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process_phase/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcessPhase_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProcessPhase.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_phase' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_phase a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProcessPhase.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProcessPhase.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcessPhase_Crud_Show_Maintab_View
