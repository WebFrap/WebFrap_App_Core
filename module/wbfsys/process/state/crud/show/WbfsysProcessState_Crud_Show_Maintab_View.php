 ...
 * @licence WebFrap.net
 */
class WbfsysProcessState_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcessState Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcessState = $this->model->getEntityWbfsysProcessState();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Process State: {@text@}',
      'wbfsys.process_state.label',
      array( 'text' => $entityWbfsysProcessState->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProcessState.show  &amp;objid='.$entityWbfsysProcessState
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProcessState.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process_state-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process_state/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcessState_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProcessState.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_state' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_state a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProcessState.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProcessState.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcessState_Crud_Show_Maintab_View
