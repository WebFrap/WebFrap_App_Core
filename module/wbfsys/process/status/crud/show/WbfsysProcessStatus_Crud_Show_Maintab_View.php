 ...
 * @licence WebFrap.net
 */
class WbfsysProcessStatus_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysProcessStatus Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysProcessStatus = $this->model->getEntityWbfsysProcessStatus();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Process Status: {@text@}',
      'wbfsys.process_status.label',
      array( 'text' => $entityWbfsysProcessStatus->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ProcessStatus.show  &amp;objid='.$entityWbfsysProcessStatus
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ProcessStatus.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_process_status-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/process_status/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysProcessStatus_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ProcessStatus.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_status' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_process_status a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ProcessStatus.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ProcessStatus.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysProcessStatus_Crud_Show_Maintab_View
