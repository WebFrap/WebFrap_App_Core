 ...
 * @licence WebFrap.net
 */
class WbfsysRoleUserMaskEmployee_Crud_Show_Maintab_Menu
  extends WgtDropmenu
{
...
BUTTON;
    ...
    if( $params->access->admin )
    {
      ...
      $entries->entryAcls = <<<HTML
        ...
        <li>
          <a class="deeplink" >{$iconRights} {$view->i18n->l( 'Rights', 'wbf.label' )}</a>
          <span>
            <ul>
              <li>
                <a 
                  href="maintab.php?c=Acl.Mgmt_Dset.listing&amp;dkey=wbfsys_role_user_mask_employee&amp;objid={$objid}" 
                  class="wcm wcm_req_ajax" >{$iconMgmt} {$view->i18n->l( 'ACLs Management', 'wbf.label' )}</a>
              </li>
              <li>
                <a 
                  href="maintab.php?c=Acl.Mgmt_Dset.listing&amp;dkey=wbfsys_role_user&amp;objid={$objid}" 
      ...
      <li>
        ...
        <ul>
          ...
          <li><a 
            ...
            href="modal.php?c=Webfrap.Docu.open&amp;key=wbfsys_role_user_mask_employee-show" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a></li>
...
HTML;
      ...
      if( $this->acl->access( 'mod-wbfsys>mod-wbfsys-cat-core_data:access', $objid ) )
      {
        $html .= '<li><a class="wgtac_mask_my_user_profile" >'.$iconMask.' '.$this->view->i18n->l('My Profile','my.user_profile.label').'</a></li>'.NL;
        ++ $numEntries;
      }

      if( $this->acl->access( 'mod-wbfsys>mod-wbfsys-cat-core_data:access', $objid ) )
      {
        $html .= '<li><a class="wgtac_mask_wbfsys_role_user-viewer" >'.$iconMask.' '.$this->view->i18n->l('Employee','wbfsys.role_user_viewer.label').'</a></li>'.NL;
        ++ $numEntries;
      }

      if( $this->acl->access( 'mod-wbfsys>mod-wbfsys-cat-core_data:access', $objid ) )
      {
        $html .= '<li><a class="wgtac_mask_wbfsys_role_user" >'.$iconMask.' '.$this->view->i18n->l('System User','wbfsys.role_user.label').'</a></li>'.NL;
        ++ $numEntries;
      }
    ...
    $html = <<<HTML
      ...
      <li>
        ...
        <ul>
          ...
          <li>
            <a class="wcm wcm_req_ajax" href="modal.php?c=Maintenance.Entity.protocolDataset&dkey=wbfsys_role_user&amp;objid={$objid}" >{$iconProtocol} {$this->view->i18n->l( 'Protocol', 'wbf.label' )}</a>
          </li>


          <li>
            <a class="wcm wcm_req_ajax" href="modal.php?c=Maintenance.Entity.statsDataset&dkey=wbfsys_role_user&amp;objid={$objid}" >{$iconStats} {$this->view->i18n->l( 'Stats', 'wbf.label' )}</a>
          </li>


          <li>
            <a class="wcm wcm_req_ajax" href="modal.php?c=Maintenance.Entity.showMeta&dkey=wbfsys_role_user&amp;objid={$objid}" >{$iconMetaData} {$this->view->i18n->l( 'Meta Data', 'wbf.label' )}</a>
          </li>
...
}//end class WbfsysRoleUserMaskEmployee_Crud_Show_Maintab_Menu
