 ...
 * @licence WebFrap.net
 */
class WbfsysRoleUserMaskEmployee_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysRoleUserMaskEmployee Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysRoleUserMaskEmployee = $this->model->getEntityWbfsysRoleUserMaskEmployee();
    ...
    (
      ...
      array( 'text' => $entityWbfsysRoleUserMaskEmployee->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.RoleUserMaskEmployee.show  &amp;objid='.$entityWbfsysRoleUserMaskEmployee
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.RoleUserMaskEmployee.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_role_user_mask_employee-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/role_user_mask_employee/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysRoleUserMaskEmployee_Crud_Show' );
  ...
  $form->setEntityEmbedPerson( $this->model->getEntityEmbedPerson( $entity ) );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-user_roles' ) || $params->access->getPathLevel( 'mgmt-wbfsys_role_user_mask_employee-ref-user_roles' ) )
    {
      ...
      if( Webfrap::loadable( 'WbfsysGroupUsers_Entity' ) )
      {
       ...
       $this->addVar( 'showTabGroupsNProfiles', true );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-user_profiles' ) || $params->access->getPathLevel( 'mgmt-wbfsys_role_user_mask_employee-ref-user_profiles' ) )
    {
      // many to many reference
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.RoleUserMaskEmployee.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_role_user_mask_employee' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_role_user_mask_employee a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.RoleUserMaskEmployee.listing' );
      }
    });
    ...
    self.getObject().find(".wgtac_mask_wbfsys_role_user-viewer").click(function(){
      \$S('#{$this->id}_dropmenu-control').dropdown('remove');
      self.close();
      \$R.get('maintab.php?c=Wbfsys.RoleUser_Viewer.show&objid={$objid}');
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.RoleUserMaskEmployee.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysRoleUserMaskEmployee_Crud_Show_Maintab_View
