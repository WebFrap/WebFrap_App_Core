 ...
 * @licence WebFrap.net
 */
class WbfsysRoleUserMaskEmployee_Ref_UserProfiles_Table_Access
  extends LibAclPermissionList
{
  /**
   * @param TFlag $params
   * @param Entity: wbfsys_user_profiles Attributes: id_user; id_profile; rowid; m_time_created; m_role_create; m_time_changed; m_role_change; m_version; m_uuid_Entity $entity
   */
  public function loadDefault( $params, $entity = null )
  {
    ...
    try 
    {
      ...
      $rootPerm = $rootContainer->getRefAccess( $params->aclRootId, $params->aclLevel, 'mod-wbfsys-cat-core_data-ref-user_profiles' );
  ...
  /**
   * @param WbfsysRoleUserMaskEmployee_Ref_UserProfiles_Table_Query $query
  ...
  public function fetchListTableDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_user_profiles.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_user_profiles') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_user_profiles.rowid ",
      'acls'
    );
...
}//end class WbfsysRoleUserMaskEmployee_Ref_UserProfiles_Table_Access
