 ...
 * @licence WebFrap.net
 */
class WbfsysRoleGroupType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysRoleGroupType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_role_group_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_role_group_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_role_group_type.rowid ",
      'acls'
    );
...
}//end class WbfsysRoleGroupType_Selection_Access
