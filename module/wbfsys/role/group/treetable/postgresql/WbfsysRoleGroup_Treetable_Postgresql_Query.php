 ...
 * @licence WebFrap.net
 */
class WbfsysRoleGroup_Treetable_Postgresql_Query
  extends LibSqlQuery
{
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if($params->loadFullSize)
      $this->calcQuery = $criteria->count( 'count(wbfsys_role_group.'.Db::PK.') as '.Db::Q_SIZE );
    ...
    {
      $this->data[] = $row;
      $ids[] = $row['wbfsys_role_group_rowid'];
    }
    ...
    while( $children = $this->loadChildren( $ids, true ) )
    {
      ...
      {
        $this->childs[$child['wbfsys_role_group_m_parent']][] = $child;
        $ids[] = $child['wbfsys_role_group_rowid'];
      }
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $access, $condition, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_role_group.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_role_group_rowid']];
      $this->data[]     = $row;
      $ids[] = $row['wbfsys_role_group_rowid'];
    }
    ...
    while( $childIds = $access->fetchChildrenIds( $pName, 'treetable', $this, $ids, $condition, $params ) )
    {
      ...
      foreach( $children as $child )
      {
        $child['acl-level'] = $childIds[$child['wbfsys_role_group_rowid']];
 ...
 /**
  ...
  public function loadChildren( $ids, $loadChildren = false )
  {
    ...
    if( $loadChildren )
      $criteria->where('wbfsys_role_group.m_parent in('.implode(',',$ids).')');
    else
      $criteria->where('wbfsys_role_group.rowid in('.implode(',',$ids).')');
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_role_group.rowid as "wbfsys_role_group_rowid"', // variant: def-rowid 
      'wbfsys_role_group.name as "wbfsys_role_group_name"', // variant: def-by-context 
      'wbfsys_role_group.access_key as "wbfsys_role_group_access_key"', // variant: def-by-context 
      'wbfsys_role_mandant.name as "wbfsys_role_mandant_name"', // variant: def-by-context  used source field wbfsys_role_mandant
      'wbfsys_role_group.id_mandant as "wbfsys_role_group_id_mandant"', // ref wbfsys_role_group def-by-context 
      'wbfsys_role_group_type.name as "wbfsys_role_group_type_name"', // variant: def-by-context  used source field wbfsys_role_group_type
      'wbfsys_role_group.id_type as "wbfsys_role_group_id_type"', // ref wbfsys_role_group def-by-context 
      'wbfsys_role_group.description as "wbfsys_role_group_description"', // variant: def-by-context 
      'wbfsys_role_group.m_parent as "wbfsys_role_group_m_parent"', // variant: def-additional 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_role_group' );
    ...
    (
      ...
      null,
      'wbfsys_role_group_type'
    );// wbfsys_role_group_type  by alias wbfsys_role_group_type
 ...
 /**
  ...
  public function setCalcQuery( $criteria, $params )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_role_group.'.Db::PK.') as '.Db::Q_SIZE );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    $criteria->where( ' wbfsys_role_group.m_parent is null ' );
    ...
    if( $this->condition )
    {
      ...
      if( is_string($this->condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_role_group.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_role_group.rowid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_role_group.rowid = \''.$part.'\'  or 
                wbfsys_role_group.name = \''.$part.'\' 
              )'
            );
         }
        else
      ...
      // search conditions for  wbfsys_role_group
      if( isset( $condition['wbfsys_role_group'] ) )
      {
        $whereCond = $condition['wbfsys_role_group'];

        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_role_group.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        if( isset( $whereCond['access_key']) && trim( $whereCond['access_key'] ) != ''  )
          $criteria->where( ' wbfsys_role_group.access_key = \''.$whereCond['access_key'].'\' ');

        if( isset($whereCond['id_type']) && count( $whereCond['id_type'] ) )
          $criteria->where( " wbfsys_role_group.id_type IN( '".implode("','",$whereCond['id_type'])."' ) " );

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_role_group.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_role_group.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_role_group']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_role_group.m_parent ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_role_group.m_parent,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_role_group-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_role_group-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_role_group-name-order"' );
...
}//end class WbfsysRoleGroup_Treetable_Postgresql_Query
