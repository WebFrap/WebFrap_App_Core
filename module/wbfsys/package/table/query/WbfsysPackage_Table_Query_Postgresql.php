 ...
 * @licence WebFrap.net
 */
class WbfsysPackage_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_package_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_package.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_package.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_package_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_package.rowid as "wbfsys_package_rowid"', // variant: def-rowid 
      'wbfsys_package.name as "wbfsys_package_name"', // variant: def-by-context 
      'wbfsys_package_type.name as "wbfsys_package_type_name"', // variant: def-by-context  used source field wbfsys_package_type
      'wbfsys_package.id_type as "wbfsys_package_id_type"', // ref wbfsys_package def-by-context 
      'wbfsys_package_status.name as "wbfsys_package_status_name"', // variant: def-by-context  used source field wbfsys_package_status
      'wbfsys_package.id_status as "wbfsys_package_id_status"', // ref wbfsys_package def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    (
      ...
      null,
      'wbfsys_package_type'
    );// wbfsys_package_type  by alias wbfsys_package_type
    ...
    (
      ...
      null,
      'wbfsys_package_status'
    );// wbfsys_package_status  by alias wbfsys_package_status
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_package.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_package.rowid = '.$condition );
        }
        else
        {
 ...
 /**
  ...
  public function checkConditions( $criteria, array $condition )
  {
    	...
    	if( isset($condition['ids']) && !empty( $condition['ids'] ) )
    	{
				$criteria->where
        (
          'wbfsys_package.rowid = IN( '. implode( ', ', $condition['ids'] ) .' ) ';
        );
    	}
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_package.rowid = \''.$part.'\'  or 
                wbfsys_package.name = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_package.name) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_package
      if( isset( $condition['wbfsys_package'] ) )
      {
        $whereCond = $condition['wbfsys_package'];

        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_package.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        if( isset($whereCond['id_type']) && count( $whereCond['id_type'] ) )
          $criteria->where( " wbfsys_package.id_type IN( '".implode("','",$whereCond['id_type'])."' ) " );

        if( isset($whereCond['id_status']) && count( $whereCond['id_status'] ) )
          $criteria->where( " wbfsys_package.id_status IN( '".implode("','",$whereCond['id_status'])."' ) " );

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_package.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_package.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_package.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_package.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_package.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_package.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_package.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_package.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_package']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_package.name ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_package.name,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
     ...
     // inject the default order
      ...
      $criteria->orderBy( 'wbfsys_package.name' );
      $criteria->selectAlso( 'wbfsys_package.name as "wbfsys_package-name-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_package-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_package-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_package-name-order"' );
...
}//end class WbfsysPackage_Table_Query_Postgresql
