 ...
 * @licence WebFrap.net
 */
class WbfsysPackageStatus_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysPackageStatus Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysPackageStatus = $this->model->getEntityWbfsysPackageStatus();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show package Status: {@text@}',
      'wbfsys.package_status.label',
      array( 'text' => $entityWbfsysPackageStatus->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.PackageStatus.show  &amp;objid='.$entityWbfsysPackageStatus
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.PackageStatus.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_package_status-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/package_status/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysPackageStatus_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.PackageStatus.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_package_status' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_package_status a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.PackageStatus.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.PackageStatus.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysPackageStatus_Crud_Show_Maintab_View
