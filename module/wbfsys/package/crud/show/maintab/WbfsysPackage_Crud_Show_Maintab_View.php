 ...
 * @licence WebFrap.net
 */
class WbfsysPackage_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysPackage Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysPackage = $this->model->getEntityWbfsysPackage();
    ...
    (
      ...
      array( 'text' => $entityWbfsysPackage->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Package.show  &amp;objid='.$entityWbfsysPackage
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Package.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_package-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/package/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysPackage_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mod-wbfsys-cat-core_data-ref-package_modules' ) || $params->access->getPathLevel( 'mgmt-wbfsys_package-ref-package_modules' ) )
    {
      // many to many reference
      // if the model is loadable append the tab box for ref: PackageModules
      if( Webfrap::loadable( 'WbfsysPackageModules_Entity' ) )
      {
        $this->addVar( 'showRefPackageModules', true );
       $this->addVar( 'showTabPackageModules', true );

      }
      else
      {
        Log::warn( 'The Model for: WbfsysPackageModules is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Package.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_package' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_package a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Package.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Package.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysPackage_Crud_Show_Maintab_View
