 ...
 * @licence WebFrap.net
 */
class WbfsysPackageType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysPackageType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysPackageType = $this->model->getEntityWbfsysPackageType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show package Type: {@text@}',
      'wbfsys.package_type.label',
      array( 'text' => $entityWbfsysPackageType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.PackageType.show  &amp;objid='.$entityWbfsysPackageType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.PackageType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_package_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/package_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysPackageType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.PackageType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_package_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_package_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.PackageType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.PackageType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysPackageType_Crud_Show_Maintab_View
