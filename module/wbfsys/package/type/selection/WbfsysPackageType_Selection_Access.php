 ...
 * @licence WebFrap.net
 */
class WbfsysPackageType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysPackageType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_package_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_package_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_package_type.rowid ",
      'acls'
    );
...
}//end class WbfsysPackageType_Selection_Access
