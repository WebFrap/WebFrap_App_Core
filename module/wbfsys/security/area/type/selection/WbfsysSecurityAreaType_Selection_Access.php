 ...
 * @licence WebFrap.net
 */
class WbfsysSecurityAreaType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysSecurityAreaType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_security_area_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_security_area_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_security_area_type.rowid ",
      'acls'
    );
...
}//end class WbfsysSecurityAreaType_Selection_Access
