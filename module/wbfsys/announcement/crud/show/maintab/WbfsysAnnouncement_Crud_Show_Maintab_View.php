 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncement_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysAnnouncement Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysAnnouncement = $this->model->getEntityWbfsysAnnouncement();
    ...
    (
      ...
      array( 'text' => $entityWbfsysAnnouncement->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.Announcement.show  &amp;objid='.$entityWbfsysAnnouncement
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.Announcement.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_announcement-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/announcement/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysAnnouncement_Crud_Show' );
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_announcement-ref-attachments' ) )
    {
    ...
    if( $params->access->getPathLevel( 'mgmt-wbfsys_announcement-ref-status' ) )
    {
      // many to many reference
      // if the model is loadable append the tab box for ref: Status
      if( Webfrap::loadable( 'WbfsysUserAnnouncement_Entity' ) )
      {
        $this->addVar( 'showRefStatus', true );
       $this->addVar( 'showTabStatus', true );

      }
      else
      {
        Log::warn( 'The Model for: WbfsysUserAnnouncement is not loadable' );
      }
    }
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.Announcement.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_announcement' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_announcement a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.Announcement.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.Announcement.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysAnnouncement_Crud_Show_Maintab_View
