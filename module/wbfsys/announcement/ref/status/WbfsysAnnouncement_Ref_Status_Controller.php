 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncement_Ref_Status_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des status Referenztabs
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $containerClass = "WbfsysAnnouncement_Ref_Status_".$listType."_Access";
    ...
    $accessStatus = new $containerClass( );
    $accessStatus->load( $user->getProfileName(), $params );

    $params->aclNode = 'mgmt-wbfsys_announcement-ref-status';

    $params->accessStatus = $accessStatus;
    $params->paramsStatus = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessStatus->listing  )
    {
    ...
    (
      $params->tabId,
      'WbfsysAnnouncement_Ref_Status_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelStatus = $this->loadModel
    (
      'WbfsysAnnouncement_Ref_Status_'.$listType
    );
    $modelStatus->setAccess( $accessStatus );
    
    $view->setModel( $modelStatus );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysAnnouncement_Ref_Status_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_announcement-ref-status-'.$listType.'-ajax',
    	'WbfsysAnnouncement_Ref_Status_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysUserAnnouncement by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {
    ...
    $this->request->removePost( 'wbfsys_user_announcement' ); //def wbfsys_user_announcement


  } // end public function cleanPost */

 /**
  * connect existining WbfsysRoleUser to a WbfsysAnnouncement
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mgmt-wbfsys_announcement-ref-status'
    );

    $params->aclNode = 'mgmt-wbfsys_announcement-ref-status';

    $params->accessStatus = $access;
    $params->paramsStatus = $params;

    $model  = $this->loadModel( 'WbfsysAnnouncement_Ref_Status_Crud' );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysAnnouncement_Ref_Status_'.$listType );
      $listModel = $this->loadModel( 'WbfsysAnnouncement_Ref_Status_'.$listType );
...
} // end WbfsysAnnouncement_Ref_Status_Controller
