 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncement_Ref_Status_Table_Access
  extends LibAclPermissionList
{
  /**
   * @param TFlag $params
   * @param Entity: wbfsys_user_announcement Attributes: id_announcement; id_user; id_status; rowid; m_time_created; m_role_create; m_time_changed; m_role_change; m_version; m_uuid_Entity $entity
   */
  public function loadDefault( $params, $entity = null )
  {
    ...
    try 
    {
      ...
      $rootPerm = $rootContainer->getRefAccess( $params->aclRootId, $params->aclLevel, 'mgmt-wbfsys_announcement-ref-status' );
  ...
  /**
   * @param WbfsysAnnouncement_Ref_Status_Table_Query $query
  ...
  public function fetchListTableDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_user_announcement.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_user_announcement') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_user_announcement.rowid ",
      'acls'
    );
...
}//end class WbfsysAnnouncement_Ref_Status_Table_Access
