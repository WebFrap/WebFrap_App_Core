 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncementType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysAnnouncementType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysAnnouncementType = $this->model->getEntityWbfsysAnnouncementType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show announcement Type: {@text@}',
      'wbfsys.announcement_type.label',
      array( 'text' => $entityWbfsysAnnouncementType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.AnnouncementType.show  &amp;objid='.$entityWbfsysAnnouncementType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.AnnouncementType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_announcement_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/announcement_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysAnnouncementType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.AnnouncementType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_announcement_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_announcement_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.AnnouncementType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.AnnouncementType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysAnnouncementType_Crud_Show_Maintab_View
