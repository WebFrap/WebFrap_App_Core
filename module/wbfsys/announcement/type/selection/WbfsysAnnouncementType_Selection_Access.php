 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncementType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysAnnouncementType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_announcement_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_announcement_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_announcement_type.rowid ",
      'acls'
    );
...
}//end class WbfsysAnnouncementType_Selection_Access
