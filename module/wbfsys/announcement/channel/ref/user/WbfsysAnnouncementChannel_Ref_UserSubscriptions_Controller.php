 ...
 * If you want to extend this class write your coden in ControllerWbfsysAnnouncementChannel
 *
 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncementChannel_Ref_UserSubscriptions_Controller
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Service zum laden des user_subscriptions Referenztabs
  * für den wbfsys_announcement_channel management knoten
  *
  ...
  */
  public function service_tab( $request, $response )
  {
    ...
    $params->refId = $objid;
    $params->refIdUserSubscriptions = $objid;
    ...
    $containerClass = "WbfsysAnnouncementChannel_Ref_UserSubscriptions_".$listType."_Access";
    ...
    $accessUserSubscriptions = new $containerClass( );
    $accessUserSubscriptions->load( $user->getProfileName(), $params );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-user_subscriptions';

    $params->accessUserSubscriptions = $accessUserSubscriptions;
    $params->paramsUserSubscriptions = $params;
    
    // ok wenn er nichtmal lesen darf, dann ist hier direkt schluss
    if( !$accessUserSubscriptions->listing  )
    {
    ...
    if( !$params->saveFormId )
      $params->saveFormId = 'wgt-form-wbfsys_announcement_channel-edit-'.$objid;
    ...
    (
      $params->tabId,
      'WbfsysAnnouncementChannel_Ref_UserSubscriptions_'.$listType,
      'displayTab',
      View::AREA
    );
    ...
    $modelUserSubscriptions = $this->loadModel
    (
      'WbfsysAnnouncementChannel_Ref_UserSubscriptions_'.$listType
    );
    $modelUserSubscriptions->setAccess( $accessUserSubscriptions );
    
    $view->setModel( $modelUserSubscriptions );
  ...
  */
  ...
  public function service_search( $request, $response )
  {
    ...
    $model   = $this->loadModel( 'WbfsysAnnouncementChannel_Ref_UserSubscriptions_'.$listType );

    $view    = $response->loadView
    ( 
    	'wbfsys_announcement_channel-ref-user_subscriptions-'.$listType.'-ajax',
    	'WbfsysAnnouncementChannel_Ref_UserSubscriptions_'.$listType,
    	'displaySearch',
    	View::AJAX
    );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  /**
   * delete a WbfsysAnnouncementChannelSubscription by an ajax request
   * @return boolean
   */
  public function cleanPost( )
  {

    $this->request->removePost( 'wbfsys_role_user' ); //def wbfsys_role_user
    $this->request->removePost( 'core_person' );
    $this->request->removePost( 'wbfsys_announcement_channel_subscription' ); //def wbfsys_announcement_channel_subscription


  } // end public function cleanPost */

 /**
  * connect existining WbfsysRoleUser to a WbfsysAnnouncementChannel
  *
  * @param TFlag $params named parameters
  */
  public function service_connect( $request, $response )
  {
    ...
    (
      ...
      $params->aclKey,
      $params->refId,
      'mod-wbfsys-cat-core_data-ref-user_subscriptions'
    );

    $params->aclNode = 'mod-wbfsys-cat-core_data-ref-user_subscriptions';

    $params->accessUserSubscriptions = $access;
    $params->paramsUserSubscriptions = $params;

    $model  = $this->loadModel( 'WbfsysAnnouncementChannel_Ref_UserSubscriptions_Crud' );
    ...
    $entityWbfsysRoleUser = $orm->get( 'WbfsysRoleUser', $refId );
    ...
    if( $model->insert( $params ) )
    {
      ...
      $ui = $this->loadUi( 'WbfsysAnnouncementChannel_Ref_UserSubscriptions_'.$listType );
      $listModel = $this->loadModel( 'WbfsysAnnouncementChannel_Ref_UserSubscriptions_'.$listType );
      $listModel->setAccess( $access );
      $ui->setModel( $listModel );

      if( !$ui->listEntry
      ( 
        $conEntiy->getData( 'id_role' ), 
        $access, 
        $params, 
        true 
      ))
...
} // end WbfsysAnnouncementChannel_Ref_UserSubscriptions_Controller
