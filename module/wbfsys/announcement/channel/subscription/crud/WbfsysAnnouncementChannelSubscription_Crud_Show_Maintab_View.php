 ...
 * @licence WebFrap.net
 */
class WbfsysAnnouncementChannelSubscription_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysAnnouncementChannelSubscription Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysAnnouncementChannelSubscription = $this->model->getEntityWbfsysAnnouncementChannelSubscription();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Announcement Subscription: {@text@}',
      'wbfsys.announcement_channel_subscription.label',
      array( 'text' => $entityWbfsysAnnouncementChannelSubscription->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Announcement Subscription: {@text@}',
      'wbfsys.announcement_channel_subscription.label',
      array( 'text' => $entityWbfsysAnnouncementChannelSubscription->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.AnnouncementChannelSubscription.show  &amp;objid='.$entityWbfsysAnnouncementChannelSubscription
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.AnnouncementChannelSubscription.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_announcement_channel_subscription-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/announcement_channel_subscription/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysAnnouncementChannelSubscription_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.AnnouncementChannelSubscription.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_announcement_channel_subscription' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_announcement_channel_subscription a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.AnnouncementChannelSubscription.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.AnnouncementChannelSubscription.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysAnnouncementChannelSubscription_Crud_Show_Maintab_View
