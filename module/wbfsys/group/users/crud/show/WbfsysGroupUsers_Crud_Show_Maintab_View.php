 ...
 * @licence WebFrap.net
 */
class WbfsysGroupUsers_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysGroupUsers Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysGroupUsers = $this->model->getEntityWbfsysGroupUsers();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Group Users: {@text@}',
      'wbfsys.group_users.label',
      array( 'text' => $entityWbfsysGroupUsers->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.GroupUsers.show  &amp;objid='.$entityWbfsysGroupUsers
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.GroupUsers.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_group_users-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/group_users/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysGroupUsers_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.GroupUsers.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_group_users' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_group_users a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.GroupUsers.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.GroupUsers.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysGroupUsers_Crud_Show_Maintab_View
