 ...
 * @licence WebFrap.net
 */
class WbfsysTagReference_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysTagReference Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysTagReference = $this->model->getEntityWbfsysTagReference();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Tag Reference: {@text@}',
      'wbfsys.tag_reference.label',
      array( 'text' => $entityWbfsysTagReference->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.TagReference.show  &amp;objid='.$entityWbfsysTagReference
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.TagReference.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_tag_reference-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/tag_reference/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysTagReference_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.TagReference.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_tag_reference' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_tag_reference a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.TagReference.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.TagReference.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysTagReference_Crud_Show_Maintab_View
