 ...
 * @licence WebFrap.net
 */
class WbfsysTreeNode_Treetable_Postgresql_Query
  extends LibSqlQuery
{
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if($params->loadFullSize)
      $this->calcQuery = $criteria->count( 'count(wbfsys_tree_node.'.Db::PK.') as '.Db::Q_SIZE );
    ...
    {
      $this->data[] = $row;
      $ids[] = $row['wbfsys_tree_node_rowid'];
    }
    ...
    while( $children = $this->loadChildren( $ids, true ) )
    {
      ...
      {
        $this->childs[$child['wbfsys_tree_node_m_parent']][] = $child;
        $ids[] = $child['wbfsys_tree_node_rowid'];
      }
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $access, $condition, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_tree_node.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_tree_node_rowid']];
      $this->data[]     = $row;
      $ids[] = $row['wbfsys_tree_node_rowid'];
    }
    ...
    while( $childIds = $access->fetchChildrenIds( $pName, 'treetable', $this, $ids, $condition, $params ) )
    {
      ...
      foreach( $children as $child )
      {
        $child['acl-level'] = $childIds[$child['wbfsys_tree_node_rowid']];
 ...
 /**
  ...
  public function loadChildren( $ids, $loadChildren = false )
  {
    ...
    if( $loadChildren )
      $criteria->where('wbfsys_tree_node.m_parent in('.implode(',',$ids).')');
    else
      $criteria->where('wbfsys_tree_node.rowid in('.implode(',',$ids).')');
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_tree_node.rowid as "wbfsys_tree_node_rowid"', // variant: def-rowid 
      'wbfsys_tree_node.name as "wbfsys_tree_node_name"', // variant: def-by-context 
      'wbfsys_tree_node.m_parent as "wbfsys_tree_node_m_parent"', // variant: def-additional 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'wbfsys_tree_node' );
 ...
 /**
  ...
  public function setCalcQuery( $criteria, $params )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_tree_node.'.Db::PK.') as '.Db::Q_SIZE );
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    $criteria->where( ' wbfsys_tree_node.m_parent is null ' );
    ...
    if( $this->condition )
    {
      ...
      if( is_string($this->condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_tree_node.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_tree_node.rowid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_tree_node.rowid = \''.$part.'\'  or 
                wbfsys_tree_node.name = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_tree_node.name) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_tree_node
      if( isset( $condition['wbfsys_tree_node'] ) )
      {
        $whereCond = $condition['wbfsys_tree_node'];

        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(wbfsys_tree_node.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_tree_node.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_tree_node.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_tree_node']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_tree_node.m_parent ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_tree_node.m_parent,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_tree_node-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_tree_node-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_tree_node-name-order"' );
...
}//end class WbfsysTreeNode_Treetable_Postgresql_Query
