 ...
 * @licence WebFrap.net
 */
class WbfsysColorNode_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysColorNode_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_color_node.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_color_node') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_color_node.rowid ",
      'acls'
    );
...
}//end class WbfsysColorNode_Selection_Access
