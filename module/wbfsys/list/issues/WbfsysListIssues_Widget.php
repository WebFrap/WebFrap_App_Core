 ...
 *
 * @package WebFrap
 * @subpackage wbfsys_list_issues
 * @author Dominik Bonsch <dominik.bonsch@s-db.de>
 * @copyright Softwareentwicklung Dominik Bonsch <contact@webfrap.de>
 * @licence WebFrap.net
 */
class WbfsysListIssues_Widget
  extends WgtWidget
{
  /**
  ...
  public function asTab( $containerId, $tabId, $tabSize = 'medium' )
  {
    ...
    $access = new WbfsysListIssues_Widget_Access( null, null, $this );
    ...
    $query  = $db->newQuery( 'WbfsysListIssues_Widget' );
    ...
    $query->fetchEmbededRolesResponsible();


    $table = new WbfsysListIssues_Widget_Table_Element( 'tableWbfsysListIssuesItem', $view );
    $table->setId( 'wgt-table-wbfsys_list_issues-widget' );
    ...
    $params->searchFormId = 'wgt-form-wbfsys_list_issues-widget-search';
    ...
    // simple Suchfeld
    $tabPanel = new WbfsysListIssues_Widget_Table_Panel( $table );
    $tabPanel->setAccess( $params->access );
      ...
      $filterSubPanel = new WbfsysListIssues_Widget_Table_SubPanel_Filter( $this );
      ...
      if( !isset( $params->filter ) )
      	$params->filter = new TFlag();
      
      
      // Setzen der Filterzustände, werden aus der URL ausgelesen
      $filterSubPanel->setFilterStatus( $params->filter );
      
      // Access wird im Panel als Rechte Container verwendet
      $filterSubPanel->setAccess( $access );
      
      // Jetzt wird das SubPanel in das HauptPanel integriert, gerendert
      // wird jedoch erst wenn das HauptPanel gerendet wird, bis dahin
      // kann das SubPanel noch verändert werden
      $tabPanel->filterPanel = $filterSubPanel;
    ...
    if( $access->insert )
    {
      ...
      (
        ...
        (
          Wgt::ACTION_JS,
          'Create',
          '$R.get(\'maintab.php?c=Wbfsys.Issue.create&amp;ltab=table&amp;target_mask=WbfsysListIssues_Widget\');return false;',
          'control/new.png',
          'wcm wcm_ui_button',
          'wbf.label'
        )
    ...
    if( $access->admin )
    {
      ...
      (
        ...
        (
          Wgt::ACTION_JS,
          'Rights',
          '$R.get(\'maintab.php?c=Wbfsys.Issue_Acl.listing\');return false;',
          'control/rights.png',
          'wcm wcm_ui_button',
          'wbf.label'
        )
    ...
    $html = <<<HTML
    <div id="{$tabId}" class="wgt_tab {$tabSize} {$containerId}" title="Wbfsys List Issues"  >
      <form
        id="wgt-form-wbfsys_list_issues-widget-search"
        class="wcm wcm_req_ajax"
        action="ajax.php?c=Widget.WbfsysListIssues.reload" 
        method="get" ></form>
      {$table}
...
HTML;
  ...
  public function runReload( $tabSize = 'medium' )
  {
    ...
    $table->setPagingId( 'wgt-form-wbfsys_list_issues-widget-search' );
...
WGTJS;
  ...
  public function append(  $tabSize = 'medium' )
  {
    ...
    $table = new WbfsysListIssues_Widget_Table_Element
    (
      'tableWbfsysListIssuesItem',
      $view
    );
...
}// end class WbfsysListIssues_Widget
