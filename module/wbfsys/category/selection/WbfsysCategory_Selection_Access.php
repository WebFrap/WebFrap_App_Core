 ...
 * @licence WebFrap.net
 */
class WbfsysCategory_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysCategory_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_category.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_category') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_category.rowid ",
      'acls'
    );
...
}//end class WbfsysCategory_Selection_Access
