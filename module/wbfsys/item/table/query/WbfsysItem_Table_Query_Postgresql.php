 ...
 * @licence WebFrap.net
 */
class WbfsysItem_Table_Query_Postgresql
  extends LibSqlQuery
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function getIds()
  {
    ...
    {
      $this->ids[] = $row['wbfsys_item_rowid'];
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT wbfsys_item.'.Db::PK.' ) as '.Db::Q_SIZE );
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $params = null )
  {
    ...
    $criteria->where
    (
      " wbfsys_item.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['wbfsys_item_rowid']];
      $this->data[]     = $row;
    }
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT wbfsys_item.rowid as "wbfsys_item_rowid"', // variant: def-rowid 
      'wbfsys_item.name as "wbfsys_item_name"', // variant: def-by-context 
      'wbfsys_item.access_key as "wbfsys_item_access_key"', // variant: def-by-context 
      'wbfsys_item.cname as "wbfsys_item_cname"', // variant: def-by-context 
      'wbfsys_item.id_entity as "wbfsys_item_id_entity"', // variant: def-by-context 
      'wbfsys_item.id_management as "wbfsys_item_id_management"', // variant: def-by-context 
      'wbfsys_item.description as "wbfsys_item_description"', // variant: def-by-context 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    (
      ...
      null,
      'wbfsys_entity'
    );// wbfsys_entity  by alias wbfsys_entity
    ...
    (
      ...
      null,
      'wbfsys_management'
    );// wbfsys_management  by alias wbfsys_management
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    if( $this->condition )
    {
      ...
      if( is_string( $this->condition ) )
      {
        ...
        {
          $criteria->where( 'wbfsys_item.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'wbfsys_item.rowid = '.$condition );
        }
        else
        {
 ...
 /**
  ...
  public function checkConditions( $criteria, array $condition )
  {
    	...
    	if( isset($condition['ids']) && !empty( $condition['ids'] ) )
    	{
				$criteria->where
        (
          'wbfsys_item.rowid = IN( '. implode( ', ', $condition['ids'] ) .' ) ';
        );
    	}
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 wbfsys_item.rowid = \''.$part.'\'  or 
                wbfsys_item.name = \''.$part.'\'  or 
                wbfsys_item.access_key = \''.$part.'\'  or 
                wbfsys_item.cname = \''.$part.'\'  or 
                wbfsys_item.description = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(wbfsys_item.name) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_item.access_key) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_item.cname) like UPPER(\'%'.$part.'%\') or 
                UPPER(wbfsys_item.description) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  wbfsys_item
      if( isset( $condition['wbfsys_item'] ) )
      {
        $whereCond = $condition['wbfsys_item'];

        if( isset( $whereCond['name']) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' wbfsys_item.name = \''.$whereCond['name'].'\' ');

        if( isset( $whereCond['access_key']) && trim( $whereCond['access_key'] ) != ''  )
          $criteria->where( ' wbfsys_item.access_key = \''.$whereCond['access_key'].'\' ');

        if( isset( $whereCond['cname']) && trim( $whereCond['cname'] ) != ''  )
          $criteria->where( ' wbfsys_item.cname = \''.$whereCond['cname'].'\' ');

        if( isset( $whereCond['id_entity']) && trim( $whereCond['id_entity'] ) != ''  )
          $criteria->where( ' wbfsys_item.id_entity = \''.$whereCond['id_entity'].'\' ');

        if( isset( $whereCond['id_management']) && trim( $whereCond['id_management'] ) != ''  )
          $criteria->where( ' wbfsys_item.id_management = \''.$whereCond['id_management'].'\' ');

        if( isset( $whereCond['description']) && trim( $whereCond['description'] ) != ''  )
          $criteria->where( ' wbfsys_item.description = \''.$whereCond['description'].'\' ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' wbfsys_item.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' wbfsys_item.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' wbfsys_item.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' wbfsys_item.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' wbfsys_item.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' wbfsys_item.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' wbfsys_item.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' wbfsys_item.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['wbfsys_item']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "wbfsys_item.name ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(wbfsys_item.name,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function checkLimitAndOrder( $criteria, $params  )
  {
     ...
     // inject the default order
      ...
      $criteria->orderBy( 'wbfsys_item.name' );
      $criteria->selectAlso( 'wbfsys_item.name as "wbfsys_item-name-order"' );
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
     ...
     // inject the default order
      ...
      $envelop->groupBy( 'inner_acl."wbfsys_item-name-order"' );
      $envelop->selectAlso( 'inner_acl."wbfsys_item-name-order"' );
      $envelop->orderBy( 'inner_acl."wbfsys_item-name-order"' );
...
}//end class WbfsysItem_Table_Query_Postgresql
