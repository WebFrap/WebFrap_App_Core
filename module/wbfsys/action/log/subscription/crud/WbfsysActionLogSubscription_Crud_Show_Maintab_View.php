 ...
 * @licence WebFrap.net
 */
class WbfsysActionLogSubscription_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysActionLogSubscription Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysActionLogSubscription = $this->model->getEntityWbfsysActionLogSubscription();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Action Log Subscription: {@text@}',
      'wbfsys.action_log_subscription.label',
      array( 'text' => $entityWbfsysActionLogSubscription->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Action Log Subscription: {@text@}',
      'wbfsys.action_log_subscription.label',
      array( 'text' => $entityWbfsysActionLogSubscription->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.ActionLogSubscription.show  &amp;objid='.$entityWbfsysActionLogSubscription
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.ActionLogSubscription.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_action_log_subscription-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/action_log_subscription/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysActionLogSubscription_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.ActionLogSubscription.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_action_log_subscription' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_action_log_subscription a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.ActionLogSubscription.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.ActionLogSubscription.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysActionLogSubscription_Crud_Show_Maintab_View
