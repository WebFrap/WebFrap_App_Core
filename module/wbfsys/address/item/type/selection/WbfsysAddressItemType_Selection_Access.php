 ...
 * @licence WebFrap.net
 */
class WbfsysAddressItemType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param WbfsysAddressItemType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'wbfsys_address_item_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-wbfsys'), UPPER('mgmt-wbfsys_address_item_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = wbfsys_address_item_type.rowid ",
      'acls'
    );
...
}//end class WbfsysAddressItemType_Selection_Access
