 ...
 * @licence WebFrap.net
 */
class WbfsysAddressItemType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysAddressItemType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysAddressItemType = $this->model->getEntityWbfsysAddressItemType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show address item Type: {@text@}',
      'wbfsys.address_item_type.label',
      array( 'text' => $entityWbfsysAddressItemType->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'address item Type: {@text@}',
      'wbfsys.address_item_type.label',
      array( 'text' => $entityWbfsysAddressItemType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.AddressItemType.show  &amp;objid='.$entityWbfsysAddressItemType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.AddressItemType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_address_item_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/address_item_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysAddressItemType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.AddressItemType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_address_item_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_address_item_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.AddressItemType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.AddressItemType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysAddressItemType_Crud_Show_Maintab_View
