 ...
 * @licence WebFrap.net
 */
class WbfsysVrefFileType_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysVrefFileType Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysVrefFileType = $this->model->getEntityWbfsysVrefFileType();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Vref File Type: {@text@}',
      'wbfsys.vref_file_type.label',
      array( 'text' => $entityWbfsysVrefFileType->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.VrefFileType.show  &amp;objid='.$entityWbfsysVrefFileType
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.VrefFileType.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_vref_file_type-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/vref_file_type/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysVrefFileType_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.VrefFileType.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_vref_file_type' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_vref_file_type a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.VrefFileType.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.VrefFileType.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysVrefFileType_Crud_Show_Maintab_View
