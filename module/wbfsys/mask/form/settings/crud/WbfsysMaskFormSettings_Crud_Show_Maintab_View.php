 ...
 * @licence WebFrap.net
 */
class WbfsysMaskFormSettings_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der WbfsysMaskFormSettings Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityWbfsysMaskFormSettings = $this->model->getEntityWbfsysMaskFormSettings();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Mask Form Settings: {@text@}',
      'wbfsys.mask_form_settings.label',
      array( 'text' => $entityWbfsysMaskFormSettings->text() )
    );
    $i18nLabel = $this->i18n->l
    (
      'Mask Form Settings: {@text@}',
      'wbfsys.mask_form_settings.label',
      array( 'text' => $entityWbfsysMaskFormSettings->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Wbfsys.MaskFormSettings.show  &amp;objid='.$entityWbfsysMaskFormSettings
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Wbfsys.MaskFormSettings.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-wbfsys_mask_form_settings-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'wbfsys/mask_form_settings/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'WbfsysMaskFormSettings_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Wbfsys.MaskFormSettings.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_mask_form_settings' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_wbfsys_mask_form_settings a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Wbfsys.MaskFormSettings.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Wbfsys.MaskFormSettings.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class WbfsysMaskFormSettings_Crud_Show_Maintab_View
