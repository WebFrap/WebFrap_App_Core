 ...
 * @licence WebFrap.net
 */
class CoreName_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der CoreName Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityCoreName = $this->model->getEntityCoreName();
    ...
    (
      'Show Name: {@text@}',
      'core.name.label',
      array( 'text' => $entityCoreName->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Core.Name.show  &amp;objid='.$entityCoreName
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Core.Name.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-core_name-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'core/name/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'CoreName_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Core.Name.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_core_name' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_core_name a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Core.Name.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Core.Name.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class CoreName_Crud_Show_Maintab_View
