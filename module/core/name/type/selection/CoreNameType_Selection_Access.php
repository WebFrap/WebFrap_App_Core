 ...
 * @licence WebFrap.net
 */
class CoreNameType_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param CoreNameType_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'core_name_type.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-core'), UPPER('mgmt-core_name_type') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = core_name_type.rowid ",
      'acls'
    );
...
}//end class CoreNameType_Selection_Access
