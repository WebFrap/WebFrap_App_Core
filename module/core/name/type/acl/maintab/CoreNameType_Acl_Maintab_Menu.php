 ...
 * @licence WebFrap.net
 */
class CoreNameType_Acl_Maintab_Menu
  extends WgtDropmenu
{
...
{$entries->support}
  ...
  <div 
  	class="wgt-panel-control {$this->view->id}-core_name_type-acl box-qfd_users"
  	style="display:none;"
  	 >
    <div 
      ...
      <ul>
        <li><a 
          class="wcm wcm_req_ajax" 
            href="modal.php?c=Webfrap.Docu.show&amp;key=core_name_type-acl" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a>
        </li>
        <!--
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Issue.create&amp;refer=core_name_type-acl" >{$iconBug} {$this->view->i18n->l('Bug','wbf.label')}</a>
        </li>
        -->
        <li><a 
          class="wcm wcm_req_ajax" 
          href="modal.php?c=Wbfsys.Faq.create&amp;refer=core_name_type-acl" >{$iconFaq} {$this->view->i18n->l('Faq','wbf.label')}</a>
        </li>
      </ul>
    </span>
  </li>
...
HTML;
   ...
   *
   * @param CoreNameType_Acl_Maintab $view
   * @param int $areaId
   * @param TArray $params
   */
  public function injectMenuLogic( $view, $areaId, $params  )
  {
    ...
    self.getObject().find(".wgtac_mask_entity_rights").click(function(){
      ...
      self.close( );
      \$R.get( 'maintab.php?c=Mgmt.Acl.listing&dkey=core_name_type' );
    });
    
    self.getObject().find(".wgtac_masks_overview").click(function(){
      \$R.get( 'modal.php?c=Mgmt.Acl.listAllMasks&dkey=core_name_type' );
      \$S('#{$this->id}-control').dropdown('close');
    });
    

    self.getObject().find('#wgt-button-core_name_type-acl-form-append').click(function(){
    
      if(\$S('#wgt-input-core_name_type-acl-id_group').val()==''){
        \$D.errorWindow('Error','Please select a group first');
        return false;
      }

      \$R.form('wgt-form-core_name_type-acl-append');
      \$S('#wgt-form-core_name_type-acl-append').get(0).reset();
      return false;
...
} // end class CoreNameType_Acl_Maintab_Menu */
