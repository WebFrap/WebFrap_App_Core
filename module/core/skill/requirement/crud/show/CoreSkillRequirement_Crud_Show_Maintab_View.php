 ...
 * @licence WebFrap.net
 */
class CoreSkillRequirement_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der CoreSkillRequirement Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityCoreSkillRequirement = $this->model->getEntityCoreSkillRequirement();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show Skill Requirement: {@text@}',
      'core.skill_requirement.label',
      array( 'text' => $entityCoreSkillRequirement->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Core.SkillRequirement.show  &amp;objid='.$entityCoreSkillRequirement
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Core.SkillRequirement.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-core_skill_requirement-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'core/skill_requirement/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'CoreSkillRequirement_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Core.SkillRequirement.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_core_skill_requirement' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_core_skill_requirement a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Core.SkillRequirement.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Core.SkillRequirement.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class CoreSkillRequirement_Crud_Show_Maintab_View
