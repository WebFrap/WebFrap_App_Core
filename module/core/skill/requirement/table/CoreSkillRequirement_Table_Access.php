 ...
 * @licence WebFrap.net
 */
class CoreSkillRequirement_Table_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param CoreSkillRequirement_Table_Query $query
  ...
  public function fetchListTableDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'core_skill_requirement.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-core'), UPPER('mod-core-cat-core_data') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = core_skill_requirement.rowid ",
      'acls'
    );
...
}//end class CoreSkillRequirement_Table_Access
