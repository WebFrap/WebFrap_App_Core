 ...
 * @licence WebFrap.net
 */
class CoreOrganisation_Selection_Access
  extends LibAclPermissionList
{
  ...
  /**
   * @param CoreOrganisation_Selection_Query $query
  ...
  public function fetchListSelectionDefault( $query, $condition, $params )
  {
    ...
    $criteria->select( array( 'core_organisation.rowid as rowid' )  );
...
SQL;
    ...
    (
      " {$joinType} JOIN
        ...
        ON
          UPPER(acls.\"acl-area\") IN( UPPER('mod-core'), UPPER('mgmt-core_organisation') )
            AND acls.\"acl-user\" = {$userId}
            AND acls.\"acl-vid\" = core_organisation.rowid ",
      'acls'
    );
...
}//end class CoreOrganisation_Selection_Access
