...
////////////////////////////////////////////////////////////////////////////////
   ...
   * @var string 
   */
  public $entityName  = 'CoreLocation';
...
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function createForm( $entity, $fields = array(), $params = null  )
  {
    ...
    foreach( $fields as $key )
    {
      ...
      {
        if(DEBUG)
          Debug::console( 'Call to nonexisting method: '.$method.' in Form: CoreLocation' );
      }
    }
 ...
 /**
  * create a search form for the CoreLocation entity
  *
  * @param Entity $entity
  * @param array $fields
  * @param TFlag $params
...
////////////////////////////////////////////////////////////////////////////////
  ...
  * @return void
  */
  public function input_x_cord( $params )
  {

      //tpl: class ui:numeric
      $inputXCord = $this->view->newInput( 'input'.$this->prefix.'XCord', 'Numeric' );
      $this->items['x_cord'] = $inputXCord;
      $inputXCord->addAttributes
      (
        array
        (
          'name'      => $this->keyName.'[x_cord]',
          'id'        => 'wgt-input-'.$this->keyName.'_x_cord'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip valid_numeric small'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $this->view->i18n->l( 'Insert value for X Cord (Location)', 'core.location.label' ),
          'maxlength' => $this->entity->maxSize( 'x_cord' ),
        )
      );
      $inputXCord->setWidth( 'small' );

      $inputXCord->setReadOnly( $this->isReadOnly( 'x_cord' ) );
      $inputXCord->setData( $this->entity->getNumeric( 'x_cord' ) );
      $inputXCord->setLabel
      (
        $this->view->i18n->l( 'X Cord', 'core.location.label' ) ,
        $this->entity->required( 'x_cord' )
      );
  ...
  }//end public function input_x_cord */
  ...
  * @return void
  */
  public function input_y_cord( $params )
  {

      //tpl: class ui:numeric
      $inputYCord = $this->view->newInput( 'input'.$this->prefix.'YCord', 'Numeric' );
      $this->items['y_cord'] = $inputYCord;
      $inputYCord->addAttributes
      (
        array
        (
          'name'      => $this->keyName.'[y_cord]',
          'id'        => 'wgt-input-'.$this->keyName.'_y_cord'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip valid_numeric small'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $this->view->i18n->l( 'Insert value for Y Cord (Location)', 'core.location.label' ),
          'maxlength' => $this->entity->maxSize( 'y_cord' ),
        )
      );
      $inputYCord->setWidth( 'small' );

      $inputYCord->setReadOnly( $this->isReadOnly( 'y_cord' ) );
      $inputYCord->setData( $this->entity->getNumeric( 'y_cord' ) );
      $inputYCord->setLabel
      (
        $this->view->i18n->l( 'Y Cord', 'core.location.label' ) ,
        $this->entity->required( 'y_cord' )
      );
  ...
  }//end public function input_y_cord */
  ...
  * @return void
  */
  public function input_z_cord( $params )
  {

      //tpl: class ui:numeric
      $inputZCord = $this->view->newInput( 'input'.$this->prefix.'ZCord', 'Numeric' );
      $this->items['z_cord'] = $inputZCord;
      $inputZCord->addAttributes
      (
        array
        (
          'name'      => $this->keyName.'[z_cord]',
          'id'        => 'wgt-input-'.$this->keyName.'_z_cord'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip valid_numeric small'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $this->view->i18n->l( 'Insert value for Z Cord (Location)', 'core.location.label' ),
          'maxlength' => $this->entity->maxSize( 'z_cord' ),
        )
      );
      $inputZCord->setWidth( 'small' );

      $inputZCord->setReadOnly( $this->isReadOnly( 'z_cord' ) );
      $inputZCord->setData( $this->entity->getNumeric( 'z_cord' ) );
      $inputZCord->setLabel
      (
        $this->view->i18n->l( 'Z Cord', 'core.location.label' ) ,
        $this->entity->required( 'z_cord' )
      );
  ...
  }//end public function input_z_cord */
  ...
  * @return void
  */
  public function input_maps_link( $params )
  {

      //tpl: class ui:text
      $inputMapsLink = $this->view->newInput( 'input'.$this->prefix.'MapsLink' , 'Text' );
      $this->items['maps_link'] = $inputMapsLink;
      $inputMapsLink->addAttributes
      (
        array
        (
          'name'      => $this->keyName.'[maps_link]',
          'id'        => 'wgt-input-'.$this->keyName.'_maps_link'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $this->view->i18n->l( 'Insert value for Maps Link (Location)', 'core.location.label' ),
          'maxlength' => $this->entity->maxSize( 'maps_link' ),
        )
      );
      $inputMapsLink->setWidth( 'medium' );

      $inputMapsLink->setReadOnly( $this->isReadOnly( 'maps_link' ) );
      $inputMapsLink->setData( $this->entity->getSecure('maps_link') );
      $inputMapsLink->setLabel
      (
        $this->view->i18n->l( 'Maps Link', 'core.location.label' ),
        $this->entity->required( 'maps_link' )
      );
  ...
  }//end public function input_maps_link */
  ...
  public function input_rowid( $params )
  {
      ...
      (
        ...
        (
          ...
          'title'     => $this->view->i18n->l( 'Insert value for Rowid (Location)', 'core.location.label' ),
        )
      );
      ...
      $inputRowid->setLabel
      (
        $this->view->i18n->l( 'Rowid', 'core.location.label' ),
  ...
  public function input_m_time_created( $params )
  {
      ...
      (
        ...
        (
          ...
          'title'     => $this->view->i18n->l( 'Insert value for Time Created (Location)', 'core.location.label' ),
      ...
      (
        $this->view->i18n->l( 'Time Created', 'core.location.label' ),
    ...
    if( !Webfrap::classLoadable( 'WbfsysRoleUser_Entity' ) )
    {
      ...
      (
        ...
        'id'        => 'wgt-input-'.$this->keyName.'_m_role_create'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
        'title'     => $this->view->i18n->l( 'Insert value for Role Create (Location)', 'core.location.label' ),
      ));
      ...
      $inputMRoleCreate->setLabel( $this->view->i18n->l( 'Role Create', 'core.location.label' ) );
  ...
  public function input_m_time_changed( $params )
  {
      ...
      (
        ...
        (
          ...
          'class'     => 'wcm wcm_ui_tip small'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $this->view->i18n->l( 'Insert value for Time Changed (Location)', 'core.location.label' ),
      ...
      (
        $this->view->i18n->l( 'Time Changed', 'core.location.label' ),
    ...
    if( !Webfrap::classLoadable( 'WbfsysRoleUser_Entity' ) )
    {
      ...
      (
        ...
        'id'        => 'wgt-input-'.$this->keyName.'_m_role_change'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
        'title'     => $this->view->i18n->l( 'Insert value for Role Change (Location)', 'core.location.label' ),
      ));
      ...
      $inputMRoleChange->setLabel( $this->view->i18n->l( 'Role Change', 'core.location.label' ) );
  ...
  public function input_m_version( $params )
  {
      ...
      (
        ...
        (
          ...
          'title'     => $this->view->i18n->l( 'Insert value for Version (Location)', 'core.location.label' ),
        )
      );
      ...
      (
        $this->view->i18n->l( 'Version', 'core.location.label' ),
  ...
  public function input_m_uuid( $params )
  {
      ...
      (
        ...
        (
          ...
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $this->view->i18n->l( 'Insert value for Uuid (Location)', 'core.location.label' ),
        )
      );
      ...
      $inputMUuid->setLabel
      (
        $this->view->i18n->l( 'Uuid', 'core.location.label' ),
...
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function search_m_role_create( $params )
  {
    ...
    $listUrl = 'modal.php?c=Wbfsys.RoleUser.selection&amp;target=core_location_m_role_create';
  ...
  */
  public function search_m_role_change( $params )
  {
    ...
    $listUrl = 'modal.php?c=Wbfsys.RoleUser.selection&amp;target=core_location_m_role_change';
