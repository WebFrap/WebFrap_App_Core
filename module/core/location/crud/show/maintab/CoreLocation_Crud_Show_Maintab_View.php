 ...
 * @licence WebFrap.net
 */
class CoreLocation_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der CoreLocation Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityCoreLocation = $this->model->getEntityCoreLocation();
    ...
    (
      'Show location: {@text@}',
      'core.location.label',
      array( 'text' => $entityCoreLocation->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Core.Location.show  &amp;objid='.$entityCoreLocation
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Core.Location.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-core_location-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'core/location/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'CoreLocation_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Core.Location.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_core_location' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_core_location a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Core.Location.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Core.Location.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class CoreLocation_Crud_Show_Maintab_View
