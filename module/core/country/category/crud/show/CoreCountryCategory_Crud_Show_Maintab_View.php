 ...
 * @licence WebFrap.net
 */
class CoreCountryCategory_Crud_Show_Maintab_View
  extends WgtMaintab
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * Das Show Form der CoreCountryCategory Maske
  *
  ...
  */
  public function displayForm( $objid, $params )
  {
    ...
    $entityCoreCountryCategory = $this->model->getEntityCoreCountryCategory();

    // fetch the i18n text for title, status and bookmark
    $i18nTitle = $this->i18n->l
    (
      'Show country Category: {@text@}',
      'core.country_category.label',
      array( 'text' => $entityCoreCountryCategory->text() )
    );
    ...
    (
      $i18nTitle,
      'maintab.php?c=Core.CountryCategory.show  &amp;objid='.$entityCoreCountryCategory
    );

    // Control Flags setzen
    // Form Action und ID setzen
    $params->formAction = 'ajax.php?c=Core.CountryCategory.update&amp;objid='.$objid;
    $params->formId = 'wgt-form-core_country_category-show-'.$objid;
    $params->refId  = $objid;
    ...
    $this->setTemplate( 'core/country_category/maintab/crud/form_show' );
    ...
    $form = $this->newForm( 'CoreCountryCategory_Crud_Show' );
...
{$bookmark}
    ...
    self.getObject().find(".wgtac_metadata").click(function(){
      ...
      \$R.get('modal.php?c=Core.CountryCategory.showMeta&objid={$objid}');
    });

    self.getObject().find(".wgtac_back_to_list").click(function(){
      self.close();
      
      if( \$S( '#wgt-maintab_tab_wgt_tab-listing_core_country_category' ).length ){
        \$S( '#wgt-maintab_tab_wgt_tab-listing_core_country_category a' ).click();
      }
      else{
        \$R.get( 'maintab.php?c=Core.CountryCategory.listing' );
      }
    });
...
BUTTONJS;
    ...
    self.getObject().find(".wgtac_open_edit").click(function(){
      \$R.get('maintab.php?c=Core.CountryCategory.edit&objid={$objid}',{success:function(){self.close();});
...
}//end class CoreCountryCategory_Crud_Show_Maintab_View
