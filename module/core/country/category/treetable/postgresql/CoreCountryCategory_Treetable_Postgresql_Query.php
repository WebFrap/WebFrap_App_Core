 ...
 * @licence WebFrap.net
 */
class CoreCountryCategory_Treetable_Postgresql_Query
  extends LibSqlQuery
{
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  ...
  public function fetch( $condition = null, $params = null )
  {
    ...
    if($params->loadFullSize)
      $this->calcQuery = $criteria->count( 'count(core_country_category.'.Db::PK.') as '.Db::Q_SIZE );
    ...
    {
      $this->data[] = $row;
      $ids[] = $row['core_country_category_rowid'];
    }
    ...
    while( $children = $this->loadChildren( $ids, true ) )
    {
      ...
      {
        $this->childs[$child['core_country_category_id_parent']][] = $child;
        $ids[] = $child['core_country_category_rowid'];
      }
 ...
 /**
  ...
  public function fetchInAcls( array $inKeys, $access, $condition, $params = null )
  {
    ...
    $criteria->where
    (
      " core_country_category.rowid  IN( ". implode( ', ', array_keys($inKeys) ) ." )"
    );
    ...
    {
      $row['acl-level'] = $inKeys[$row['core_country_category_rowid']];
      $this->data[]     = $row;
      $ids[] = $row['core_country_category_rowid'];
    }
    ...
    while( $childIds = $access->fetchChildrenIds( $pName, 'treetable', $this, $ids, $condition, $params ) )
    {
      ...
      foreach( $children as $child )
      {
        $child['acl-level'] = $childIds[$child['core_country_category_rowid']];
 ...
 /**
  ...
  public function loadChildren( $ids, $loadChildren = false )
  {
    ...
    if( $loadChildren )
      $criteria->where('core_country_category.id_parent in('.implode(',',$ids).')');
    else
      $criteria->where('core_country_category.rowid in('.implode(',',$ids).')');
 ...
 /**
  ...
  public function setCols( $criteria )
  {
    ...
    $cols = array
    (
      'DISTINCT core_country_category.rowid as "core_country_category_rowid"', // variant: def-rowid 
      'core_country_category.name as "core_country_category_name"', // variant: def-by-context 
      'core_country_category.title as "core_country_category_title"', // variant: def-by-context 
      'core_country_category.description as "core_country_category_description"', // variant: def-by-context 
      'core_country_category.id_parent as "core_country_category_id_parent"', // variant: def-additional 
    );
  ...
  public function setTables( $criteria   )
  {
    ...
    $criteria->from( 'core_country_category' );
 ...
 /**
  ...
  public function setCalcQuery( $criteria, $params )
  {
    ...
    if( $params->loadFullSize )
      $this->calcQuery = $criteria->count( 'count(DISTINCT core_country_category.'.Db::PK.') as '.Db::Q_SIZE );
   ...
   * 
   * In Treetable werden auf rootebene nur Rootknoten mit leeren  id_parent
   * ausgelesen
   *
   * @param LibSqlCriteria $criteria
  ...
  public function appendConditions( $criteria, $condition, $params )
  {
    ...
    $criteria->where( ' core_country_category.id_parent is null ' );
    ...
    if( $this->condition )
    {
      ...
      if( is_string($this->condition) )
      {
        ...
        {
          $criteria->where( 'core_country_category.rowid = '.$this->condition );
        }
        else
        {
    ...
    if( $condition )
    {
      ...
      if( is_string( $condition) )
      {
        ...
        {
          $criteria->where( 'core_country_category.rowid = '.$condition );
        }
        else
        {
  ...
  public function checkConditions( $criteria, array $condition )
  {
      ...
      if( isset($condition['free']) && trim( $condition['free'] ) != ''  )
      {
         ...
         )
         {
            ...
            (
              '(
 core_country_category.rowid = \''.$part.'\'  or 
                core_country_category.name = \''.$part.'\' 
              )'
            );
         }
        else
          ...
          if( strpos( $condition['free'], ',' ) )
          {
            ...
            foreach( $parts as $part )
            {
              ...
              ('(
                ...
                UPPER(core_country_category.name) like UPPER(\'%'.$part.'%\')
              )');
      ...
      // search conditions for  core_country_category
      if( isset( $condition['core_country_category'] ) )
      {
        $whereCond = $condition['core_country_category'];

        if( isset( $whereCond['name'] ) && trim( $whereCond['name'] ) != ''  )
          $criteria->where( ' UPPER(core_country_category.name) like UPPER(\'%'.$whereCond['name'].'%\') ');

        if( isset( $whereCond['title'] ) && trim( $whereCond['title'] ) != ''  )
          $criteria->where( ' UPPER(core_country_category.title) like UPPER(\'%'.$whereCond['title'].'%\') ');

        if( isset( $whereCond['description'] ) && trim( $whereCond['description'] ) != ''  )
          $criteria->where( ' UPPER(core_country_category.description) like UPPER(\'%'.$whereCond['description'].'%\') ');

        // append meta information
        if( isset($whereCond['m_role_create']) && trim($whereCond['m_role_create']) != ''  )
          $criteria->where( ' core_country_category.m_role_create = '.$whereCond['m_role_create'].' ');

        if( isset($whereCond['m_role_change']) && trim($whereCond['m_role_change']) != ''  )
          $criteria->where( ' core_country_category.m_role_change = '.$whereCond['m_role_change'].' ');

        if( isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where( ' core_country_category.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if( isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where( ' core_country_category.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

        if( isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != ''  )
          $criteria->where( ' core_country_category.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

        if( isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != ''  )
          $criteria->where( ' core_country_category.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

        if( isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != ''  )
          $criteria->where( ' core_country_category.rowid >= \''.$whereCond['m_rowid'].'\' ');

        if( isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != ''  )
          $criteria->where( ' core_country_category.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

      }//end if( isset ($condition['core_country_category']) )
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "core_country_category.id_parent ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(core_country_category.id_parent,1,1)) = '".strtoupper($params->begin)."'" );
        }
  ...
  public function injectAclOrder( $criteria, $envelop, $params  )
  {
    ...
    else // if not use the default
    {
      ...
      $envelop->groupBy( 'inner_acl."core_country_category-name-order"' );
      $envelop->selectAlso( 'inner_acl."core_country_category-name-order"' );
      $envelop->orderBy( 'inner_acl."core_country_category-name-order"' );
...
}//end class CoreCountryCategory_Treetable_Postgresql_Query
