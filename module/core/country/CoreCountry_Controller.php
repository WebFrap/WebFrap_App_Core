...
////////////////////////////////////////////////////////////////////////////////
  ...
  *   Datensatzes des types: core_country
  *
  ...
  *
  *   @url maintab.php?c=Core.Country.create
  *
  *   @type UI Service
  ...
  */
  public function service_create( $request, $response )
  {
    ...
    $params->contextKey = 'core_country-create';

    $access = new CoreCountry_Crud_Access_Create( null, null, $this );
    ...
    (
      'form-core_country-create',
      'CoreCountry_Crud_Create',
      'displayForm',
      null,
      true
    );
    // laden des models und direkt übergabe in die view
    /* @var $model CoreCountry_Crud_Model */
    $model = $this->loadModel( 'CoreCountry_Crud' );
  ...
  *   @tutorial <a href="http://webfrap.net/doc/{{version}}/index.php?page=architecture.views.overview" >Tutorial Viewtypes</a>
  *
  *   @access maintab.php?c=Core.Country.edit&objid=123
  *
  *   @type UI Service
  *   @param LibRequestHttp $request
  *   @param LibResponseHttp $response
  ...
  */
  public function service_edit( $request, $response )
  {
    ...
    $entityCoreCountry = $model->getEntityCoreCountry( $objid );
    ...
    $params->entity = $entityCoreCountry;

    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-edit-'.$objid;
    
    if( !$params->maskRoot )
    	$params->maskRoot = 'core_country:edit';

    $access = new CoreCountry_Crud_Access_Edit( null, null, $this );
    $access->load( $user->getProfileName(), $params, $entityCoreCountry );
    ...
    $view = $response->loadView
    (
      'form-core_country-edit-'.$objid,
      'CoreCountry_Crud_Edit',
      'displayForm',
      null,
      true
    );
  ...
  */
  public function service_show( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-show-'.$objid;
    ...
    if( is_null($params->aclRoot) || 1 == $params->aclLevel  )
    {
      ...
      $params->maskRoot      = 'core_country:show';
...
////////////////////////////////////////////////////////////////////////////////
 ...
 /**
  * de:
  * Service zum Erstellen neuer Datensätze des types: core_country
  *
  ...
  * zu erstellen.
  *
  * @call POST ajax.php?c=Core.Country.insert
  * {
  ...
  */
  public function service_insert( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-insert';

    $access = new CoreCountry_Crud_Access_Insert( null, null, $this );
    $access->load( $user->getProfileName(),  $params );
    ...
    else
    {
    	...
    	$entityCoreCountry = $model->getEntity();
    
      // der contextKey wird benötigt um potentielle Konflikte in der UI
      // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
      $params->contextKey = 'core_country-update-'.$entityCoreCountry;
      ...
      {
        $access = new CoreCountry_Crud_Access_Update( null, null, $this );
        $access->load( $user->getProfileName(), $params, $entityCoreCountry );
      }
      $params->access = $access;
      ...
      if( !$params->mask )
        $params->mask = 'CoreCountry';

      // laden der angeforderten view
      $view = $response->loadView
      (
        'listing_core_country',
        $params->mask.'_'.$listType,
        'displayInsert'
      );

      // model wird benötigt
      /* @var $model CoreCountry_Table_Model */ // nicht zwangsweise dieses model aber höchstwarhscheinlich
  ...
  *
  * @call POST ajax.php?c=Core.Country.update
  * {
  *   @get_param: cname ltype, der Type der des Listenelements. Sollte sinnigerweise
  *     der gleich type wie das Listenelement sein, für das die Suche angestoßen wurde
  *
  ...
  */
  public function service_update( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-update-'.$objid;
    ...
    // if fails stop here
    if( !$model->fetchUpdateData( $entityCoreCountry, $params ) )
    {
      // wenn die daten nicht valide sind, dann war es eine ungültige anfrage
      throw new InvalidRequest_Exception
      (
  ...
  * @param CoreCountry_Crud_Model $model the model
  ...
  *
  * @access DELETE ajax.php?c=Core.Country.delete&objid=123
  *
  * @param LibRequestHttp $request
  * @param LibResponseHttp $response
  ...
  */
  public function service_delete( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-delete-'.$objid;

    $access = new CoreCountry_Crud_Access_Delete( null, null, $this );
    $access->load( $user->getProfileName(), $params, $entityCoreCountry );
    ...
    $error = $model->delete( $entityCoreCountry, $params );
    ...
    $error = $view->displayDelete( $entityCoreCountry, $params );
  ...
  */
  public function service_deleteAll( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-delete';
  ...
  * der eintrag muss direkt mit der rowid adressiert werden
  *
  * @access DELETE ajax.php?c=Core.Country.deleteSelection&objid=123
  *
  * @param LibRequestHttp $request
  * @param LibResponseHttp $response
  * @return boolean success flag
...
////////////////////////////////////////////////////////////////////////////////
  ...
  * @tutorial <a href="http://webfrap.net/doc/{{version}}/index.php?page=architecture.views.overview" >Tutorial Viewtypes</a>
  *
  * @url maintab.php?c=Core.Country.listing
  *
  * @param LibRequestHttp $request
  * @param LibResponseHttp $response
  *
  ...
  */
  public function service_listing( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-listing';
    ...
    $containerClass = 'CoreCountry_'.$listType.'_Access';
    ...
    /* @var $model CoreCountry_Table_Model */
    $model = $this->loadModel( 'CoreCountry_'.$listType );
    $model->setAccess( $access );

        $protocol = Protocol::getDefault();
        $protocol->updateLastVisited
        (
          'core_country-'.$params->ltype,
          'CoreCountry',
          'List of Countrys'
        );
  ...
  */
  ...
  /**
   * the default selection for the management  CoreCountry
   * @param LibRequestHttp $request
   * @param LibResponseHttp $response
   * @return boolean
   */
  public function service_selection( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-selection';

    // ok nun kommen wir zu der zugriffskontrolle
    $access = new CoreCountry_Selection_Access( null, null, $this );
    $access->load( $user->getProfileName(), $params );
    ...
    $view = $response->loadView
    (
      'selection_core_country',
      'CoreCountry_Selection',
      'displaySelection',
      null,
      true
    );
    ...
    $model = $this->loadModel( 'CoreCountry_Selection' );
    $model->setAccess( $access );
    $view->setModel( $model );
  ...
  *
  * @call GET/POST: maintab.php?c=Core.Country.filter
  * {
  *   @get_param: cname ltype, der Type der des Listenelements. Sollte sinnigerweise
  *     der gleich type wie das Listenelement sein, für das die Suche angestoßen wurde
  *
  ...
  */
  public function service_filter( $request, $response )
  {
    ...
    /* @var $model CoreCountry_Selection_Model */
    $model   = $this->loadModel( 'CoreCountry_Selection' );
    $model->setAccess( $access );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function service_data( $request, $response )
  {
    ...
    if( $params->context && 'create' == $params->context )
      $params->contextKey = 'core_country-data-'.$objid;
    else
      $params->contextKey = 'core_country-data';
    ...
    // wenn alles glatt geht gibt die view null zurück und der keks ist gegessen
    $error = $view->displayData( $entityCoreCountry, $params );
  ...
  */
  public function service_append( $request, $response )
  {
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-append-'.$objid;
    ...
    $ui = $this->loadUi( 'CoreCountry_'.$listType );
    $model->setAccess( $access );
    $ui->setModel( $model );
  ...
  public function service_textByKey( $request, $response )
  {
    ...
    $access = new CoreCountry_Crud_Access_Listing( null, null, $this );
    $access->load( $user->getProfileName(), $params );
    ...
    // der contextKey wird benötigt um potentielle Konflikte in der UI
    // bei der Anzeige von mehreren Windows oder Tabs zu vermeiden
    $params->contextKey = 'core_country-text_by_key_'.$objid;

    $params->target = $request->param( 'target', Validator::CNAME );

    $ui = $this->loadUi( 'CoreCountry_Crud' );
    $model->setAccess( $access );
    $ui->setModel( $model );


    $ui->textByKey( $entityCoreCountry, $params );
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function cleanPost( )
  {
    ...
    $this->request->removeData( 'core_country' );
...
} // end class CoreCountry_Controller */
