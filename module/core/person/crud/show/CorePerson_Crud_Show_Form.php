 ...
 * @licence WebFrap.net
 */
class CorePerson_Crud_Show_Form
  extends WgtCrudForm
{
////////////////////////////////////////////////////////////////////////////////
// Attributes
...
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  public function renderForm( $params = null  )
  {
    ...
    $this->input_CorePerson_Gender( $params );
    ...
    $this->input_CorePerson_IsAliasFor( $params );
    $this->input_CorePerson_TaxNumber( $params );
    $this->input_CorePerson_Pkid( $params );
    $this->input_CorePerson_Mimetype( $params );
  ...
  * @return void
  */
  public function input_CorePerson_Gender( $params )
  {
    $i18n     = $this->view->i18n;
    
    if( !isset( $this->listElementData['core_person_gender'] ) )
    {
      ...
      //p: Selectbox
      $inputGender = $this->view->newItem( 'inputCorePersonGender', 'BaseGender_Selectbox' );
      $this->items['core_person-gender'] = $inputGender;
      $inputGender->addAttributes
      (
        array
        (
          'name'      => 'core_person[gender]',
          'id'        => 'wgt-input-core_person_gender'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $i18n->l( 'Insert value for {@attr@} ({@src@})', 'wbf.label', array( 'attr' => 'Gender', 'src' => 'Person' ) ),
        )
      );
      ...
      $inputGender->setReadonly( $this->fieldReadOnly( 'core_person', 'gender' ) );
      $inputGender->setRequired( $this->fieldRequired( 'core_person', 'gender' ) );


      $inputGender->setLabel( $i18n->l( 'Gender', 'core.person.label' ) );
      ...
      if( $acl->access( 'mod-crm>mgmt-crm_contact_calls:insert' ) )
      {
        ...
        $inputGender->editUrl = 'index.php?c=Crm.ContactCalls.listing&amp;target='.$this->namespace.'&amp;field=gender&amp;publish=selectbox&amp;suffix='.$this->suffix.'&amp;input_id=wgt-input-core_person_gender'.$this->suffix;
      }
      ...
      if( !isset( $this->listElementData['core_person_gender'] ) )
      {
        // daten der selectbox daten
        $inputGender->setData( DaoSelectbox::get( 'base_gender' ) );
      
      }
      else
      {
        $inputGender->setData( $this->listElementData['core_person_gender'] );
      }
  ...
  }//end public function input_CorePerson_Gender */
  ...
  * @return void
  */
  public function input_CorePerson_IsAliasFor( $params )
  {
    $i18n     = $this->view->i18n;
    ...
    if( !Webfrap::classLoadable( 'CorePerson_Entity' ) )
    {
      ...
      $inputIsAliasFor = $this->view->newInput( 'inputCorePersonIsAliasFor', 'Window' );
      $this->items['core_person-is_alias_for'] = $inputIsAliasFor;
      $inputIsAliasFor->addAttributes(array
      (
        'readonly'  => 'readonly',
        'name'      => 'core_person[is_alias_for]',
        'id'        => 'wgt-input-core_person_is_alias_for'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
        'title'     => $i18n->l( 'Insert value for {@attr@} ({@src@})', 'wbf.label', array( 'attr' => 'Is Alias For', 'src' => 'Person' ) ),
      ));
      ...
      $inputIsAliasFor->setReadonly( $this->fieldReadOnly( 'core_person', 'is_alias_for' ) );
      $inputIsAliasFor->setRequired( $this->fieldRequired( 'core_person', 'is_alias_for' ) );
      $inputIsAliasFor->setLabel( $i18n->l( 'Is Alias For', 'core.person.label' ) );


      $listUrl = 'modal.php?c=Core.Person.selection'
        .'&amp;suffix='.$this->suffix.'&amp;input=core_person_is_alias_for'.($this->suffix?'-'.$this->suffix:'');
      ...
      $inputIsAliasFor->buildJavascript( 'wgt-input-core_person_is_alias_for'.($this->suffix?'-'.$this->suffix:'') );
  ...
  }//end public function input_CorePerson_IsAliasFor */
  ...
  * @return void
  */
  public function input_CorePerson_TaxNumber( $params )
  {
    $i18n     = $this->view->i18n;

      //tpl: class ui:text
      $inputTaxNumber = $this->view->newInput( 'inputCorePersonTaxNumber' , 'Text' );
      $this->items['core_person-tax_number'] = $inputTaxNumber;
      $inputTaxNumber->addAttributes
      (
        array
        (
          'name'      => 'core_person[tax_number]',
          'id'        => 'wgt-input-core_person_tax_number'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $i18n->l( 'Insert value for {@attr@} ({@src@})', 'wbf.label', array( 'attr' => 'Tax Number', 'src' => 'Person' ) ),
          'maxlength' => $this->entity->maxSize( 'tax_number' ),
        )
      );
      $inputTaxNumber->setWidth( 'medium' );

      $inputTaxNumber->setReadonly( $this->fieldReadOnly( 'core_person', 'tax_number' ) );
      $inputTaxNumber->setRequired( $this->fieldRequired( 'core_person', 'tax_number' ) );
      $inputTaxNumber->setData( $this->entity->getSecure('tax_number') );
      $inputTaxNumber->setLabel( $i18n->l( 'Tax Number', 'core.person.label' ) );
  ...
  }//end public function input_CorePerson_TaxNumber */
  ...
  * @return void
  */
  public function input_CorePerson_Pkid( $params )
  {
    $i18n     = $this->view->i18n;

      //tpl: class ui:text
      $inputPkid = $this->view->newInput( 'inputCorePersonPkid' , 'Text' );
      $this->items['core_person-pkid'] = $inputPkid;
      $inputPkid->addAttributes
      (
        array
        (
          'name'      => 'core_person[pkid]',
          'id'        => 'wgt-input-core_person_pkid'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $i18n->l( 'Insert value for {@attr@} ({@src@})', 'wbf.label', array( 'attr' => 'Personal Ident', 'src' => 'Person' ) ),
          'maxlength' => $this->entity->maxSize( 'pkid' ),
        )
      );
      $inputPkid->setWidth( 'medium' );

      $inputPkid->setReadonly( $this->fieldReadOnly( 'core_person', 'pkid' ) );
      $inputPkid->setRequired( $this->fieldRequired( 'core_person', 'pkid' ) );
      $inputPkid->setData( $this->entity->getSecure('pkid') );
      $inputPkid->setLabel( $i18n->l( 'Personal Ident', 'core.person.label' ) );
  ...
  }//end public function input_CorePerson_Pkid */
  ...
  * @return void
  */
  public function input_CorePerson_Mimetype( $params )
  {
    $i18n     = $this->view->i18n;

      //tpl: class ui:hidden
      $inputMimetype = $this->view->newInput( 'inputCorePersonMimetype', 'Hidden' );
      $this->items['core_person-mimetype'] = $inputMimetype;
      $inputMimetype->addAttributes
      (
        array
        (
          'name'      => 'core_person[mimetype]',
          'id'        => 'wgt-input-core_person_mimetype'.($this->suffix?'-'.$this->suffix:''),
          'class'     => 'wcm wcm_ui_tip medium'.($this->assignedForm?' asgd-'.$this->assignedForm:''),
          'title'     => $i18n->l( 'Insert value for {@attr@} ({@src@})', 'wbf.label', array( 'attr' => 'Mimetype', 'src' => 'Person' ) ),
  ...
  }//end public function input_CorePerson_Mimetype */
...
}//end class CorePerson_Crud_Show_Form */
