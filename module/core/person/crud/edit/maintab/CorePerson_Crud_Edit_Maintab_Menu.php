 ...
 * @licence WebFrap.net
 */
class CorePerson_Crud_Edit_Maintab_Menu
  extends WgtDropmenu
{
    ...
    <div class="wgt-dropdownbox" id="{$this->id}-cruddropbox" >
      <ul id="{$this->id}-cruddrop-menu"  >
        <li><a
          ...
          title="Save the Person and close the Tab after saving" >{$iconSaveUCl} {$view->i18n->l('Save &amp; Close','wbf.label')}</a></li>
      </ul>
...
BUTTON;
    ...
    if( $params->access->admin )
    {
      ...
      $entries->entryAcls = <<<HTML
          ...
          <li>
            <a 
              href="maintab.php?c=Acl.Mgmt_Dset.listing&amp;dkey=core_person&amp;objid={$objid}" 
...
{$this->switchDataProfile( $objid, $params )}
  <ul>
{$entries->customStart}
{$entries->masks}
{$entries->profiles}
...
HTML;
  ...
  <div class="wgt-panel-control" >
    <button
      class="wcm wcm_ui_button wgtac_back_to_list wcm_ui_tip-top"
      title="Go back to the Persons list" >{$iconBack} {$view->i18n->l('Back to the list','wbf.label')}</button>
  </div>
      ...
      <li>
        ...
        <ul>
          ...
          <li><a 
            ...
            href="modal.php?c=Webfrap.Docu.open&amp;key=core_person-edit" >{$iconHelp} {$this->view->i18n->l('Help','wbf.label')}</a></li>
    ...
    $html = <<<HTML
      ...
      <li>
        ...
        <ul>
          ...
          <li>
            <a class="wcm wcm_req_ajax" href="modal.php?c=Maintenance.Entity.protocolDataset&dkey=core_person&amp;objid={$objid}" >{$iconProtocol} {$this->view->i18n->l( 'Protocol', 'wbf.label' )}</a>
          </li>


          <li>
            <a class="wcm wcm_req_ajax" href="modal.php?c=Maintenance.Entity.statsDataset&dkey=core_person&amp;objid={$objid}" >{$iconStats} {$this->view->i18n->l( 'Stats', 'wbf.label' )}</a>
          </li>


          <li>
            <a class="wcm wcm_req_ajax" href="modal.php?c=Maintenance.Entity.showMeta&dkey=core_person&amp;objid={$objid}" >{$iconMetaData} {$this->view->i18n->l( 'Meta Data', 'wbf.label' )}</a>
          </li>
...
HTML;
   ...
   * @param TArray $params
   */
  protected function switchDataProfile( $objid, $params )
  {
    ...
    $iconDataProfiles         = $this->view->icon('control/data_profiles.png'      ,'Data Profiles');
    $iconDataProfile         = $this->view->icon('control/data_profile.png'      ,'Data Profile');
  ...
  <ul>
  <li>
    <a>{$iconDataProfiles} {$this->view->i18n->l('Profiles','wbf.label')}</a>
    <span>
    <ul>
HTML;


      if( $this->acl->access( 'mod-wbfsys>mod-wbfsys-cat-core_data:access', null, true ) )
      {
        $html .= '<li>'
          . '<a class="wcm wcm_req_ajax" href="maintab.php?c=Wbfsys.RoleUser.editWhere&amp;objid='
          . $objid.'&attr=id_person" >'
          . $iconDataProfile.' '
          . $this->view->i18n->l( 'System User', 'wbfsys.role_user.label' )
          . '</a></li>'.NL;
        ++ $numEntries;
      }
...
HTML;
  ...
  }//end public function switchDataProfile */

}//end class CorePerson_Crud_Edit_Maintab_Menu
